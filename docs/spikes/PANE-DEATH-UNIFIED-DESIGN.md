...
| **D7** | 락 8의 "연속 2회 absent → terminal 합성"만 개정 | **개정(범위 확대)** | 그 행만이 아니다 — issuer 단일 발행 모델에서 absent-합성 failed는 **initial 발행 예산을 소진**해 살아있는 pane의 진짜 결과를 영구 봉쇄한다(§1.4). 구 락 11의 "연속 absence = 결정적 부정 증거" 문구·구 락 3의 `terminal_uncommitted` 도착지도 함께 개정(교훈 27), §9-ter 항목 3을 본 설계가 **종결**한다 |
...
| strict ACK | `classifyAck` `:2687-2699` · 3분기 `SendResultOutcome` `:2671-2679` · ACK 봉투 전량 소비 `:2728-2755` | **비수락 통일** — §2.3 ① |
...
- 브랜치 proposal builder의 순이득 1건을 편입: 성공형 제안은 `failed` status에서도 **실제 output
  ...
  main의 현행 failed-summary 치환(`failed reason=…` `:2578-2580`)을 성공형 제안에 쓰지 않는다.
...
| ACK 주입 production seam | **배선·동작** | `relay-client.ts:352-380` (`__ackInjector`) — §6.7-bis 함정 3("relay 신규 주입 표면" 비용)은 이미 지불됨 |
...
gate(`card-ops.ts:137-278`) · ACK 주입 seam(`relay-client.ts:352-380`) · `impl-0270.test.ts`
(§4.3 재기술 후).
...
| 구독-대기 헬퍼 `hasPaneStatusSubscription` | `…pane-cleanup.test.ts:141-163` · `still-running.test.ts:174-193` · `impl-02311:342-357` · `impl-02312:252-267` · `impl-0239:296-311` | 이식 |
| `test-setup.ts` env 오염 차단 | `ec99b2c:scripts/test-setup.ts:9-18` (diffstat +10 실측) | 이식 |
| reinject CR 관측 확장 · 픽스처 시딩 레이스 제거 · close 정확-개수 패턴 | `inject-verify:260-275` · `scrape-delta:527-529,548-549` · `herdr-lifecycle:366-367` (인용) | 이식 |
