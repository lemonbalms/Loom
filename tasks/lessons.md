...
- [workers] 2026-07-19 (9) sonnet claude-mem 거부 루프: sonnet이 마커형 페이로드 거부 → 그 거부가 claude-mem에 저장되어 후속 benign 페이로드까지 재거부. 대응: ① 처음부터 benign goal-ack형 설계 ② 거부 시 새 카드로 페이로드 교체 ③ 무오염 확인 목적이면 거부 회신 자체도 증거로 유효. **심화(2026-07-20 hookSensor 스모크)**: 공유-홈 오염이 자연문 benign 승인 대기 유발까지 재거부(A 거부→C·D 강화) → 우회 = 수동 pane 스폰(`herdr agent start`+`agent send` — 브릿지 dispatch와 달리 untrusted 마커 미부착)으로 permission_prompt 실발화 성공 · 오너 홈 `defaultMode:auto`는 benign Bash 자동 승인이라 재현에 워커 argv `--permission-mode default` 스왑 필수. **갱신 2026-07-20 (0.26.1)**: 신 마커+benign goal-ack는 오염 홈에서도 sonnet 무거부 완주 1회 실증(SMOKE-SONNET26) — 단 변인 2개(마커·페이로드형)라 마커 단독 효과 단정 금지.
