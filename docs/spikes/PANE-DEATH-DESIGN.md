...
| 결과 커밋 전 terminal event 이후 `done` 금지 | ✅ | terminal fence + **로컬 발행 1회**(local single-issue) arbitration |
...
spawned → submitted → running → completion_candidate → result_sending
    ...
    └─ (result_sending 미진입) terminal                        └─ (result_sending 진입 후) terminal
         ...
         → failed/blocked (로컬 발행 1회)                             ACK 판정이 아래 3분기로 종결한다
```
...
| 조건 | 전이 | result |
|---|---|---|
| strict ACK 성공(`status ∈ {queued, delivered}` ∧ `recipientCount=1`) | `result_relay_accepted` → `cleanup_requested` → `terminal_expected` | 추가 발행 **없음** |
| 명시적 거부 | 단일 소유자가 `failed`로 종결 | failed 1회 |
| transport error / ACK 유실 | **`commit_unknown`** | **두 번째 result 금지 · cleanup 금지** |
...
type PaneLifecycle = {
  ...
  phase: "spawned" | "submitted" | "running" | "completion_candidate" |
    "result_sending" | "result_relay_accepted" | "cleanup_requested" |
    "commit_unknown" | "terminal_uncommitted" | "terminal_expected";
  sawWorking: boolean;
...
1. **Terminal fence**: `pane.closed`/검증된 `pane.exited`/bounded reconcile absent가
   `result_relay_accepted` 전에 관측되면 completion task/poll을 취소한다. 이후 어떤 late status/read도
   `done`을 만들 수 없다.
...
3. **Activity fence**: agentKind별 active indicator가 남아 있으면 completion을 유예한다.
   ...
   → **정본은 §9-bis 락 10.**
4. **Result commit**: 위 게이트를 통과한 scrape로 v1 result를 만들고, `sendResult()`의 ACK가
   **strict 판정**(`status ∈ {queued, delivered}` ∧ `recipientCount=1`)을 통과할 때만
   `result_relay_accepted`다. 그 전에는 board 관점 완료가 아니다.
   → **strict ACK 판정과 `commit_unknown` 메커니즘의 정본은 §9-bis 락 5.**
5. **Cleanup**: **동일 generation의 lifecycle이 `result_relay_accepted → cleanup_requested`를 먼저
   완료한 경우에만** 뒤따르는 code 129/SIGKILL/`pane.closed`를 `terminal_expected`로 분류하고 추가
   failed result를 보내지 않는다. 그 밖의 모든 terminal은 uncommitted 후보다(락 1).
6. **Uncommitted terminal**: `result_sending`에 **진입한 적 없는** flight의 uncommitted terminal은
   failed result를 **로컬 발행 1회**(local single-issue) 보낸다.
   - `status="failed"`
...
8. **status event는 advisory다.** `pane.agent_status_changed`는 단독으로 완료 근거가 될 수 없다.
   완료 판정은 **start evidence + activity fence(락 10)** 와 **result commit(락 5)** 를 함께
   만족해야 성립한다.
...
| 케이스 | 관측 | 초안 판정 | 왜 맞는가 |
|---|---|---|---|
| **A — SIGHUP, 산출물 정상** | pane.close 뒤 exit 129; 결과/marker 회수 시점은 로그 불충분 | 동일 generation이 `result_relay_accepted → cleanup_requested`를 먼저 완료했으면 `terminal_expected`(추가 발행 없음). `result_sending` 미진입이면 `failed/pane_terminated_before_commit` → board blocked. `result_sending` 진입 후면 `terminalPending` latch 뒤 ACK 3분기 — 명시적 거부는 failed 1회, transport error/ACK 유실은 `commit_unknown`(두 번째 result·cleanup 금지) | SIGHUP 자체를 실패 의미로 쓰지 않으며, 정상 산출물을 자동 폐기하지도 않음. 커밋 여부가 불명일 때 실패로 단정하지도 않음 |
...
다음에는 prompt echo나 반제품을 성공으로 승인한다. **정확한 기계 분류는 `committed success` ·
`uncommitted/needs verification` · `commit_unknown`(커밋 여부 미상 — 추가 발행도 cleanup도 금지)의
구분**이다. board `blocked`는 실패 확정이 아니라 타워 검증 요청 상태다.
...
- terminal event가 deferral 중 오면 timer를 취소한다. 이후 처리는 **`result_sending` 진입 여부로
  ...
  - **진입 후**(deferral 취소와 `result_sending` CAS가 경합해 순서가 뒤집힌 경우) — 발행하지 않고
    ...
    error·ACK 유실 → `commit_unknown`)에 판정을 맡긴다.
...
5. strict ACK 성공(`status ∈ {queued, delivered}` ∧ `recipientCount=1`) → `result_relay_accepted` →
   `cleanup_requested` → pane.closed/SIGHUP 상당 event → done **로컬 발행 1회**, failed 0건.
   `terminalPending`이 latch돼 있어도 추가 발행 0건.
   또한 ACK가 **명시적 거부**면 단일 소유자가 failed로 종결(로컬 발행 1회)하고 cleanup을 하지 않는다.
6. ACK가 **transport error / 유실** → **`commit_unknown`**; 두 번째 result 0건, `pane.close` 0건,
   binding이 send 전에 사라지지 않음. (**`blocked` 아님** — 커밋 여부가 미상이므로 실패로 단정하지
   않는다.) `commit_unknown` 보존 10분 만료 시 structured log + counter 1건 발행 후 dispose.
...
8. `flights.set()` +1s 초기 presence check 동작; event 누락 + `pane.list` absent 1회 → 유지;
   grace 3s 이상 간격의 연속 2회 absent → terminal 합성; pane 재등장 → strike reset·취소;
   한 episode 12회(≈60s) 초과 → 무한 폴링 없이 `commit_unknown` 전이.
...
| strict ACK 성공 뒤 bridge auto-close | done 1, failed 0, expected cleanup log(동일 generation의 `result_relay_accepted → cleanup_requested` 선행 확인) |
| final marker처럼 보이는 text 뒤 ACK 전 kill | auto done 0, `completion_evidence` note + blocked |
| `result_sending` 중 kill + 이후 strict ACK 성공 | `terminalPending` latch 1, **추가 result 0**, done 1 |
| `result_sending` 중 kill + ACK transport error/유실 | **`commit_unknown` 1**, result 0(두 번째 발행 없음), `pane.close` 0, 10분 만료 시 운영자 가시 log+counter 1 |
...
1. `result_relay_accepted` 정의가 strict ACK 판정(`status ∈ {queued, delivered}` ∧
   `recipientCount=1`)으로 충분한가 — 그리고 이름이 보장 경계(relay-accept commit)를 정확히 표현하는가
...
5. **이중 발행 방지 — 발행 소유권 CAS + in-flight terminal 보류** *(strict ACK 판정과 `commit_unknown`
   메커니즘의 **정본**. 락 9는 이를 참조만 한다.)*
...
8. **bounded reconcile strike — 연속성·reset·generation 명시**
   ...
   **cadence 수치 (이번 통합 패스에서 확정):**
   ...
   | 한 reconcile episode 최대 시도 | **12회(≈60s)** — 초과 시 무한 폴링 대신 `commit_unknown`으로 전이 |
   | 시작 | `flights.set()` |
   | 중단 | `terminal_expected` · `terminal_uncommitted` 확정 · `commit_unknown` 진입 중 하나 |
   | `commit_unknown` 보존 | **10분**. 만료 시 **운영자 가시 신호(structured log + counter) 발행 후** dispose |
   | `commit_unknown`의 reconcile 참여 | **없음**(이미 cleanup 금지 상태) — tombstone만 유지 |
   | 프로세스 종료 시 `commit_unknown` | 운영자 가시 로그로 남긴다(조용한 소멸 금지) |
...
9. **범위 판정 — 이번 PATCH 채택 범위 / wire exactly-once는 범위 밖 / ACK 보장 경계**
   ...
   **메커니즘 정본은 락 5다.** 이 항목은 그 메커니즘을 **참조만** 하고, 범위 판정과 보장 경계에 집중한다.
   ...
   락 5가 규정하는 `commit_unknown` 상태와 strict ACK 판정은 로컬 변경으로 저렴하므로 **이번 PATCH에
   채택**한다. 반면 `(cardId, seq, dispatchHandoffId)` 기반 relay/tower idempotency와 inbox reconcile은
   relay 표면 변경이 필요하므로 **별도 PATCH로 분리**한다. 그것이 없는 동안 이 PATCH는
...
- [ ] B phase registry + terminal fence 구현 — union에 `commit_unknown` 포함,
      `terminalPending`은 phase가 아닌 별도 필드(§6.1)
...
- [ ] strict ACK 판정(`status ∈ {queued, delivered}` ∧ `recipientCount=1`) + 3분기 종결
      (성공 / 명시적 거부 / `commit_unknown`) 구현(락 5)
...
- [ ] bounded reconcile 구현(한 번의 miss로 terminal 판정 금지) — 초기 +1s · poll 5s · grace 3s ·
      연속 2회 · episode 최대 12회(≈60s) 초과 시 `commit_unknown`(락 8)
- [ ] `commit_unknown` 보존 10분 + 만료·프로세스 종료 시 운영자 가시 log/counter(조용한 소멸 0건)
- [ ] unit/integration §7.1 green (특히 4·5·6의 `result_sending` 진입 여부 분기)
- [ ] Mac Smoke 1 green
