...
### G1 구현 전 producer/consumer scan 증거 (2026-07-21)

tracked source·scripts·루트 운영 스크립트를 `CARD_RESULT_LABEL`, `buildResultBody`,
`serializeCardAttachment`, `parseResultFromAttachments`, `applyCardResult`, `intent: card.done`,
`[DONE]` 기준으로 전역 scan했다.

- **production producer 1곳:** `packages/host/src/bridge-runtime.ts`의 `sendResult`만
  `loom-card-result` attachment와 result body를 만든다. 현행 호출 표면은
  `sendFailedResult` 7곳 + `finishCard` 11곳이지만 모두 이 단일 발행 함수로 수렴한다.
  `packages/protocol/src/card-contract.ts`는 schema/body builder이고 `dist/loom.js`는 source bundle
  mirror이므로 별도 producer가 아니다.
- **board mutation consumer 1곳:** MCP stdio `apply_card_result` → `toolApplyCardResult` →
  `packages/host/src/card-ops.ts:applyCardResult`만 remote result를 board에 반영한다.
- **repo 운영 consumer 21개:** tracked `.loom-claim*.ts` 21/21이
  `parseResultFromAttachments`로 구조화 attachment를 읽고 같은 `applyCardResult`를 호출한다.
  `[DONE]`가 등장하는 곳은 handoff id 주석뿐이며 body 단독으로 board를 전이시키지 않는다.
- `scripts/watch-card.ts`의 `[DONE]` 예시는 pane marker 감시용 범용 CLI 사용례이고 remote
  card-result/board consumer가 아니다. CLI inbox의 body parse는 표시용이며 board mutation이 없다.
- 따라서 **repo 안 body-only 자동 board consumer는 0**이다. 새 bridge→구 Tower는 기존
  `failed→blocked`, 구 bridge→새 Tower는 G2/G3의 ingress fence로 `done→blocked`가 되어 양방향
  순차 업그레이드가 fail-closed한다.

**외부 rollout gate:** 저장소 밖 producer/consumer 목록은 repo 증거로 부재를 주장하지 않는다.
배포 전 운영자가 관리하는 목록을 확인해야 하며, 하나라도 있으면
`failed + needs_verification`과 remote `done→blocked`를 먼저 수용하도록 이행하거나 배포를
중단한다. 확인되지 않은 body-only 자동화는 지원 계약 밖이다. repo scan은 G2 착수를 허용하지만
이 운영자 확인 전 배포는 허용하지 않는다.
