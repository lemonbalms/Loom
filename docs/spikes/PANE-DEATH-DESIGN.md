...
**착수 전 아키텍트 결정 2건 (구현자에게 넘기면 임의로 고른다):**

1. **명시적 거절 후 발행되는 failed result 자체의 ACK 처리** (3차 Medium 미결) —
   구현 A는 그 failed send에도 strict ACK를 **재귀 적용**해 유실 시 `commit_unknown`으로 가고,
   구현 B는 **로컬 최종**으로 보고 latch 해제 후 dispose한다.
2. **`flight.seq` ↔ `cardSeq` 두 seq 출처 통일** — `finishCard`는 스폰 시 고정된 `flight.seq`를,
   `sendFailedResult`는 단조 증가 `cardSeq`를 쓴다. 통일 없이 `(cardId, seq, dispatchHandoffId)`
   멱등키를 붙이면 **같은 카드의 done과 failed가 서로 다른 seq 공간에서 나온다.**
