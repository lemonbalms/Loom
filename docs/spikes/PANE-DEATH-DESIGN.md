...
- **레코드 내용 = 메타데이터만:** `cardId · seq · dispatchHandoffId · 상태 · 시각 · 사유 · 카운터`.
  워커 산출물 본문 금지(0.26.0 D6 계측 레코드 규약과 동형 — 본문성 §5.1 · 간접 프롬프트 주입
  표면 차단).
- **dedup 키 = `(cardId, dispatchHandoffId, seq)`** (payload 각인 튜플과 동일 — 결정 2).
- **flush = 레코드당 즉시 append.**
- **보존 = 운영자 ack까지.**
- **표면화:** 브릿지 상태 표면(`loom bridge status`·doctor 등)이 **미해소 quarantine 카운트를
  표시**한다(조용한 축적 금지).

**해제:** in-memory unknown 레코드의 해제는 (a) **운영자 ack** 또는 (b) **프로세스 종료**(로그 필수)
뿐이다. durable 레코드가 진입 시점부터 존재하므로 in-memory 해제는 **정보 파괴가 아니다.**
**타이머 유발 dispose는 금지** — 10분 타이머는 **재알림(re-escalate)만** 유발한다(가역 행동, 불변식
정합). *(자동 reconcile 회수는 여전히 범위 밖 — §6.7 요구 4 · 결정 ⑧.)*
...
- **`:850` 특례:** cardId가 `extractCardIdLoose(att.content) ?? "task_0"`로 **열화**하므로(실측 확인),
  이 경로는 **`dispatchHandoffId` 단독 키**로 동작한다(`h.id`는 신뢰 가용).
...
| 현 상태 | 이벤트 | 다음 |
|---|---|---|
| 구성 | send | `relay_accepted`(종결) · 명시적 거부 · transport/ACK 유실 |
| 명시적 거부 | failed 재구성 **1회** | send |
| failed send | strict ACK | `relay_accepted`(종결) · `send_unknown`(흡수) |
| transport/ACK 유실 | — | `send_unknown`(흡수 — 종결) |

재귀 깊이는 구조적으로 **2**에서 끝난다(`needs_verification` → `failed` → 흡수). ACK 판정은 send
...
매 적용 시 fresh read)인 반면 브릿지 `cardSeq`/issuer seq는 **in-memory 휘발**이다. 따라서
**현행 설계도 브릿지 재시작만으로 result 삼킴이 재현된다**(재시작 후 seq=1 < 디스크 last_seq=N).
이 유실은 R43이 신설을 요구한 문제가 아니라 **현행에 이미 존재하는** 문제다.
...
**게이트(같은 PATCH):** `applyCardResult`에서 —

- `payload.dispatchHandoffId` 존재 ∧ `task.handoffId`(영속본)와 **불일치** → **stale attempt의 늦은
  result로 drop**(관측 로그 + counter — 조용한 삼킴 금지).
- 일치 → per-dispatch `seq` dedup.
- **필드 부재 → 현행 스칼라 폴백**(하위 호환 — `dispatchHandoffId`는 스키마 optional
  `card-contract.ts:42-45`, 단 브릿지 생성 result는 항상 채운다).

**근거:** (i) R43 H3 — issuer-local seq는 재dispatch마다 재시작해 현행 스칼라 게이트에 삼켜진다;
각인과 판정자는 분리 불가 (ii) 순수 튜플 dedup은 현행 스칼라가 attempt-교차 단조성으로 우연히
막던 **stale-attempt 늦은 result 수용**을 새로 연다(fable-advisor 보정) — 그래서 currency 게이트가
필요하다 (iii) 비교 값 2개가 이미 같은 함수 스코프에 있어 새 저장소·전송 불요 (iv) 부수 이득 —
현행에도 존재하는 **브릿지 재시작 seq 리셋 유실**이 함께 해소된다(`dispatchHandoffId`는 relay
서버 생성 + 보드 영속이라 브릿지 메모리 리셋에 면역).
...
8. **bounded reconcile strike — 연속성·reset·generation 명시**
   ...
   **cadence 수치 — 알림·재평가 전용 운영 기본값 (§6.6):**
   ...
   | unknown 상태 **해제** | (a) 운영자 ack 또는 (b) 프로세스 종료(로그 필수)뿐. durable 레코드가 진입 시점부터 존재하므로 in-memory 해제는 정보 파괴가 아니다 |
   ...
   **재알림만** 유발하며(가역 행동), (c) in-memory 해제는 **운영자 ack 또는 프로세스 종료**만
   유발한다. 이로써 이 락 블록은 자기모순 0이 된다. **매체 = 브릿지-로컬 append-only JSONL**(§6.7.1
...
13. **불확실성 2종 분리 + 상수의 지위**
    ...
    *R43b 정합 (결정 ①):* 두 unknown 상태는 **진입 시점에 durable quarantine 레코드 1건**을 남긴다
    ...
    만료는 dispose가 아니라 재알림만** 유발한다(락 8 개정 — 결정 ②). in-memory 해제는 운영자 ack
    또는 프로세스 종료뿐이고, durable 레코드가 진입 시점부터 존재하므로 해제가 정보를 파괴하지 않는다.
...
- [ ] **tower currency 게이트(§6.7.3) 편입 — `applyCardResult`에서 `payload.dispatchHandoffId` ↔
      ...
      부재=현행 스칼라 폴백. 브릿지 재시작 seq 리셋 유실도 함께 해소(락 9 범위)**
- [ ] **durable quarantine 레코드(§6.7.1) — 브릿지-로컬 append-only JSONL, unknown 상태 **진입
      시점** 기록, dedup 키 `(cardId, dispatchHandoffId, seq)`, 메타데이터만(본문 금지), 보존=운영자
      ack까지, 미해소 카운트 표면화. durable alert와 동일 1건(이중 기록 0건).
      `recordResultDeliveryUnconfirmed`(휘발) 재사용 0건**
...
- [ ] unknown 상태 진입 시 durable quarantine 1건 + 10분 타이머는 **재알림만**(만료 dispose 0건 —
      결정 ②) + 해제는 운영자 ack·프로세스 종료만 + 프로세스 종료 시 운영자 가시 log/counter(조용한
      소멸 0건)
...
휘발이라 재사용 금지 명기. 좌표 전면 정정(발행 호출지점 17→**18** 등). 자동 reconcile 회수(§6.7
