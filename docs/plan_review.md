...
- **R38 M-1 (v0.24.1, PLAN 문안 lock)** — D6 헬퍼 추출 경계: 헬퍼 = env 판정 → `RoomRegistryOptions`(`{ephemeral:true}` 또는 `{stateDir}`) 반환까지만 · `new RoomRegistry` 생성·try/catch·에러 문구·exit는 각 호출자 유지(`relay/cli.ts` 기존 문구·경로 그대로) · 예시명 `resolveRegistryOptionsFromEnv()` 류로 개명 · 소규모 복제 폴백 시에도 동일 경계. **반영 후 재리뷰 없이 `approved` 전환 가능 (no R38b — Fable 사전 승인).**
- (R24–R37은 전부 closed.)
