/**
 * SMOKE-VPS1 card.done 결과를 타워 보드에 반영 + 인박스 정리.
 * card.done handoff: <FILL AFTER DISPATCH> (from node-vps-1) — SMOKE-VPS1 card.
 * 실행: env -u LOOM_RELAY_URL -u LOOM_RELAY_TOKEN LOOM_PROFILE=claude-impl bun run .loom-claim-smoke-vps1.ts
 * 선례: .loom-claim-smoke-wsl2d.ts (parse→apply→accept).
 */
...
// FIXME: 디스패치 후 node-vps-1이 보낸 [DONE] card.done handoff id로 교체.
const doneId = "ho_b3b593bf1993fbbb"; // [DONE] card.done from node-vps-1 (SMOKE-VPS1)
...
console.log(`\nSMOKE-VPS1 board transition: ${applied.task.id} -> ${applied.status}`);
