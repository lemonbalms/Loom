...
const task = addTask({
  title: "HOOKCACHE-B — 원인 B B-4 소스 패치: claude-mem worker-service.cjs qJ()/eQ() 분 ts 제거 + 감지장치 (codex pane)",
  notes: "IMPL · brief=.loom-hookcache-B-brief.md · impl=codex · 오너 락 (b)=B-4 + 감지장치 · 홈 blast radius 주의",
});
...
const result = await dispatchCard({
  ...
  prompt: [
    ...
    "요지: 당신은 codex 구현자다. claude-mem이 SessionStart로 주입하는 헤더의 분 단위 타임스탬프를 제거해",
    "프롬프트 캐시 재사용 상한 1분 고정(원인 B)을 해소한다.",
    "중요 — 설계 문서 §5가 지목한 scripts/context-generator.cjs는 데드 번들이다(호출처 0건). 거기를 고치면 무효다.",
    "실제 대상은 ~/.claude/plugins/cache/thedotmack/claude-mem/13.11.0/scripts/worker-service.cjs 의",
    "953행 qJ()(플레인 헤더)와 955행 eQ()(ANSI 헤더) 두 함수이며 둘 다 날짜까지만 반환하도록 고친다.",
