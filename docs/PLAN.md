...
#### 0.28.1 — 2026-07-22 (`approved` **R46 author-close — High 0 · Medium 1(중복 제출 방지 4절 lock) · Low 4 전량 반영 · no R46b** — **herdr 0.7.5 / protocol-17 호환 어댑터 — 기존 pane named start · 원자적 `agent.prompt` 제출 · env·cwd의 `tab.create`/`pane.split` 선행 이관** (PATCH))

> **권위 문서 고지 (2026-07-22).** 이 버전의 **유일한 정본은 [`docs/spikes/HERDR-0.7.5-COMPAT.md`](spikes/HERDR-0.7.5-COMPAT.md) §1~§6**이다. 본 PLAN 항목은 그 문서를 **제품 게이트 단위로 분해**하는 문서이지 와이어 사실을 재서술하는 문서가 아니다 — 스키마·메서드·에러 코드의 축자 근거는 COMPAT §2·§3의 실측 표를 참조하고 여기에 복제하지 않는다(중복 서술 = 요약-본문 드리프트 = 이 리포 reject의 근인). **COMPAT가 명시하지 않은 어댑터 설계는 이 버전에서 발명하지 않는다.**
>
> **v0.28.0 PANE-DEATH 락은 전량 보존된다.** 락 **U1~U11** · 회귀 게이트 **G-1~G-11** · PATCH 1–5 커밋(`24ceede` → `d49a6b1`)을 이 버전은 건드리지 않는다. 0.7.5가 detect 이벤트에 실을 수 있는 **`final_status` · `released`** 는 **파서가 관용(tolerate)** 하되 **완료 권위로 절대 쓰지 않는다**(COMPAT §4 말미 축자 · 락 U1·U2·U3). 이것이 이 버전의 **하드 경계**다 — 어댑터가 새 필드를 근거로 board `done`을 만들거나 pane을 닫으면 권한 컷이 무효화된다.

**Product one-liner:** 로컬 herdr가 **0.7.4/protocol 16 → 0.7.5/protocol 17**로 올라가면서 Loom의 워커 스폰·주입 경로가 **와이어 레벨에서 무효**가 됐다 — `agent.start`는 이제 `argv`/`env`/`cwd`/`tab_id`/`workspace_id`/`split`/`focus`를 받지 않고 **이미 존재하는 pane**에 `name`+`kind`+`pane_id`로 붙으며, 텍스트 주입에 쓰던 **`agent.send`는 protocol 17의 메서드가 아니다**(COMPAT §3 실측). v0.28.1은 이 어댑터를 tests-first로 이식한다: **①** pane을 `tab.create`/`pane.split`으로 **먼저** 만들고 거기서 `env`(`LOOM_CARD`·`LOOM_HOOK_SOCK`)와 `cwd`를 넘긴다 → **②** `agent.start{name,kind,pane_id}`로 named start → **③** **원자적 `agent.prompt`** 로 제출한다(`agent.send_keys`는 명시적 키 조합 전용) → **④** 이벤트·`agent.wait`·스크레이프·`pane.close`의 **기존 완료 정책은 무변경**. 어댑터가 서기 전까지 `dogfood:herdr`/`dogfood:up`은 호스트 변경 **이전에** fail-closed다.

**Why:**
- **config만 17로 올리는 것은 우회다(COMPAT §1 · §5-2 락).** `herdrProtocol`만 17로 바꾸면 기동 ping은 통과하고 전역 이벤트 구독도 살아 있을 수 있으나 **첫 워커 스폰이 protocol-16 형상으로 나가 실패한다.** 그래서 `scripts/dogfood-herdr-check.sh`는 **라이브 0.7.5/17 그리고 커밋된 `HERDR_PROTOCOL_EXPECTED === 17`** 을 둘 다 요구하며, 상수 단독 bump는 금지다(COMPAT §4 표 1행 축자 — "never alone").
- **스키마 버전 필드가 그대로라서 조용히 깨진다.** 0.7.4·0.7.5 양쪽 `schema version = 1`인데 **계약은 깨졌다**(COMPAT §3 표 1행). 버전 필드로는 감지되지 않으므로 **픽스처 대조가 유일한 기계적 방어선**이다.
- **주입 경로가 방법 자체를 잃었다.** `injectPromptAndSubmit`의 protocol-16 M-2 구현(두 번의 독립 `agent.send` + 고정 `BARE_ENTER = "\r"`)은 **호출할 메서드가 없다.** 0.7.5는 대신 **원자적 `agent.prompt`**(bracketed-paste 존중, `A != B` 같은 텍스트에 안전)와 **논리 키용 `agent.send_keys`** 로 갈랐다.
- **dogfood 레인이 외부 요인으로 막혀 있다.** 오너 표준이 **현행 0.7.5**이고 다운그레이드·brew 0.7.4 핀·병존 세션은 **타깃 아키텍처가 아니다**(COMPAT §5-1). 어댑터가 유일한 해제 경로다.
- **kind allowlist에 `grok`이 필요하다.** 0.7.5 CLI `agent start --kind`가 `grok`을 포함하며(COMPAT §2.2), grok은 현 dogfood **기본 구현 레인**이다.

**PATCH 근거(버전 판정):**
- **PATCH인 이유:** COMPAT §5-4가 이 작업을 **"product compatibility PATCH"** 로 못박았다. Loom 자신의 계약 — card contract v1 · `done`/`failed` enum · relay/conv wire · MCP 도구 이름·입력 schema · board 상태 전이 — 은 **전부 무변경**이고, 바뀌는 것은 **Loom이 herdr에게 거는 하위 RPC 호출 형상**과 그 호출을 감싸는 브릿지-로컬 배선뿐이다. 제품 표면·성공 정의·페이즈 범위는 그대로다.
- **MINOR가 아닌 이유:** 새 제품 표면이 없다. 신규 CLI·신규 MCP 도구·신규 wire 필드 0건이며, `HerdrClient`는 **외부 서버 어댑터**이지 Loom의 공개 계약이 아니다. 관측 가능한 제품 동작 변경은 "0.7.5에서 워커 스폰이 다시 성공한다"이며 이는 **호환 복구**다.
- **그럼에도 R{n}이 필수인 이유(`WORKFLOW.md` §5.1 — 3행 동시 해당):** ① **"프로토콜 와이어 의미 변경(필수 필드 추가, 호환 깨짐)"** — `agent.start`의 필수 필드가 `{name, argv}` → `{name, kind, pane_id}`로 교체되고 `agent.send`가 사라진 **호환 깨짐**이다. ② **"호환 제거 / 컷오버"** — `HERDR_PROTOCOL_EXPECTED` 16 → 17은 **되돌릴 수 없는 단방향 컷오버**이며(다운그레이드·병존 기각, COMPAT §5-1) 구 herdr에 대한 지원을 그 시점에 끊는다. ③ **"보안·신뢰 경계 변경"** — M-2(비신뢰 프롬프트 vs 제출)의 **분할-호출 축이 소멸**하고 신뢰 원시가 `agent.prompt` 단일 호출로 재정의된다(아래 M-2 절). **→ 신규 R46 필수 · author-close 불가.**

**What (범위 — PATCH; herdr RPC 호출 형상 이식 + env·cwd 이관 + 주입 원시 교체 + 픽스처/테스트 재기술 · **Loom 자신의 표면(card contract v1 · relay/conv wire · MCP 이름·입력 schema · board 전이)은 전부 무변경** · **v0.28.0 락 U1~U11 불변**):**

**① protocol-17 인터페이스 정본 (COMPAT §2.1·§3 실측 — 이 표를 벗어난 파라미터를 보내지 않는다):**

| 단계 | 호출 | 계약 |
|---|---|---|
| **1. pane 선생성** | `tab.create` / `pane.split` | **`env` · `cwd`는 여기서만 전달된다.** `LOOM_CARD`·`LOOM_HOOK_SOCK` 등 워커 env는 전부 이 단계로 이관. `agent.start`에 `split`을 넘기지 않는다(COMPAT §3.1 말미) |
| **2. named start** | `agent.start` | **필수 `name` · `kind` · `pane_id`** / **선택 `args[]` · `timeout_ms`(3000–300000])**. **제거됨: `argv` · `cwd` · `env` · `focus` · `tab_id` · `workspace_id` · `split`.** 네이티브 인자는 `--` 뒤 `args[]`. `kind`는 상호작용형만 허용되고 이름은 strict 검증 — **kind map에 `grok` 포함 의무**(COMPAT §2.2) |
| **3. 제출 (primary)** | `agent.prompt{target, text, wait?}` | **주 경로.** 서버가 paste+Enter를 원자적으로 소유하며 bracketed-paste를 존중한다. 프롬프트 본문은 **오직 opaque text 파라미터** — 셸 보간·명령 조립에 절대 넣지 않는다 |
| **3'. 키 조합** | `agent.send_keys{target, keys[]}` | **명시적 키 조합 전용**(논리 키). 프롬프트 제출의 기본 경로가 **아니다** — 비신뢰 텍스트를 keys로 변환하지 않는다 |
| **4. 대기(가용)** | `agent.wait{target, until?, timeout_ms?}` · `pane.wait_for_output` | 서버 소유 대기가 **가용**하다. 임시 스크레이프 루프보다 적합한 자리에서 선호하되 **완료 판정 권위를 옮기지 않는다**(U1~U3 불변) |
| **오류 코드** | `protocol_mismatch` · `agent_prompt_stalled` · `agent_not_running` | 기계 판독 `protocol_mismatch`를 불투명 RPC 에러보다 우선 처리 · 대기된 프롬프트가 ~5s간 상태 변화 없으면 `agent_prompt_stalled`(주입 재시도 정책이 **이 코드를 명시**해야 한다) · pane 소멸 시 `agent.wait`가 `agent_not_running`을 즉시 반환(fail-fast 정리와 정렬) |

**② 영향 파일 (COMPAT §4 표):**

| 영역 | 경로 | 변경 |
|---|---|---|
| 프로토콜 상수 | `packages/host/src/herdr-client.ts` | `HERDR_PROTOCOL_EXPECTED` **16 → 17** — **어댑터와 동반해서만**, 단독 bump 금지 |
| 클라이언트 API | 동일 | `agentStart`(기존 pane + kind) · `tabCreate`/`paneSplit`(env·cwd) · `agentPrompt` / `agentSendKeys` / `agentWait` 신설 · `injectPromptAndSubmit` 재작성 · **`submitDelayMs` 옵션 제거**(R46 L-1 — protocol-16의 dual-send 사이 유예값이었고 원자적 `agent.prompt`에는 유예를 넣을 두 호출 사이가 없다. 존치하면 죽은 노브가 정책을 서술하는 것처럼 읽힌다. **코드 편집은 이 PATCH 웨이브의 PATCH 2에서 수행 — 지금은 편집하지 않는다**) |
| 스폰 / 풀 | `packages/host/src/bridge-runtime.ts` | env 실은 shell pane 생성 → `start(kind)` → equalize 순서. **레거시 무힌트 폴백도 같은 형태로 재작성** |
| 주입 호출부 | `bridge-runtime.ts` (dispatch · conv 재주입 경로) | 신규 주입 호출 · **`verifyInjectOrRetry`**(0.23.5에서 `verifySubmitOrRetry`/`verifyConvSubmitOrRetry`가 통합된 실제 현행 헬퍼명 — R46 L-2 좌표 정정) ↔ `agent_prompt_stalled` 재검토 |
| fake + 테스트 | `fake-herdr.ts` · `bridge.test.ts` · `herdr-lifecycle.test.ts` · `conv.test.ts` · `inject-live.test.ts` … | `agent.send` 전제 제거 · **신규 호출 순서 락** · **`BARE_ENTER` 처분을 이 웨이브에서 확정**(R46 L-4): 논리 Enter 키가 여전히 필요하면 **`agent.send_keys`용 논리 키 표현으로 교체·export**하고, 필요 없으면 **제거**한다 — production과 테스트가 **하나를 일관되게** 택해야 하며, `agent.send` 소멸 후에도 상수만 남아 **미검토 dual-send 의미론**이 잔존하는 상태는 금지다. **스키마 근거가 키 토큰 이름을 명시하지 않으므로 herdr 축자 키 토큰을 여기서 발명하지 않는다** — PATCH 1 픽스처 포집에서 실측한 표현을 쓴다 |
| 픽스처(기존) | `docs/spikes/fixtures/herdr-v0.7.4/` | **불변 역사 자료 — 수정 금지** |
| 픽스처(신규) | `docs/spikes/fixtures/herdr-v0.7.5/` | 구현 시점에 라이브 schema/methods/RPC 샘플 포집 |
| dogfood 게이트 | `scripts/dogfood-herdr-check.sh` | 이미 0.7.5/17 요구 — Loom이 17을 기대하는 순간 green |
| 운영 문서 | `HANDOFF_WINDOWS.md` · 노드 부트 팩 | 버전 핀 문안 · **업그레이드 후 플러그인 재설치**(0.7.5부터 플러그인 install/link/enable이 **user-global**, 세션 격리 아님 — COMPAT §2.1) |

**③ 구현 순서 (COMPAT §6 축자 — tests-first · red 선행 규율은 v0.28.0에서 승계):**

| # | 단계 | 내용 |
|---|------|------|
| **PATCH 1** | **픽스처 락 + expected-red (fixture/tests only · production 0줄)** | COMPAT §6-1. 라이브 schema·methods·RPC 샘플로 `docs/spikes/fixtures/herdr-v0.7.5/`를 쓰고, protocol-16 클라이언트에 대한 **expected-red** 계약을 적는다. **`herdr-v0.7.4/`는 불변**(COMPAT §5-3) — 갱신이 아니라 **병존**이다. 이 커밋의 red는 결함이 아니라 **다음 커밋들의 수용 계약**이며 커밋 메시지에 red/green 구분을 명기한다 |
| **PATCH 2** | **`HerdrClient` 표면** | COMPAT §6-2. 신규 start/prompt/keys/wait · tab/split의 env·cwd · `HERDR_PROTOCOL_EXPECTED = 17`. `protocol_mismatch` 구조적 처리 |
| **PATCH 3** | **`bridge-runtime` 스폰 재배선** | COMPAT §6-3. 풀 경로 + 레거시 폴백 경로 동시. 주입 호출부 교체 및 `agent_prompt_stalled` 대응 재시도 정책 — **재시도 정책의 수용 계약은 아래 Security/trust "중복 제출 방지 4절"(R46 M-1 lock)이다**(4절 전부를 만족하지 않으면 이 PATCH는 미완이다) |
| **PATCH 4** | **fake-herdr + 유닛 스위트 green** | COMPAT §6-4. `fake-herdr.ts`를 protocol-17 형상으로 올리고 PATCH 1의 red를 green으로 닫는다. **`fake-herdr.ts:565` status가 underscore 전용이고 실서버는 dotted라는 기존 픽스처 갭을 여기서 재확인**(제품은 양쪽 수용 — 활성 함정) |
| **PATCH 5** | **라이브 스모크 + dogfood 게이트 + 문서** | COMPAT §6-5·§6-6. `claude`·`codex`·`grok` **3종 각각** 라이브 1회: env 도달·프롬프트 제출·status 이벤트·스크레이프·close. 이어 `dogfood:herdr`/`dogfood:up` green → HANDOFF·Windows·노드 노트 갱신 · `implementation-notes.md`에 **M-2/주입 Deviation 기록** · COMPAT 문서 verdict를 **shipped + 커밋 SHA**로 전환 |

**픽스처 규약 (COMPAT §5-3 락):** `docs/spikes/fixtures/herdr-v0.7.4/`는 **불변 역사 자료**다 — 덮어쓰지 않고 **별도 `herdr-v0.7.5/` 트리를 신설**한다. 두 픽스처의 **차집합이 계약 파손의 유일한 기계적 증거**다(schema version 필드가 양쪽 `1`이라 버전으로는 감지 불가 — COMPAT §3 표 1행).

**이벤트 additive 필드 정책 (하드 경계):** 0.7.5의 `pane_agent_detected`는 **`final_status` · `released`** 를 실을 수 있다(COMPAT §3 표). 파서는 이 필드들을 **관용한다**(미지 필드로 인한 파싱 실패 금지). 그러나 **단독으로도, 조합으로도 완료 권위가 아니다**(COMPAT §4 말미 축자 · §2.4 "PANE-DEATH 완료 권위 의미론은 재정의하지 않는다"). 구체적으로: 이 필드를 근거로 board `done`을 구성하거나(U1) 원격 result를 `done`으로 적용하거나(U2) card pane을 자동으로 닫는(U3) 코드는 **금지**다. **회귀 게이트 G-1·G-2·G-3은 이 버전에서도 그대로 green이어야 한다.**

**M-2 결정 (비신뢰 프롬프트 vs 제출 — COMPAT §3.2 · Deviation 기록 의무):**

> **채택: `agent.prompt`를 신뢰 제출 원시(trusted opaque submit primitive)로 삼는다** — COMPAT §3.2의 **preferred default**.

protocol 16의 M-2는 *"비신뢰 텍스트가 제출 명령에 보간되지 않도록 두 번의 독립 `agent.send`로 분리하고 `BARE_ENTER = "\r"`를 고정한다"* 였다. protocol 17의 원자적 `agent.prompt`는 paste+Enter를 **서버 측에서 결속**하므로 **분할-호출 축 자체가 소멸**한다 — 지킬 대상이 사라졌지 우회한 것이 아니다. 결정 내용:

- **herdr가 paste·Enter를 소유한다**는 것을 명문화한다. Loom은 제출 시점을 조립하지 않는다.
- **프롬프트 본문은 끝까지 opaque text 파라미터**다 — 셸을 경유하지 않고, 명령 문자열에 보간되지 않으며, keys 시퀀스로 변환되지 않는다. **M-2의 보호 대상(비신뢰 텍스트의 명령-문맥 탈출)은 그대로 유지**되고 그 집행 지점만 클라이언트 분할에서 **서버 원자성 + 파라미터 경계**로 옮겨간다.
- **`agent.send_keys`는 명시적 키 조합에만 쓴다.** 비신뢰 텍스트를 키로 바꾸는 경로를 만들지 않는다.
- **dual-`agent.send` 유지는 불가능하다** — 메서드가 없다(COMPAT §3.2 말미 축자 "Do not silently keep dual-`agent.send`"). 침묵 존치 금지.
- **strict M-2 보존안은 채택하지 않는다.** COMPAT §3.2가 그 조건으로 건 *"composer 레이스를 재도입하지 않으면서 안정적으로 제출되는 비원자 경로를 라이브 재측정으로 찾은 경우"* 는 **측정되지 않았다** — 근거 없이 선택할 수 없다(UK-6).
- **기록 의무:** 이 결정은 `implementation-notes.md` **Deviations**에 기록한다(COMPAT §6 done-when 5행). PLAN 기재만으로는 done-when을 만족하지 않는다.

**Non-goals (이 버전 아님 — COMPAT §2.2·§2.4):**
- `agent.view.set` / `agent.view.clear` — **명시적 non-goal**.
- 플러그인 `[[startup]]` 훅 — Loom이 herdr 플러그인이 되지 않는 한 non-goal.
- UI 노브(`ui.sidebar_start_collapsed`, 새 workspace 이름 프롬프트, 사이드바 스타일) — non-goal.
- macOS `HERDR_AGENT=<agent>` 포그라운드 프로세스 힌트 — **선택적 운영 노트**일 뿐 어댑터 범위 아님(래핑된 에이전트용).
- 세션 내비게이터 UX · 사운드 알림 · 마우스/선택 · 그래픽스 teardown · Windows CPU · Ceph cwd 캐시 · 원격 mise 경로 탐색.
- **herdr 다운그레이드 · 0.7.4 브루 핀 · 0.7.4 병존 세션** — 오너 기각(COMPAT §5-1 · 활성 함정).
- **PANE-DEATH 완료 권위 의미론 재정의**(U1~U11 / v0.28.0) — 새 이벤트 필드를 **관용만** 하고 done 권위를 여기서 다시 정의하지 않는다.
- **Windows/원격 노드 상태 전진** — 로컬 어댑터 증명 전에는 손대지 않는다(플러그인 재설치 운영 노트는 예외).

**Security / trust:**
- **신뢰 경계가 이동하되 넓어지지 않는다.** M-2의 보호 대상은 유지되고 집행 지점만 herdr 서버 원자성으로 옮긴다. Loom은 여전히 **셸을 경유하지 않고**, 프롬프트 본문을 **명령 문자열에 보간하지 않는다.**
- **`agent.prompt`를 신뢰한다는 것은 herdr 프로세스를 신뢰한다는 뜻이다** — 이는 **기존 신뢰 관계의 재확인**이지 확대가 아니다(Loom은 이미 herdr 소켓에 워커 스폰 권한을 위임하고 있다). 새 원격 표면·새 인증 주체·새 네트워크 경로는 **0건**이다.
- **env가 pane 생성 시점으로 앞당겨진다.** `LOOM_CARD`·`LOOM_HOOK_SOCK`은 이제 `tab.create`/`pane.split`에 실린다 — 담기는 값의 집합은 무변경이고 **전달 시점만** 이동한다. 비밀 값의 범위가 늘지 않는지는 이식 시 대조 대상이다.
- **fail-closed가 기본값이다.** `HERDR_PROTOCOL_EXPECTED` 단독 bump 금지(COMPAT §5-2)와 `dogfood-herdr-check.sh`의 **호스트 변경 이전** 차단(COMPAT §5-5)은 이 버전에서도 유지된다 — 어댑터 미완 상태에서 워커가 절반만 스폰되는 창을 만들지 않는다.
- **새 이벤트 필드는 신뢰 입력이 아니다.** `final_status`/`released`는 **워커·서버가 만드는 관측물**이고 이 트랙의 불변식은 *"완료는 사람이 확정한다"* 이다. 관용은 파싱 층에서 끝나고 **판정 층으로 올라오지 않는다.**
- **`agent_prompt_stalled` 재시도는 멱등 경계 안에서만.** 재시도 정책이 프롬프트를 중복 제출해 워커가 같은 작업을 두 번 하지 않도록 한다 — 비가역 행동은 결정적 증거 또는 멱등 경로에서만(트랙 불변식 2문).
- **중복 제출 방지 4절 (R46 M-1 lock — 어댑터 재시도 정책의 수용 계약).** 원자적 `agent.prompt`는 paste+Enter를 서버가 소유하므로 **재발행 1회 = 워커 작업 1회 추가 실행**이다. protocol-16의 "빈 composer에 CR만 재전송"이 무해 no-op이었던 성질은 **소멸**한다. 따라서:
  1. **전문 제출은 verify/주입 시도당 최대 1회.** 한 번의 `agent.prompt` 전문 제출 이후, 같은 주입 시도 안에서 **stall(`agent_prompt_stalled`)·타임아웃을 관측하면 재발행이 아니라 먼저 프로브**(현행 verify 루프의 스크레이프 판정)를 돈다.
  2. **재발행은 양성 프로브 미스에서만.** 스크레이프가 **composer 부재를 실제로 보여준 경우**(프로브 문자열도, TUI paste-플레이스홀더도 없음)에 한해 동일 캐시 문자열로 `agent.prompt`를 1회 재발행한다 — 재파생 금지(R30 L-1 승계).
  3. **프로브 히트(Ink 플레이스홀더 포함)와 read-fail은 재발행을 금지한다.** 둘 다 "제출됐을 수 있음"과 양립하므로 전문 재발행 대신 **경계 있는 `agent.send_keys` Enter 넛지**만 허용한다(protocol-16의 CR 재전송 자리를 그대로 승계 — 비신뢰 텍스트를 키로 바꾸지 않는다).
  4. **예산 소진은 fail-visible.** 재발행·넛지 예산을 소진하면 **조용한 재시도도, 무한 대기도 없이** 현행 `inject_unconfirmed` 계열로 관측 가능하게 실패한다.
  이 4절은 **현행 `reinjectUsed`의 verify-호출(주입 시도) 스코프를 그대로 보존**하며(R30 M-2 — flight 단위가 아니다), **새 타이밍 상수를 도입하지 않는다**(lessons (30) — 미실측 상수로 정확성 갭을 닫지 않는다). 근거는 UK-9: `agent_prompt_stalled`는 **상태 변화 미관측**이지 **미제출의 증명이 아니다**.

**추정 / 실측 구분 (lessons (30) — 미실측을 "없음"으로 적지 않는다):**

| 명제 | 지위 |
|---|---|
| 라이브 herdr = 0.7.5 · protocol 17 · `live_handoff` capability true | **실측 확정**(COMPAT §7, 2026-07-22 `herdr --version` · `status server --json`) |
| `AgentStartParams` 필수 = `name,kind,pane_id`(0.7.4 픽스처는 `name,argv`) | **실측 확정**(COMPAT §7 — 라이브 schema vs 픽스처 대조) |
| 라이브 메서드에 `agent.prompt`/`agent.send_keys`/`agent.wait` 존재 · **`agent.send` 엔트리 없음** | **실측 확정**(COMPAT §7) |
| `tab.create`/`pane.split`이 `env`+`cwd`를 계속 수용 | **실측 확정**(COMPAT §7 — CLI + schema 양쪽) |
| 라이브 메서드 수 **89** · 신규 `agent.*` 5종 | **실측 확정**(COMPAT §3 표 말행) |
| `agent.start --kind`에 `grok` 포함 | **실측 확정**(COMPAT §2.2 — 릴리스 노트/CLI) |
| detect 이벤트가 `final_status`/`released`를 **실을 수 있다** | **릴리스 노트 + schema 대조**(COMPAT §3) — **어느 조건에서 실리는지는 미측정**(UK-5) |
| 명시적 거부·거절 봉투의 실제 wire 형상 | **미실측** — v0.28.0 UK-1 승계(이 버전이 좁히지 않는다) |
| 비원자 제출 경로가 composer 레이스 없이 가능한지 | **미측정**(UK-6) — 그래서 strict M-2 보존안을 선택하지 않았다 |
| `agent_prompt_stalled`가 **프롬프트 미제출**을 뜻하는지 | **미실측**(UK-9) — COMPAT §3 축자는 *"관측된 상태 변화 없음"* 뿐이다. 제출 후 워커가 조용히 사고 중인 경우와 구분되지 않는다 |
| 0.7.5 하에서 Loom 브릿지의 실제 스폰 성공 | **미검증** — PATCH 5 라이브 스모크가 유일한 증거 경로 |
| `fake-herdr.ts:565` status underscore vs 실서버 dotted 갭 | **기존 실측**(활성 함정) — PATCH 4에서 재확인 대상 |

**Unknowns (§3.5 — v0.28.0의 UK-1~UK-4와 번호가 겹치지 않게 이어서 매긴다):**
- **UK-5** — detect 이벤트의 `final_status`/`released`가 **어느 조건에서 실리는지**. correctness에 걸리지 않는다 — 이 버전은 두 필드를 **관용만** 하고 판정에 쓰지 않으므로, 실리든 안 실리든 만들어지는 코드가 달라지지 않는다. **후속 관측 축(후속 C)의 입력 후보**로만 둔다.
- **UK-6** — 비원자 제출 경로(strict M-2 보존)가 composer 레이스 없이 성립하는지. **미측정**이며 그래서 preferred default를 택했다. 재측정으로 성립이 증명되면 M-2 결정은 **재론 가능**하다 — 그때까지 `agent.prompt` 단일 원시가 정본이다.
- **UK-7** — `agent.wait`/`pane.wait_for_output`가 현행 still-running 유예·스크레이프 루프를 **어디까지 대체할 수 있는지**. COMPAT §2.1은 *"적절한 자리에서 선호"* 까지만 말한다. **이 버전은 대체하지 않고 가용성만 확보**한다 — 완료 판정 경로를 손대는 것은 U1~U3 인접이므로 별도 게이트가 필요하다.
- **UK-8** — 다중 노드(WSL/VPS/Windows)에서 **플러그인 user-global 전환**의 실제 영향. 0.7.3 시대 세션-로컬 상태에서 올라온 노드는 사용자당 1회 재설치/재링크가 필요하다(COMPAT §2.1). **로컬 어댑터는 증명 완료**(`6e2df8a` · live 3-kind · dogfood unblocked); **per-user reinstall/relink는 ops 요건·후속 관측**으로 남는다(노드마다 사용자 1회 — 세션 단위 아님).
- **UK-9** — **`agent_prompt_stalled`가 "미제출"을 뜻하는가.** COMPAT §3 축자는 *"대기된 프롬프트에 ~5s간 **관측된 상태 변화 없음**"* 이며, 이는 **미제출의 증명이 아니다** — 제출됐지만 워커 TUI가 아직 상태를 바꾸지 않은 경우와 같은 코드로 수렴한다. **미실측**이며 라이브 프로브 없이는 좁혀지지 않는다. 그러나 이 버전의 재시도 정책은 **어느 답이 나와도 안전하도록** 설계된다(Security/trust 중복 제출 방지 4절): 미제출이면 프로브가 composer 부재로 미스를 내 재발행이 열리고, 실제로는 제출됐다면 프로브 히트·read-fail이 재발행을 금지하고 넛지만 남긴다. **따라서 UK-9는 착수를 막지 않으며**, 실측되면 프로브 라운드를 줄이는 최적화의 입력이 될 뿐 정책의 안전성을 바꾸지 않는다.

**Done when (COMPAT §6 done-when 축자 + 이 PLAN의 게이트 승계):**
- [x] `HERDR_PROTOCOL_EXPECTED === 17` **그리고** 라이브 서버 0.7.5/17
- [x] Host herdr/bridge 표면 green — **462 pass / 0 fail / 34 files / 302.02s**. 버전 bump 직후 첫 전체 트리 실행(stale checkpoint-hardcode drift 진단) **761 pass / 2 fail / 2746 expect / 58 files / 302.64s** — 두 실패 모두 오래된 `scripts/handoff-checkpoint.test.ts` 하드코드(release-checkpoint drift)로 분류. 교정 후 checkpoint **24 pass / 0 fail / 120 expect**. 권위 final full **763 pass / 0 fail / 2760 expect / 58 files / 302.48s**.
- [x] **라이브 단일 카드 스모크 3종 — `claude` · `codex` · `grok` 각각**: interactive readiness · env 도달 · named prompt 제출 · working→idle 이벤트 · scrape marker · close
- [x] `bun run dogfood:herdr`가 compatibility ok 출력 · `dogfood:up` exit 0 (persisted config migration 후)
- [x] **M-2 / 주입 정책이 `implementation-notes.md`의 신규 섹션 `§0.28.1`(Deviations)에 기록**됨 — R46 L-3: 기존 protocol-16 M-2/bare-Enter deviation 항목을 덮어쓰지 않고 **0.28.1 전용 섹션을 신설**해 append한다(원자적 `agent.prompt` 채택 · dual-`agent.send` 소멸 · 중복 제출 방지 4절 · `BARE_ENTER` 처분 · `submitDelayMs` 제거 · launch readiness · named target · Fable name guard · config migration을 각각 기재)
- [x] `docs/spikes/HERDR-0.7.5-COMPAT.md` verdict가 **shipped through `6e2df8a`** 로 전환 · dogfood unblocked
- [x] **v0.28.0 회귀 게이트 G-1 · G-2 · G-3이 여전히 green**(이벤트 additive 필드가 완료 권위로 새지 않았다는 증거) — 제품 공개 card/relay/conv/MCP 입력 표면과 PANE-DEATH U1–U11 무변경

**Implemented as of `6e2df8a` (2026-07-22, herdr 0.7.5 / protocol-17 adapter · R46 author-close approved · Advisor: fable-advisor consulted: yes · release close complete/shipped for Loom 0.28.1):** tests-first 시퀀스 `194d901`(fixture/expected-red) → `c0fcc00`(client) → `1284eef`(bridge) → `e538cad`(coverage)로 protocol-17 어댑터를 이식했다. 라이브 보정 4종: launch readiness `848675f`/`5ac6d31`(`agent.start` ACK가 `launch_pending`일 수 있어 `agent.get`→`interactive_ready` 폴링; `agent.wait` idle은 launch barrier가 아님) · named prompt target `edf3b59`/`48ecba3`(제출 타깃 = 저장 exact agent name, pane id 아님 — 라이브 Claude pane-id 프롬프트 침묵 오배송 실측) · persisted dogfood protocol migration `9f13b47`/`8ebfd11`(`herdrProtocol:16` 설정이 ready early-exit 전 auto-migrate; config-only protocol bump는 여전히 어댑터 우회로 금지) · Fable 5 자문 후 collision-free strict target identity `1351add`/`6e2df8a`(exact `loom-${cardId}-${seq}`, strict lowercase/safe seq/length≤32, fail-closed `agent_name_unrepresentable`, hash/truncation 금지 — 구 pane·재시작 안전). 라이브 3종(claude/codex/grok) 스모크 통과(interactive readiness · env · named prompt · working→idle · scrape · close). `bun run dogfood:herdr` compatibility ok · `dogfood:up` exit 0. host full **462/0** · 진단 first full **761/2**(stale checkpoint-hardcode drift) → checkpoint **24/0/120 expect** → 권위 final full **763 pass / 0 fail / 2760 expect / 58 files / 302.48s**. CLI Loom **v0.28.1** · dist guard green. 제품 공개 card/relay/conv/MCP 입력 표면과 PANE-DEATH U1–U11 무변경. **Next:** Phase D automation.

**Review impact:** herdr RPC 호출 형상 전면 교체(`agent.start` 필수 필드 교체 · `agent.send` 소멸 · `agent.prompt`/`agent.send_keys`/`agent.wait` 신설) · env·cwd의 `tab.create`/`pane.split` 이관 · `HERDR_PROTOCOL_EXPECTED` 16→17 **단방향 컷오버** · **M-2 신뢰 원시 재정의**(분할 호출 → 원자적 `agent.prompt`) · 0.7.5 픽스처 신설(0.7.4 불변 병존) · fake-herdr 및 브릿지 테스트 재기술. **Loom 자신의 표면(card contract v1 · relay/conv wire · MCP 이름·입력 schema · board 전이)과 v0.28.0 락 U1~U11 · 게이트 G-1~G-11은 전부 무변경.** 정본 [`docs/spikes/HERDR-0.7.5-COMPAT.md`](spikes/HERDR-0.7.5-COMPAT.md) §1~§6. **R46 필수 — author-close 불가**(`WORKFLOW.md` §5.1 — 프로토콜 와이어 의미 변경 + 호환 컷오버 + 보안·신뢰 경계, 3행 동시 해당). **R46이 최소 확인해야 할 것:** ① §1의 protocol-17 인터페이스 표가 COMPAT §2.1·§3 실측과 **축자 일치**하는가(발명된 파라미터·누락된 필수 필드가 없는가) ② M-2 결정(원자적 `agent.prompt` = 신뢰 opaque 제출 원시)이 **보호 대상을 실제로 유지**하는가, 아니면 집행 지점 이동을 근거로 보호를 약화시켰는가 — 특히 UK-6(비원자 경로 미측정) 위에 선 선택임을 정직하게 기재했는가 ③ 이벤트 additive 필드 관용이 **완료 권위로 새지 않도록** G-1·G-2·G-3 유지를 done-when에 건 것으로 충분한가 ④ 픽스처 규약(0.7.4 불변 · 0.7.5 병존)이 schema version 필드 무변경이라는 **감지 불가 조건**에 대한 유일한 기계적 방어선으로 성립하는가 ⑤ 구현 순서가 **tests-first 규율**을 실제로 만족하는가 — PATCH 1이 fixture/tests only(production 0줄)이고 PATCH 4의 green 종결이 test-after로 오독되지 않는가(v0.28.0 미검증 이월 ⑪의 재범 방지) ⑥ 이 버전이 PATCH로 판정된 근거(Loom 공개 표면 무변경 · 하위 RPC 어댑터 한정)가 **컷오버의 비가역성**과 양립하는가.

#### 0.28.0 — 2026-07-21 (`approved` **R45 author-close — High 0 · M1 기각(검증자 트리 오독) · M2 처방 정밀화(U8 범위 무영향 실측) · Low 3 반영** — **완료 권한 컷 — 브릿지 자동 `done` 폐지 · tower ingress `blocked` 격리 · card 자동 `pane.close` 0곳 · 비수락 ACK 단일 흡수** (MINOR))

> **R44 pending-revision → 통합 설계 확정 → 본 PLAN (2026-07-21).** PANE-DEATH 두 노선(main pre-C 전달층 · 브랜치 authority cut)을 단일 정본으로 통합한 설계 [`docs/spikes/PANE-DEATH-UNIFIED-DESIGN.md`](spikes/PANE-DEATH-UNIFIED-DESIGN.md)(작성 = Fable 5)를 **R44**가 검증했다 — verdict **pending-revision**, **High 0 · Medium 4 · Low 5**, 자기모순 전수 검사 **0건**(이 트랙 3연속 reject의 근인이 재발하지 않았다). 레인은 codex 사용량 하드 한도(리셋 2026-07-25 14:27)로 **grok 재배정** — 설계 작성자가 Fable 5이므로 벤더 다양성·발견자≠수정자가 둘 다 성립한다(규칙 5의 문자에서 벗어나되 목적은 유지). 원장 [`docs/reviews/PANEDEATH-R44.md`](reviews/PANEDEATH-R44.md), 정본·수정 금지. Medium 4 + Low 2는 같은 세션에 설계에 반영 완료(좌표 드리프트 `:850 :1114 :1204`→**`:893 :1160 :1250`** · accepted 관측점 (a)/(b) 분리 · 경쟁 패자 패턴 좌표 · "3분기/2+1" 명명 규약 · 기준 커밋 `89dd931`→`694e08c`). **본 PLAN은 그 설계를 PATCH 단위로 분해하는 문서이지 설계를 재서술하는 문서가 아니다** — 락 문안은 복제하지 않고 `설계 §6 U{n}`으로 참조한다(중복 서술 = 요약-본문 드리프트 = 이 트랙 reject의 근인).
>
> **잔여 Low 3건 종결 (2026-07-21).** R44 §5의 미반영 Low 3건을 본 PLAN 작성과 동반 처리했다: **L2**(§0.2 D7 교차참조 오지정) → 설계 D7 행의 교차참조를 **`§0.3-3 · §1.3`**으로 정정 · **L4**(D10 map-miss 무로그가 G-표에 미등재) → **회귀 게이트 G-10 신설**(flight map miss inert 경로 구조 로그) + §5.2 M3에 한 줄 등재 · **L5**(§0.3 헤더 "5건" vs 본문 6) → **no-op 종결(기각)**. **L5 기각 근거 — 검증자의 지적도 검증 대상이다:** R44가 지적한 *"§0.3 헤더 '5건'"* 이라는 문자열은 **검증 대상 커밋 `694e08c`에 존재한 적이 없다**(아키텍트 직접 실측). 헤더에 개수를 신설하는 것은 오히려 **요약-본문 드리프트를 자초**하므로 반영하지 않는다. 이 트랙에서 요약과 본문이 어긋나 High를 미종결시킨 것이 3차 reject의 근인이었다 — 개수를 요약에 박는 관행 자체가 그 근인이다.
>
> **UNVERIFIED ⑥ 종결 — 관측 불가 확정 (2026-07-21).** R44가 *"M2 반영의 전제 · 구현 착수 시 가장 먼저 확인할 것"* 으로 못박은 항목(ACK 주입 경로에서 `resultPhase`/`relayAcceptedAt`의 테스트 관측 가능성)을 아키텍트가 확인해 **관측 불가로 확정**했다: `type Flight`(`bridge-runtime.ts:375`)이 **미export**이고 `flights` 맵은 `startBridgeRuntime`(`:566`) **클로저 로컬**이라 테스트가 읽을 수 없으며, 외부 참조는 `impl-0270.test.ts:811`의 **소스 문자열 락 1건뿐**(런타임 관측 아님)이다. 결정적으로 **락 U3가 `pane.close`(`:2618` — `:2610`의 `const sent = outcome.branch === "accepted"`가 게이트하는 유일 대상)를 제거하면 주입 경로의 accepted 양성 관측물이 0이 된다.** 처방(fable-advisor 자문 채택): **① §4.2 accepted (a) 행을 "오분류 부재 + 보존 계약 확인(음성 대조군)"으로 재설계**하고, **② 판정 로직의 양성 락을 신설 `classifyAck` 순수 유닛 행으로 이관**한다(→ 회귀 게이트 **G-11** 신설 · `classifyAck` 모듈 스코프 승격+export는 production 변경이므로 **M3 소속**). **기각 — 안 1(accepted 관측 카운터 제품 코드 신설): 재론 금지.** 설계 §5.1이 `recordResultDeliveryUnconfirmed`(`:540` — production 호출자 0)를 **"휘발 카운터"로 폐기**했으므로, 같은 종의 카운터를 성공 경로에 새로 세우는 것은 설계 자기모순이다.

**Product one-liner:** 브릿지는 지금도 **스스로 완료를 확정한다** — `finishCard(flight, "done", …)` 구성 **5곳**(`bridge-runtime.ts:2171 :2197 :2256 :2288 :2315`, 실측)이 board `done`을 만들고, 성공 발행 뒤 `:2618`이 card pane을 닫으며, tower는 원격 result의 `done`을 **그대로 `done`으로 적용**한다(`card-ops.ts:317-318` 실측 — `payload.status === "done" ? "done" : "blocked"`). 이는 트랙 불변식 1문("완료는 사람이 확정한다")과 **정면 충돌**하고, 이 트랙이 최초에 실증한 결함 213/215/217(가짜 done)의 구조적 원천이 그대로 남아 있다는 뜻이다. v0.28.0은 **권한 컷(authority cut)** 을 실행한다: 브릿지는 `done`을 **구성할 수 없고**(U1), tower ingress는 원격 result로 `done`을 **만들 수 없으며**(U2), card pane을 닫는 자동 경로는 **0곳**(U3)이 된다. 동시에 v0.27.0이 세운 전달층(issuer · strict ACK · durable quarantine · currency gate)은 **전량 유지**되며 — 자동 `done`이 사라진 세계에서 알림 유실은 곧 카드 영구 정지이므로 전달층은 **더 load-bearing해진다** — 코드-설계 불일치 3건(비수락 rejected 분기의 조용한 정지 · quarantine `process_exit` fold가 미해소를 지움 · 운영자 ack 표면 미배선)을 함께 닫는다. 설계 정본은 [`docs/spikes/PANE-DEATH-UNIFIED-DESIGN.md`](spikes/PANE-DEATH-UNIFIED-DESIGN.md)이며 **§1**(통합 상태기계)·**§2**(전달층 사양)·**§3**(cleanup 권한)·**§4**(관측점)·**§5**(마이그레이션 순서·회귀 게이트)·**§6**(락 U1~U11)·**§7**(후속 A/B/C)·**§8**(미확인·정직성)이 각 PATCH의 유도 원점이다.

**트랙 불변식(전 PATCH의 유도 원점 — 설계 §1 도입부 축자):**

> **완료는 사람이 확정한다. 브릿지는 전달과 회복만 소유한다.**
> **비가역 행동은 결정적 증거 또는 멱등 경로에서만 허용된다.**

**Why:**
- **권한 경계가 코드에 없다.** 브릿지 자동 종결점은 실측 5곳 + 자동 close 1곳이고, tower는 원격 `done`을 그대로 반영한다. 불변식은 문서에만 있고 **타입·경로 부재로 잠기지 않았다.** 설계 §0.1 축자대로 권한 경계는 config 옵션이 아니라 **API 부재**로 잠가야 한다 — 자동 `done`을 옵션으로 남기면 락이 아니라 스위치가 되고 §6 락 전부가 조건부가 된다(제3안 기각 근거).
- **명시적 거부(`rejected`) 분기가 조용한 정지다 (설계 §0.3-1 · 실측).** `sendResult`의 `rejected` 분기(`:2780`)는 **로그 한 줄 외에 아무것도 남기지 않는다** — quarantine 미진입 · board 전이 없음 · durable 기록 0. 사람-확정 모델에서 이는 "알림 유실 = 카드 영구 정지"의 정확한 사례이고, v0.27.0 자신의 결정 1(*"비수락 결과는 유실·명시 거절을 구분하지 않고 단일 흡수 상태 `send_unknown`으로 보내고 durable alert를 낸다"*)과 **코드가 어긋난다.**
- **quarantine `process_exit` fold가 미해소를 지운다 (설계 §0.3-4 · 실측).** `foldQuarantineLines`가 `process_exit`를 `ack`/`auto_resolve`와 **동일하게 `open.delete`** 한다(`result-quarantine.ts:128-133`). 정상 종료 시 `onProcessExit`(`:361-392`)가 전 미해소에 `process_exit`를 append하므로 **재기동 replay가 미해소 카운트를 복원하지 못한다**(크래시 경로만 복원). 카드는 여전히 `doing`에 잔존하는데 **알림만 사라진다** — §6.7.1의 "기동 replay로 미해소 복원" 취지와 정반대다.
- **미해소 레코드를 닫을 합법 경로가 없다 (설계 §0.3-6 · 실측).** `QuarantineStore.ack`(`:294-315`)는 production 호출 **0곳**(유일 호출 = `impl-0270.test.ts:217`)이다. 위 fold 정정을 하면 미해소가 **영구 잔존**하므로 운영자 ack 표면 배선은 **선택이 아니라 정정의 짝**이다.
- **accepted 판정에 양성 락이 없다 (UNVERIFIED ⑥ 종결의 귀결).** accepted를 게이트하던 유일한 양성 부수효과가 `:2618`의 `pane.close`인데 U3가 그것을 제거한다 — 관측점을 재설계하지 않으면 **"전달됐는가"가 어떤 테스트로도 관측되지 않는다.** 부정 어서션(`closeCallsFor === closesBefore`)은 close 제거 후 **전 suite에서 자동 참**이 되므로 감지력이 0이다(교훈 40의 재현 차단 = 락 U7).

**MINOR 근거(버전 판정):**
- **PATCH가 아닌 이유:** 표면은 전부 동결이나 **`apply_card_result`의 적용 의미가 의도적으로 변경**된다(설계 §6 **U2**) — 원격 result는 어떤 payload status에서도 board `done`을 만들지 못하고 `blocked`로 격리된다(legacy `done` → `blocked`, reason `legacy_remote_done_requires_verification`). 이는 "완료 기준 보강·보안 범위 명확화"(§Version control PATCH 정의)가 아니라 **관측 가능한 동작 변경**이며, MCP 도구 설명·`HERDR_DESIGN.md`·`DISPATCH-DEMO.md` 등 **공개 계약 문서 갱신을 동반**한다. 더해 브릿지 발행 API가 `finishCard(flight, status, …)`에서 **proposal 전용 API로 재구성**되고(완료 상태를 입력받는 API 제거), card 자동 `pane.close`가 0곳이 되어 **운영 절차 자체가 바뀐다**(pane 수동 정리).
- **MAJOR가 아닌 이유:** 제품 방향·성공 정의 불변이고 **표면 전부 동결**이다(설계 §6 **U10**) — card contract v1 · `done|failed` enum · `cardSeq` 소비자 · relay/conv wire · MCP 도구 **이름·입력 schema** · herdr RPC 표면 전부 무변경. `(cardId, dispatchHandoffId, seq)` payload 각인도 v0.27.0 규약 그대로 유지한다.
- **R{n} 필수 판정(`WORKFLOW.md` §5.1):** ① "새 **MINOR**" 행 → 필수. ② 완료 attestation·발행 권위·tower 적용 의미는 R29/R32/R33 M-lock 인접 **신뢰 경계**다 → 필수. ③ 설계 §8도 독립적으로 `rgate=needed`로 판정했다(문서 말미 태그 `rgate=needed`). **→ 신규 R45 필수 · author-close 불가.**

**What (범위 — MINOR; 권한 컷 + 전달층 정정 3건 + 관측점 재설계 · **card contract v1 · `done|failed` enum · relay/conv wire · MCP 이름·입력 schema · herdr RPC 표면 전부 무변경(설계 §6 U10)** — 바뀌는 것은 브릿지 발행 API 형상 · tower-local 적용 의미 · quarantine fold 의미 · 테스트 관측점이다):**

**PATCH 분해는 설계 §5.2의 M1~M5를 그대로 PATCH 경계로 삼는다 — red-test 선행 규율(브랜치 §8 승계)이므로 테스트와 production을 같은 커밋에 섞지 않는다.** 각 단계의 production diff는 **선행 red-test 커밋 해시를 증거로 남긴다.** **구현 레인 = Claude**(grok은 R44 검증자이므로 **발견자≠수정자**를 유지해야 한다; codex는 사용량 한도로 다운 — 리셋 2026-07-25 14:27).

| # | 항목 | 내용 |
|---|------|------|
| **PATCH 1 (M1)** | **tests-only expected-red 커밋 — 계약을 먼저 적는다** | 설계 §5.2 M1 그대로. **production 변경 0줄** — 이 커밋의 red는 결함이 아니라 **다음 커밋들의 수용 계약**이다. 내용: ① **브릿지 `done` 구성 0곳** 게이트(구 §7.1-0의 "(C) 게이트"를 v0.28.0에서 **live로 승격** — 설계 §1.4) · ② **tower `done→blocked` fence**(브랜치 `bridge.test.ts` 이식분) · ③ **card close 3경로 pane 보존 양성**(`fake.listPaneIds()`에 paneId 잔존 — 부정 어서션 단독 금지) · ④ **at-most-one 3건**(완료↔terminal · claimed 경로 · spawn 실패 — relay 수신 `toHaveLength(1)` 정확 개수) · ⑤ **가드 순서 고정**(§2.4 합성 순서 — 이식, green 확인용). **①②③은 현행 red가 정상이고 ④⑤는 green 확인용**이라는 구분을 커밋 메시지에 명기한다(red/green 혼동 시 다음 단계 진단이 엉킨다). **락:** 설계 §6 **U1·U2·U3·U4·U7**. **게이트:** G-1 · G-2 · G-3 · G-9. **대상 파일:** `packages/host/src/bridge.test.ts`(이식) · `pane-cleanup.test.ts` · `impl-0270.test.ts`. |
| **PATCH 2 (M2)** | **tower A2 합성 — `card-ops.ts` 격리 매핑 + 공개 계약 문서** | 설계 §2.4가 정본. **순서 근거(명기 의무): 구 bridge → 새 tower 안전이 먼저다.** tower를 먼저 fence로 올리면 rolling upgrade 표(설계 §2.4 말미)의 **양방향 fail-closed**가 이 시점부터 성립한다 — 새 bridge→구 tower는 `failed→blocked`(구 tower 기존 매핑으로 이미 안전)이지만, **구 bridge→새 tower는 tower가 먼저 고쳐져야만 `done→blocked`로 격리**되기 때문이다. bridge를 먼저 고치면 그 사이 구 bridge 인스턴스의 `done`이 board에 그대로 반영되는 창이 남는다. 변경면: `payload.status === "done" ? "done" : "blocked"`(`card-ops.ts:317-318` 실측)를 **어떤 status에서도 `blocked`** 로 고정하고 legacy `done`에 reason `legacy_remote_done_requires_verification`을 붙인다. **currency gate·스캔 폴백·L-2 authenticity·terminal guard·seq dedup은 전부 무변경 유지**(main 실코드 `card-ops.ts:137-315` — 브랜치는 이것들을 갖지 못했고 findTask miss 즉시 `{ok:false}`라 H-new-1 strand가 재발한다). 합성 가드 순서는 **설계 §2.4 코드블록이 정본**이다(브랜치 A2의 축자 순서가 아니다 — currency는 "태스크 해석" 단계의 일부이고 해석된 태스크는 예외 없이 L-2를 통과하므로 authenticity 우회는 생기지 않는다). 공개 계약 문서 동반 갱신: **MCP `apply_card_result` 설명**(`packages/mcp-server/src/stdio.ts:265` 부근 — 현행 "done → done, failed → blocked" 축자 서술) · **`docs/HERDR_DESIGN.md`**(`:175` `:380` `:619` 인용부) · **`docs/spikes/DISPATCH-DEMO.md`**. **락:** 설계 §6 **U2·U10**. **게이트:** G-2 · G-4. **대상 파일:** `packages/host/src/card-ops.ts` · `packages/mcp-server/src/stdio.ts` · `docs/HERDR_DESIGN.md` · `docs/spikes/DISPATCH-DEMO.md`. |
| **PATCH 3 (M3)** | **bridge A1+A4 — proposal 전용 API · 자동 `done` 5곳 제거 · 자동 close 3곳 제거 · 정정 3건 · 관측 배선 2건** | 이 PATCH가 권한 컷의 본체다. ① **발행 표면 재구성(A1 × main issuer, 설계 §2.2):** 완료 상태를 입력받는 `finishCard(flight, status, …)`를 제거하고 `proposeCardVerification(flight, opts)`(성공형) / `finishCardFailure(flight, reason, opts)`(failed형)로 이원화 — **이름은 브랜치를 승계하되 본체는 main의 발행 경로**(issuer acquire → seq 채번 → supersede → payload 각인 → `sendResult` 3분기 소비, `:2540-2668`)를 재사용한다. 브랜치 순이득 1건 편입: 성공형 제안은 `failed` status에서도 **실제 output summary를 보존**한다(main의 `failed reason=…` 치환 **`:2575-2578`**을 성공형에 쓰지 않는다 — **좌표 정정(아키텍트 실측)**: 설계 §2.2의 `:2578-2580` 표기는 삼항식 전체가 `const summary =` `:2575`에서 시작해 `.slice(0, 900)` `:2578`로 끝나므로 **끝 2줄이 무관 영역**을 가리킨다). ② **자동 `done` 구성 5곳 제거**(`:2171 :2197 :2256 :2288 :2315` — 아키텍트 재실측 일치). 제거 후 브릿지 card 경로의 `status:"done"` 구성은 **0곳**이어야 한다. ③ **자동 `pane.close` 3곳 제거**(`:1305` events_subscribe_failed · `:1981` inject_unconfirmed · `:2618` 성공 발행 뒤 cleanup — 재실측 일치). **유지(무관 경로): pool-root `:1127` · conv 명시 close `:1532`.** `paneCleanup:"auto"`는 conv 명시 close만 제어하고 card worker에 무효과가 된다(config 설명·`status` 도움말 문안 동반). ④ **§2.3 ① 비수락 통일:** `SendResultOutcome`(`:2671-2679`)에서 `rejected` variant를 제거하고 catch 블록의 **메시지-정규식 분기**(`:2774-2778` `looksTransport` + `/reject\|denied\|forbidden\|invalid/i`)를 **행동 분기에서 제거** — 비수락은 사유 불문 전부 `send_unknown` 흡수 + quarantine enter 1건이고 사유는 `reject:`/`ack_<status>_rc<n>`/`transport:` **reason 접두로만** 각인한다. **⭐ 규모 판단 근거(아키텍트 실측 — 구현자 오독 차단):** `classifyAck`(`:2687-2699`)는 **이미 2값 반환**(`"accepted" \| "send_unknown"` — 코드가 이미 2+1)이고 `rejected`는 **`sendResult` catch 블록(`:2780`) 1곳에서만** 구성된다. 따라서 **①의 실제 변경면은 분류기가 아니라 `SendResultOutcome` 타입 층 + 그 catch 블록의 정규식 분기 제거에 국한**된다 — 분류기를 손대는 구현은 스펙 이탈이다. ⑤ **§2.3 ② quarantine fold 정정:** `foldQuarantineLines`(`result-quarantine.ts:128-133`)에서 `process_exit`를 해소(`open.delete`)가 아니라 **관측 주석(no-op)** 으로 재분류. `process_exit` append 자체(`:361-392`)는 가시성 기록으로 유지. ⑥ **§2.3 ③ 운영자 ack 표면 배선:** `QuarantineStore.ack`(`:294-315`)의 production 호출 표면 신설 — 최소 CLI `loom bridge quarantine ack <cardId> <dispatchHandoffId> [--seq N\|--presence]` + `loom bridge status`의 미해소 카운트 표시. **이것은 "사람의 quarantine 해소" 기록이지 전달 확인이 아니다**(관측점 §4와 혼동 금지). ⑦ **`classifyAck` 모듈 스코프 승격 + export** — 현재 `startBridgeRuntime`(`:566`) 클로저 로컬(들여쓰기 2칸, 실측)이라 판정 로직에 양성 락을 걸 수 없다. **인자만 쓰는 완전 순수 함수이고 외부 캡처 0**(실측)이므로 이동은 무해·동작 무변경. **테스트가 요구해 발생한 리팩터이나 production 변경이므로 M4가 아닌 M3 소속**(§5.2 규율 — M4는 tests-only). ⑧ **flight map miss inert 경로 구조 로그**(설계 D10 이관분 · 구 §6.2 규칙 7) — 발행 후 `removeCardFlight`로 map에서 사라진 pane의 herdr 이벤트가 `onHerdrEvent`에서 inert 처리될 때 **structured log/counter 1건**(현행 silent return). **락:** 설계 §6 **U1·U3·U4·U5·U6·U7**. **게이트:** G-1 · G-3 · G-5 · G-6 · G-7 · G-10 · G-11. **대상 파일:** `packages/host/src/bridge-runtime.ts` · `result-quarantine.ts` · `bridge-config.ts` · `packages/cli/src/index.ts`(quarantine ack CLI · status 표시). |
| **PATCH 4 (M4)** | **테스트 재기술 §4.3 전량 + 브랜치 테스트 순이득 이식 (tests-only)** | 설계 §4.3 표가 정본. ① `impl-0270.test.ts` ①(`:662-684`)·①b(`:686-705`) → **accepted (a) 주입 seam 행(음성 대조군)** 으로 이전: 어서션 = 주입기 호출 **정확 1건**(테스트 측 클로저 계측) + **pane 보존 양성** + 보조로 이 cardId의 `kind:"enter"` 레코드 0건(파일 fold 경유). **`flight.resultPhase`/`relayAcceptedAt`을 어서션하지 마라 — 관측 불가**(UNVERIFIED ⑥ 종결). **relay 수신 payload도 요구하지 마라 — 주입은 wire send를 early-return으로 우회한다**(`relay-client.ts:382` · `impl-0270.test.ts:669-671` 주석 축자). **이 함정은 v0.27.0 구현 검증에서 이미 한 번 발생한 재발형이다.** ② **신설 `classifyAck` 순수 유닛 4케이스**(`queued/1`→`accepted` · `delivered/1`→`accepted` · `peer_unknown/0`→`send_unknown` · `delivered/2`→`send_unknown`) — accepted **판정 로직의 양성 락**이 서는 유일한 자리. ③ **신설 accepted (b) 실 relay 도달 행** — `pane-cleanup.test.ts` real-relay 경로에 payload **정확 1건** + `(cardId, dispatchHandoffId, seq≥1)` 각인 + `status="failed"`/`reason="needs_verification"`/summary 보존 + board `blocked` + notes `last_seq=N` + pane 보존. **(b)가 없으면 "전달됐는가"가 어떤 테스트로도 관측되지 않는다.** **선행 필요:** `pane-cleanup.test.ts:92-109`의 `awaitCardResult`는 **첫 매치를 반환할 뿐 개수를 세지 않으므로**, "정확 1건"을 쓰려면 **필터 결과 length를 반환하는 헬퍼 변형이 선행**돼야 한다(이 PATCH 항목). ④ `impl-0270.test.ts` ②(`:718`)·②b(`:744`)·③(`:761`) → 비수락 행으로 재기술(②b·③에 quarantine 양성 추가; 미해소 어서션은 **test-owned store 기준** — 브릿지 자신의 store는 클로저 로컬이라 직접 호출 불가). ⑤ `impl-0270.test.ts` ⑤(`:785-790`) → payload 각인 유지 + close 관측을 pane 보존 양성으로 교체. ⑥ `impl-02311.test.ts` ⑤⑥ · `inject-verify.test.ts` ③ → **감지력 복원**(브랜치에서 `toBe(0)`/`toHaveLength(0)` 반전 = 진짜 약화 3건): result 정확 1건 양성 + pane 보존 양성 + **회수 책임 주석**(후속 B·운영자). ⑦ `presence_unknown`은 **도달 불가 명시 주석**(스킵 블록으로 위장 금지 — U7). ⑧ 브랜치 순이득 이식: 구독-대기 헬퍼 `hasPaneStatusSubscription`(5파일) · `scripts/test-setup.ts` env 오염 차단(+10줄) · reinject CR 관측 확장 · 픽스처 시딩 레이스 제거 · close 정확-개수 패턴. **⚠️ 브랜치 `bridge.test.ts` +244줄은 전량 정독 후 이식**(설계 §8-3 미확인 — diffstat·부분 인용까지만 확인됨). **락:** 설계 §6 **U7**(정본) · U1~U5 게이트 실증. **게이트:** G-1 · G-3 · G-5 · G-9 · G-11 · G-8(전체 스위트). **대상 파일:** `impl-0270.test.ts` · `pane-cleanup.test.ts` · `impl-02311.test.ts` · `inject-verify.test.ts` · `still-running.test.ts` · `impl-02312.test.ts` · `impl-0239.test.ts` · `bridge.test.ts` · `scripts/test-setup.ts`. |
| **PATCH 5 (M5)** | **dist 재번들 + 버전 0.28.0 + config/status/docs 문안** | VERSION 동기 갱신 **2곳** — CLI `VERSION`(`packages/cli/src/index.ts:144` 실측 현행 `"0.27.0"`) + MCP serverInfo(`packages/mcp-server/src/stdio.ts:403` 실측 현행 `"0.27.0"`). **0.23.7 미갱신 사고 선례**로 이 두 좌표는 매 릴리스 체크 항목이다. dist 재번들은 **양 노선 dist가 모두 0.27.0을 주장하는 충돌을 자연 해소**한다(설계 §8-4 — 그래서 dist 대조는 생략이 정당하다). config/status 문안: `paneCleanup` 설명(card worker 무효과) · `loom bridge status`의 **미해소 quarantine 카운트** + **보존 card pane 카운트**(설계 §3.2 (ii) — 관측만, 자동 GC 아님). **락:** 설계 §6 **U10·U11**. **게이트:** G-8. **대상 파일:** `packages/cli/src/index.ts` · `packages/mcp-server/src/stdio.ts` · `packages/host/src/bridge-config.ts` · `dist/`. |

**PATCH 2 implemented as of `0b335a1` (2026-07-22, Grok headless 구현 · Codex 아키텍트 독립 검증):** 선행 red 기준 `24ceede` 위에서 지정 4파일만 변경(+23/−13) — tower `applyCardResult`의 모든 fresh 원격 result를 `blocked`로 고정하고 legacy `done`에 `legacy_remote_done_requires_verification`을 각인했으며, currency/scan fallback/L-2/terminal/seq 가드 순서는 보존했다. MCP 설명·`HERDR_DESIGN`·`DISPATCH-DEMO` 공개 계약도 U2에 맞춰 동기화했다. 집중 G-2 계약 **3/3 green**, host·MCP typecheck **2/2 green**, `git diff --check` green. 전체 suite는 단계적 red-test 규율대로 **717 pass / 8 skip / 11 fail**: PATCH 3의 ①·③ expected-red 4건, PATCH 4에 배정된 legacy test expectation 3건, 선재 HANDOFF Phase-C 기대 1건, macOS 장경로 socket/연쇄 fixture flake 3건으로 분류했으며 PATCH 2 대상 밖 테스트를 production 커밋에 섞지 않았다. 다음 = **PATCH 3 (M3)**.

**PATCH 3 implemented as of `c475604` (2026-07-22, Codex CLI orchestration · Grok 4.5 headless 구현 · Codex 아키텍트 독립 검증):** 선행 `0b335a1` 위에서 bridge completion authority를 제거했다(+587/−135, production 6파일). 성공형 result는 실제 summary를 보존한 `failed/needs_verification` 제안만 발행하고, card `status:"done"` 구성 0곳·card auto-close 0곳을 정적 확인했다. 비수락 ACK는 `send_unknown` + durable quarantine 한 경로로 합쳤고, `process_exit` replay 보존·운영자 ack CLI·live daemon authenticated loopback ack·미해소 status·순수 `classifyAck`·flight map-miss 구조 관측을 배선했다. host·CLI typecheck, diff-check, 독립 quarantine/CLI/classifier smoke(**unresolved=0 · classify=4/4**)는 green. 전체 suite는 단계적 red-test 규율대로 **691 pass / 8 skip / 37 fail**: PATCH 4 tests-only 재기술 대상의 legacy `done`/auto-close 기대 **33건**, 선재 HANDOFF checkpoint 기대 1건, macOS 장경로 socket 2건, 선재 offline dispatch durability fixture 1건으로 전량 분류했으며 production 커밋에 테스트를 섞지 않았다. 다음 = **PATCH 4 (M4) tests-only**.

**PATCH 4 implemented as of `f9b0230` (2026-07-22, Codex CLI orchestration · Grok 4.5 headless 구현/교정 · Codex 아키텍트 독립 검증):** 선행 `c475604` 위에서 §4.3과 분류된 legacy `done`/auto-close 기대를 proposal/blocked/pane-preserved 계약으로 재기술했다(테스트/하네스 10파일 +923/−227, **production diff 0**). `classifyAck` 4분기, accepted 주입 seam 음성 대조군, real-relay payload exact-one·각인·board blocked, 비수락 quarantine exact-one, live/offline ack, pane 보존 양성, settle 후 중복 탐지, 구독 대기/env 격리/fixture seed/close-count 브랜치 순이득을 고정했다. PLAN 표에서 빠졌지만 HANDOFF가 명시한 `scrape-delta.test.ts`·`herdr-lifecycle.test.ts`도 포함했다. `ec99b2c:bridge.test.ts`는 전량 감사했으며 main의 PATCH 1이 guard-order/local-done과 3개 exact-one race를 더 강하게 이미 고정해 무변경으로 종결했다. 집중 9파일 **165 pass / 1 fail**(선재 macOS 장경로 socket), typecheck **6/6 green**, 전체 **738 pass / 5 fail / 743 tests / 306.40s**: 선재 checkpoint 1 + macOS 장경로 socket 2 + 선재 offline durability 1 + 전체-suite 부하에서만 발생한 구독 관찰 timeout 1이며, 마지막은 즉시 단독 **30/0 / 15.87s**로 비재현됐다. PATCH 4 소유의 재현 가능한 실패는 0. 다음 = **PATCH 5 (M5) version/dist/config/status/docs close**.

**PATCH 5 implemented as of `d49a6b1` (2026-07-22, Codex CLI orchestration · Grok 4.5 headless 구현 · Codex 아키텍트 독립 검증):** CLI `VERSION`과 MCP `serverInfo.version`을 0.28.0으로 동기화하고 `dist/loom.js`를 재번들했다. `paneCleanup`은 explicit conv close 전용·card pane 항상 보존임을 명료화했고, live status에 `quarantineUnresolved`를 유지한 채 count-only `preservedCardPanes`를 추가했다. 이 카운트는 현재 프로세스가 Flight-backed result 뒤 보존했다고 관찰한 pane ID만 세며 terminal 관찰/stop에서 제거하고 이전 bridge 상태를 복원하지 않는다(자동 cleanup·receipt·recovery 권위 0). PLAN 대상 표가 status 구현 파일을 빠뜨려 `bridge-runtime.ts`와 양성 테스트 2건을 보수적으로 포함했다. focused **36 pass / 1 known macOS socket fail**, typecheck **6/6**, `loom v0.28.0`, dist guard green, 전체 **741 pass / 4 fail / 745 tests / 302.93s** — checkpoint 1 · macOS 장경로 socket 2 · 선재 offline durability 1로 전량 분류됐고 PATCH 4 suite-order timeout은 비재현. PANE-DEATH PATCH 1–5 종결; 다음 독립 제품 gate = **herdr 0.7.5/protocol-17 adapter**.

**회귀 게이트 (설계 §5.3 — G-1~G-11 전량이 v0.28.0 수용 조건):**
...
| G-4 | rolling-upgrade fixture **양방향** `blocked` 수렴 | 설계 §2.4 |
| G-5 | 비수락 ACK 전형(`peer_unknown/0` · `delivered/2` · transport throw · 명시 거부 주입) 전부 quarantine enter 1건 + result 재발행 0 | 락 U5·U6 |
| G-6 | quarantine 재기동 replay가 **정상-종료 이력 후에도** 미해소 복원 · 운영자 ack fold 동작 | 락 U6 |
| G-7 | `DispatchResultIssuer`·`claimResultIssue`·`commit_unknown` 식별자 **리포 잔존 0건**(브랜치 부분 이식 차단) | 설계 §2.2 |
| G-8 | 기존 M-1 · L-2 · 0.23.4 · 0.23.7 · CONV_SPEC 회귀 green · 전 패키지 typecheck · **전체 스위트 차집합 0**(아키텍트 독립 실행 SSOT — lessons (18)) | 기존 계약 |
| G-9 | at-most-one: 경쟁 3형(완료↔terminal · claimed 경로 · spawn 실패) relay 수신 **정확 1건** | 락 U4 |
| **G-10** | flight map miss inert 처리의 **구조 로그**: 발행 후 map에서 사라진 pane의 herdr 이벤트가 inert 처리될 때 structured log/counter 1건(조용한 return 0건). **판정 대상 심볼(R45 Low 3 — 실측 확정)**: 함수 `onHerdrEvent`(`packages/host/src/bridge-runtime.ts:1995`)의 **두 map miss 경로** — ① `const flight = flights.get(paneId)`(`:2001`)가 falsy **그리고** ② `const convFlights.get(paneId)`(`:2007`)도 falsy → 함수가 **아무 로그 없이 끝난다**(`:2011` 이후 암묵 return; 현행 무로그 실측). 통과 조건 = 이 정확한 경로(card flight miss ∧ conv flight miss)에서 **관측물 1건**이 남는 것 — `paneId`·정규화된 `ev`를 필드로 갖는 structured log **또는** 단조 증가 카운터 중 **하나면 충분**(둘 다 요구하지 않는다). ⚠️ **`:1999`의 `if (!paneId) return`은 G-10 대상이 아니다** — 그것은 map miss가 아니라 **pane_id 없는 이벤트**의 별개 경로다. 심볼 이름·매체(log vs counter)·필드 집합은 **구현 자유** | 설계 §0.2 D10 · 구 §6.2 규칙 7 — **본 세션 신설**(R44 L4 반영) · 심볼 실측 = R45 Low 3 |
| **G-11** | `classifyAck` 순수 유닛 **4케이스 직접 어서션** 통과(모듈 스코프 export 후 직접 호출) — G-5의 비수락 quarantine 레코드로는 덮이지 않는 **accepted의 양성 락** | 락 U5·U7 — **본 세션 신설**(UNVERIFIED ⑥ 종결분) |

**Out of scope (이 버전 아님 — 설계 §7):**
- **후속 A — durable outbox · stable eventId · 재전송 · tower atomic dedup+apply receipt · Flight-less startup recovery.** relay/wire 표면 변경이 필요해 로컬 변경으로 닫히지 않는다. 그 전까지 **U11의 과장 금지가 정직성 경계**다.
- **후속 B — cleanup grant(tower→bridge 역방향 계약) · tombstone/TTL 재도입 · pane GC.** 설계 §3.2: main 구 락 11의 자동 cleanup 조건("사람 확정 + tower receipt")은 **그 receipt를 브릿지에 전달할 역방향 계약이 존재하지 않아** 만족할 방법이 없다. **조건을 만족할 수 없는 자동화는 규정할수록 위험하다** — 구현자가 "확정 신호의 근사물"(board 폴링·시간 경과)을 발명하게 되고 그 근사물이 정확히 이 트랙이 금지해온 "불확실 관측발 비가역 행동"이다. 따라서 v0.28.0의 규범은 **"card 자동 close 0"** 으로 잠근다(U3).
- **후속 C — 자연사 관측(`pane_exited` 구독·처리) + bounded reconcile cadence + replay 방어.** 실측: `bridge-runtime.ts`에 `pane_exited`/`pane.exited` **비테스트 소스 0건**, 전역 구독도 `pane.closed`뿐(`:660`). **자연사한 pane은 양 노선 모두에서 관측되지 않는다** — 트랙 이름이 약속한 "PANE-DEATH 인지"는 v0.28.0 이후에도 열려 있다. pane 보존(U3) 하에서 자연사의 피해는 "카드가 `doing`에 잔존"으로 국한되고 이는 quarantine·board 정체로 사람에게 보이므로 **급성 위험이 아니라 관측 부채**다.
- **conv 경로의 ACK 폐기·비가역 상태 전진 — U4 후속(기존 명명 승계).** 실측: `void sendWorkerTurnFromPane` `:1587 :2492 :2511 :2516` · 봉투 미소비 `:1353 :1372 :1830`. **양 노선 공히 미수정**(main pre-C도 conv를 고치지 않았음이 실측 확정). **card-only 완료를 주장하지 않는다**(U11).
- **`rejection_escalation`(tower가 제안을 "거절"하는 계약)** — 거절을 표현할 계약 자체가 없어 dormant 행 유지. 시대 매개변수화(구 §6.7.2)의 pre-C `[v0.27.0]=done` 행은 **역사화**된다(v0.28.0부터 성공형 initial = `needs_verification` 단일 — 설계 §1.4).
- **사람/verifier provenance 강제 · 자동 `done` 재도입** — verifier 결과·artifact hash·durable 적용이 모두 갖춰진 좁은 경로에만(구 락 13 조건 승계). **운영 답답함은 사유가 아니다.**
- **`PaneLifecycle` 12원소 union** — 채택하지 않는다(설계 §1.4). v0.28.0의 실상태는 main 코드의 최소 집합(`ResultSendPhase` 4원소 `:369-373` + CAS/latch 불리언)이면 충분하고, `awaiting_human_verification`은 브릿지 상태가 아니라 **board(`blocked`)와 사람의 상태**다 — 브릿지가 관측하지 않는 것을 브릿지 phase로 두지 않는다.
- **bridge restart 후 active binding 복원** · **`ROLE-PERMISSION-PROFILE-UNIFICATION.md`**(독립 MINOR 후보, PANE-DEATH와 무관 — 브리프 §4-5 종결).

**Security / trust:**
- **경계가 좁아지는 방향이다.** 브릿지는 `done`을 **구성할 수 없게** 되고(U1), tower는 원격 result로 `done`을 **만들 수 없게** 되며(U2), card pane을 닫는 자동 경로가 **0곳**이 된다(U3). 완료 권위가 **사람의 명시적 로컬 board mutation** 하나로 수렴하므로 권한 확대는 없다. 되돌리면 결함 213/215/217(가짜 done)의 구조적 원천이 부활한다.
- **rolling upgrade는 양방향 fail-closed다.** 새 bridge→구 tower = `failed→blocked`(구 tower 기존 매핑) · 구 bridge→새 tower = `done→blocked`(fence). **어느 조합에서도 `done`이 자동으로 서지 않는다** — 그래서 PATCH 2(tower)가 PATCH 3(bridge)보다 **먼저**다.
- **provenance는 강제되지 않는다(정직 기재).** v0.28.0은 board `done`을 **명시적 로컬 mutation으로만** 만들지만 그 mutation의 주체가 사람/verifier임을 **증명하지 않는다** — 운영 규약이지 기계적 보장이 아니다(설계 §1.1 각주 · provenance 강제는 후속).
- **quarantine 레코드는 메타데이터-only.** 워커 산출물 본문을 싣지 않는다(0.26.0 D6 계측 레코드 규약과 동형 — 간접 프롬프트 주입 표면 차단). 성공 경로의 수신증을 이 파일에 쓰지 않는다(U6) — 예외 원장에 전달 저널을 섞으면 사람-해소(`ack`)와 전달-확인이 **데이터 레벨에서 섞여** 불변식의 두 권한 경계가 붕괴한다.
- **신규 로컬 표면 1종:** 운영자 ack CLI(`loom bridge quarantine ack …`)는 **브릿지-로컬 JSONL을 읽고 append**할 뿐 relay·tower·wire를 경유하지 않는다. 원격 표면 확대 0.
- **payload 각인은 영속 표면이다(무변경 승계).** `(cardId, dispatchHandoffId, seq)`는 tower inbox에 영속되므로 사후 정정 불가 — v0.27.0 규약을 **그대로 유지**하며 이번 PATCH가 각인 규약을 바꾸지 않는다는 것이 U10의 내용이다.

**추정 / 실측 구분 (lessons (30) — 미실측을 "없음"으로 적지 않는다):**

| 명제 | 지위 |
|---|---|
| 브릿지 자동 `done` 구성 **5곳** = `:2171 :2197 :2256 :2288 :2315` | **코드 실측 확정** — 본 PLAN 작성 시 `finishCard(flight, "done"` grep 재확인, 설계·R44와 일치 |
| card 자동 `pane.close` **3곳** = `:1305 :1981 :2618` · 무관 유지 2곳 = `:1127`(pool-root) `:1532`(conv 명시) | **코드 실측 확정** — `pane.close` grep 재확인, 설계와 일치 |
| tower가 원격 `done`을 `done`으로 적용 | **코드 실측 확정**(`card-ops.ts:317-318` — `payload.status === "done" ? "done" : "blocked"`) |
| Flight-less 3경로 = `:893`(payload_invalid) `:1160`(agent_kind_not_allowed) `:1250`(herdr_spawn_failed) | **코드 실측 확정** — `await sendFailedResult({` 6곳 중 나머지 3곳(`:1293 :1321 :1966`)은 `flight`를 넘긴다. **구 좌표 `:850 :1114 :1204`는 R44 M1로 폐기 — 역사로만 두고 구현 탐색에 쓰지 마라** |
| `classifyAck`는 **이미 2값 반환**이고 `rejected`는 catch 블록 1곳에서만 구성 | **코드 실측 확정**(`:2687-2699` 정의 · `:2671-2679` `SendResultOutcome` · `{ branch: "rejected" }` = `:2780` 단독) — §2.3 ① 규모 판단의 근거 |
| `classifyAck`가 `startBridgeRuntime` 클로저 로컬(승격 필요) | **코드 실측 확정**(`startBridgeRuntime` `:566` · `classifyAck` 들여쓰기 2칸 · 외부 캡처 0) |
| `flight.resultPhase`/`relayAcceptedAt`은 **테스트에서 관측 불가** | **코드 실측 확정**(`type Flight` `:375` 미export · `flights` 맵 클로저 로컬 · 외부 참조는 `impl-0270.test.ts:811` 소스 문자열 락 1건) — **UNVERIFIED ⑥ 종결분** |
| `pane.close`(`:2618`)를 게이트하는 `const sent = outcome.branch === "accepted"`(`:2610`)의 대상은 그 close **단 하나** | **코드 실측 확정** — 그래서 U3 적용 후 주입 경로 accepted 양성 관측물이 0이 된다 |
| quarantine `process_exit`가 미해소를 해소로 fold | **코드 실측 확정**(`result-quarantine.ts:128-133` `open.delete` · `:361-392` 종료 시 전 미해소 append) — **단 구현 의도는 미확인**(UK-3) |
| `QuarantineStore.ack` production 호출 0 | **코드 실측 확정**(`:294-315` 정의 · 유일 호출 `impl-0270.test.ts:217`) |
| `pane_exited` 처리 코드 0건 | **코드 실측 확정**(비테스트 소스 grep 0건 · 전역 구독은 `pane.closed`뿐 `:660`) — **단 `pane.closed` 구독으로 전달되는지는 미확인**(UK-2) |
| VERSION 현행 `0.27.0` 2곳 | **코드 실측 확정**(`packages/cli/src/index.ts:144` · `packages/mcp-server/src/stdio.ts:403`) |
| `awaitCardResult`(`pane-cleanup.test.ts:92-109`)가 개수를 세지 않음 | **인용**(설계 §4.2 註 실측) — M4 헬퍼 변형 선행 필요의 근거 |
| relay 명시-거부 봉투의 실제 wire 형상 | **미실측**(UK-1) — 그래서 정규식 휴리스틱을 **행동 분기에서 제거**한다(어떤 형상이 오든 흡수로 수렴) |
| 브랜치 `bridge.test.ts` +244줄의 이식 가치 | **부분 확인**(diffstat + 인용 `:244-300 :344-346 :802-906`) — **전량 정독은 M4 착수 시 의무** |

**Unknowns (§3.5 — 본 PLAN의 미해소 질문. 설계 §6 락 U{n}과 번호가 충돌하지 않도록 `UK-` 접두를 쓴다):**
- **UK-1** — relay 명시-거부 봉투의 실제 wire 형상(설계 §8-1). correctness에는 걸리지 않는다(비수락 전부가 `send_unknown` 흡수로 수렴 — U5). 영향 범위는 **quarantine `reason` 각인의 품질**뿐이며 실측 후 개선한다. **v0.28.0 착수를 막지 않는다.**
- **UK-2** — `pane_exited`가 `pane.closed` 구독으로 전달되는지(설계 §8-2). **후속 C의 선결 프로브**이며 v0.28.0 범위 밖. 본 PLAN이 실측한 것은 "처리 코드 0건"뿐이고 구독 표면 실측은 하지 않았다.
- **UK-3** — quarantine `process_exit` fold의 **원래 구현 의도**(설계 §8-5). 코드=SSOT 원칙상 "현 동작 = 정상 종료 시 미해소 소실"이 사실이고 §6.7.1 취지와의 불일치 판정은 **설계의 해석**이다 — **R45에서 재확인 대상**(해석이 틀렸다면 PATCH 3 ⑤가 무근이 된다).
- **UK-4** — 설계 §6 **U8의 철회 문구**("동일 generation 결속 연속 absence = 결정적 부정 증거" 명명 철회)가 **R43b §4의 "락 11 재론 기각"과 충돌하는가.** → **미검증 이월 ⑨ 참조.** 이것이 **U8 철회의 정당성이 걸린 유일한 미검증 항목**이다.

> **U8의 v0.28.0 범위 판정 — ⑨ 미검증은 착수를 막지 않는다 (R45 Medium 2 반영 · 아키텍트 실측 확정).**
> R45(grok 레인)는 *"U8이 잠긴 상태인데 그 정당성(⑨)이 열려 있다"* 며 **"구현 전 ⑨ 종결 또는 U8을
> provisional 표기"** 를 처방했다. **처방을 축자 반영하지 않는다(교훈 31 — 처방이 아니라 결함을 본다).
> U8을 provisional로 강등하지 않으며 §6 U8 문안은 불변이다.** 근거는 **범위**다:
> - **U8은 v0.28.0의 어떤 PATCH에도 걸리지 않는다.** 위 PATCH 표의 락 목록 실측 —
>   PATCH 1 `U1·U2·U3·U4·U7` / PATCH 2 `U2·U10` / PATCH 3 `U1·U3·U4·U5·U6·U7` /
>   PATCH 4 `U7`(+`U1`·`U5` 게이트 실증) / PATCH 5 `U10·U11`. **U8은 어디에도 없다.**
> - **U8은 G-1~G-11 어느 게이트의 판정 근거도 아니다**(판정 근거 열 실측: `U1`·`U2`·`U3`·설계 §2.4·
>   `U5·U6`·`U6`·설계 §2.2·기존 계약·`U4`·설계 §0.2 D10·`U5·U7`).
> - **코드 표면 영향 0.** U8이 다루는 presence 계열 관측의 배선은 **후속 C**이고, v0.28.0에서
>   `presence_unknown`은 **도달 불가**로 명시돼 있다(설계 §2.5·§4.2 표 `presence_unknown` 행·§7).
>   즉 U8의 문안이 옳든 그르든 **v0.28.0이 만드는 코드는 한 줄도 달라지지 않는다.**
>
> **→ 따라서 ⑨ 미검증은 v0.28.0 구현 착수를 막지 않는다.** 단 **⑨는 후속 C 착수의 선결 조건**이다 —
> presence 계열을 실제로 배선하는 시점에 비로소 U8의 문안이 하중을 받기 때문이다. **codex 레인
> 복구(2026-07-25 14:27) 후 최우선 재검증 지정은 유지한다.**
>
> **정직성 고지:** R43b §4는 이 긴장을 *"(C) 본체 R 게이트에서 재확인"* 으로 미뤘고, **v0.28.0이
> 바로 그 본체다.** 그럼에도 **U8이 이번 범위 밖이라 R44·R45 어느 게이트에서도 R43b §4와의 축자
> 대조는 이뤄지지 않았다.** 본 PLAN은 "충돌 없음"을 주장하지 않는다 — 주장하는 것은 **"이번 버전의
> 코드 표면에 U8이 닿지 않으므로 판단을 미뤄도 안전하다"** 까지다.

**미검증 이월 ("없음"과 구별 — 교훈 28. 종결분과 존속분을 명시 구분한다):**

| # | 출처 | 항목 | 처리 |
|---|---|---|---|
| ⑥ | R44 UNVERIFIED | ACK 주입 경로에서 `resultPhase`/`relayAcceptedAt`의 테스트 관측 가능성 | **종결 — 관측 불가 확정**(위 blockquote). (a) 행 재설계 + `classifyAck` 유닛(G-11) 신설로 대체 |
| ① | R44 / 설계 §8-1 | relay 명시-거부 봉투의 실제 wire 형상 | **존속** → UK-1 |
| ② | R44 / 설계 §8-2 | `pane_exited`가 `pane.closed` 구독으로 전달되는지 | **존속** → UK-2 · 후속 C |
| ③ | R44 / 설계 §8-3 | 브랜치 `bridge.test.ts` +244줄 전량 정독 미완 | **존속** — **M4(PATCH 4) 착수 시 전량 정독 의무** |
| ④ | R44 / 설계 §8-5 | quarantine `process_exit` fold의 원 구현 의도 | **존속** → UK-3. **R45가 다루지 않았다**(grok UNVERIFIED #6으로 재신고) — 초판의 "R45 재확인" 표기는 사실과 어긋나 정정한다. **PATCH 3 ⑤ 착수 시 확인**이 실질 기한이다(그 정정이 이 해석 위에 서 있으므로, 해석이 틀리면 PATCH 3 ⑤가 무근이 된다) |
| ⑤ | R44 / 설계 §8-6 | `bun test` · 라이브 스모크 · dist 대조 미실행(설계 단계 지시대로) | **존속** — 구현 웨이브에서 G-8이 해소. dist 대조는 **M5 재번들이 자연 해소**(설계 §8-4) |
| ⑦ | R44 / 설계 §8-7 | 브랜치 워크트리 HEAD `dfc1eeb` 태그 정렬 · 미커밋 문서 변경(`tasks/lessons.md` 충돌 소지) | **존속** — 통합 시 별도 처리, 본 PLAN 범위 밖 |
| ⑧ | R44 보고자 추가 | "검사한 좌표 87"이라는 **수치 자체가 미검증**(표본 6건만 재실측) | **존속** — 본 PLAN이 재실측한 좌표는 위 실측 표에 열거한 범위뿐이다 |
| **⑨** | R44 보고자 추가 · **R45 Medium 2 재지적** | **U8 ↔ R43b §4 "락 11 재론 기각" 충돌 여부.** grok은 충돌 없음으로 봤으나 **근거가 얇다** — 정합 표 한 줄뿐이고 R43b §4 **축자 대조 미실시**. R44·R45 어느 게이트에서도 축자 대조는 이뤄지지 않았다 | **존속 · 최우선.** **codex 레인 복구 후(리셋 2026-07-25 14:27) 가장 먼저 재검증**한다 — 락 U8 철회의 정당성이 걸린 **유일한** 미검증 항목이다. **단 v0.28.0 착수는 막지 않는다** — U8이 어떤 PATCH·게이트에도 없고 presence 배선은 후속 C이며 `presence_unknown`은 도달 불가(위 blockquote 실측). **⑨는 후속 C 착수의 선결 조건**으로 재지정 |
| ⑩ | R44 보고자 추가 | shell 명령 3건이 allowlist 밖이라 차단 — grok이 read_file/grep으로 우회했으나 **그 3건이 겨냥한 확인은 이뤄지지 않았을 수 있다** | **존속** |
| **⑪** | **R45 GAPS (신규)** | **red-test 선행 규율(테스트 커밋과 production 커밋을 섞지 않는다)의 전 PATCH 경계 판정 미실시.** R45는 `classifyAck` 모듈 스코프 승격의 **M3 배정 1건만** 검사했고, PATCH 1~5 전 경계에 대해 "이 커밋에 production 변경이 섞이는가"를 전수 판정하지 않았다 | **존속 — 미확인**(“없음”이 아니다). **R46 또는 구현 착수 시 확인 항목.** 각 PATCH의 diff 성격(tests-only / production)을 커밋 전에 대조한다 — 특히 **PATCH 1·PATCH 4는 tests-only 선언**이므로 production 1줄이라도 섞이면 규율 위반이다 |

**정직성 — 이 PLAN이 주장하지 않는 것 (설계 §6 U11 준수 · §8 정직성 절 승계):**
- **delivery liveness 완결을 주장하지 않는다.** durable outbox·재전송·apply receipt는 전부 후속 A다.
- **exactly-once를 주장하지 않는다.** 최대 보장은 "committed 판정이 거짓 양성을 내지 않는다 + 로컬 발행 1회(local single-issue)"까지이며 이는 wire exactly-once와 **별개 어휘**다(U5).
- **유실·고아를 해소하지 않는다.** `send_unknown` 흡수와 durable quarantine은 이를 **관측 가능하게 만들 뿐 회수하지 않는다.** 정확한 효과 한 줄(구 §9-ter 승계): **"거짓 성공 제거, 유실·고아는 관측 가능하되 존속."**
- **자연사 pane을 인지하지 않는다(후속 C).** 트랙 이름이 약속하는 것의 **절반**(거짓 성공 제거 · 전달 무결 · 권한 경계)만 닫히고 **관측 축은 부채로 남는다.**
- **자동 cleanup을 하지 않는다(후속 B).**
- **conv 경로 완결을 주장하지 않는다(U4 후속).**

**운영 대가(정직 기재 — 이 PLAN이 지불하게 만드는 것):**
- **pane이 쌓인다.** card 자동 close가 0곳이므로 후속 B 전까지 **운영자 수동 정리**다. v0.28.0의 완화는 (i) 사람 확정 후 운영자 수동 close 허용(사람 권한이므로 불변식 위반 아님) (ii) `loom bridge status`에 **보존 pane 수 표시**(관측만) 둘뿐이다. 자동 GC는 없다.
- **사람 확정 리드타임이 전 카드의 필수 경로가 된다.** (C) 전환의 알려진 대가이며, 이를 줄이는 자동화는 **락 U1~U3의 재도입 조건(구 락 13)** 을 통해서만 정당화된다 — **운영 답답함은 사유가 아니다.**

**Review impact:** 브릿지 발행 API 형상(완료 상태 입력 API 제거 → proposal 전용) · 자동 `done` 5곳 및 자동 `pane.close` 3곳 제거 · **tower `apply_card_result`의 적용 의미 변경**(원격 result → `blocked` 격리, 공개 계약 문서 갱신 동반) · quarantine `process_exit` fold 의미 정정 · 운영자 ack CLI 신설 · 관측점 재설계(부정 어서션 → 분기별 양성). **card contract v1 · `done|failed` enum · relay/conv wire · MCP 이름·입력 schema · herdr RPC 표면은 전부 무변경**(U10)이나 적용 의미 변경이 board 상태 전이를 바꾸고, card 자동 close 제거는 **운영 절차 자체를 바꾼다**(pane 수동 정리). 설계 정본 [`docs/spikes/PANE-DEATH-UNIFIED-DESIGN.md`](spikes/PANE-DEATH-UNIFIED-DESIGN.md) §1~§8 · 락 **U1~U11** · 회귀 게이트 **G-1~G-11**. R44 검증 원장 [`docs/reviews/PANEDEATH-R44.md`](reviews/PANEDEATH-R44.md)(정본·수정 금지) — Medium 4·Low 2는 설계에 반영 완료, 잔여 Low 3건은 본 PLAN 작성과 동반 종결(L2·L4 반영 · **L5 no-op 기각**). 선행 원장 [`PANEDEATH-R43{,B,C,D,E}.md`](reviews/) · [`PANEDEATH-CODEX-REVIEW3.md`](reviews/PANEDEATH-CODEX-REVIEW3.md) 전부 정본·수정 금지. **R45 필수 — author-close 불가**(`WORKFLOW.md` §5.1 신규 MINOR + 신뢰 경계, 설계 §8 `rgate=needed`). **R45가 최소 확인해야 할 것:** ① PATCH 2→3 순서 근거(구 bridge→새 tower 안전 선행)가 rolling upgrade 표와 정합하는가 ② PATCH 3의 §2.3 ① 변경면이 **분류기가 아니라 타입 층 + catch 블록**에 국한된다는 판정이 옳은가 ③ UNVERIFIED ⑥ 종결(관측 불가 확정)과 그 처방(G-11 신설 · (a) 행 음성 대조군화)이 **교훈 40의 재범을 실제로 차단**하는가 ④ quarantine fold 정정(PATCH 3 ⑤)이 UK-3(구현 의도 미확인) 위에 서 있음을 범위 경계가 정직하게 반영했는가 ⑤ 정직성 절이 주장하지 않는 것(liveness 완결·exactly-once·자연사 인지·자동 cleanup)과 운영 대가(pane 축적·사람 확정 리드타임)를 **U11 수준으로** 기재했는가 ⑥ 미검증 이월 ⑨(U8 ↔ R43b §4)의 **최우선 재검증 지정**이 락 철회의 정당성 요구를 충족하는가.

#### 0.27.0 — 2026-07-21 (`approved` **R43e approve — v0.27.0 게이트 통과** — **result 발행 단일 소유권 + ACK 경계 — relay ACK 실측 선결 · strict ACK 판정 · issuer 중앙화 + seq 소유권 이전 · tower currency 게이트 · fire-and-forget 제거** (MINOR))

> **R43 reject → R43b 개정 (2026-07-21).** R43(원장 `docs/reviews/PANEDEATH-R43.md`, 정본·수정 금지 · 2레인 독립 수렴)이 High 6·Medium 5로 reject했다. 아키텍트가 fable-advisor 자문 + 코드 실측(evidence-r43b)을 거쳐 결정 ①~⑩을 확정하고 PLAN·설계에 반영했다: 락 8 만료 dispose 삭제 → 진입 시점 durable quarantine(H2·H6) · dispatch-scoped result issuer 중앙화 + seq 소유 Flight→issuer(전제 P) · `cardSeq` 삭제 요구 철회(전제 Q — attempt 카운터) · tower currency 게이트 편입(H3·H4) · 가드 5곳·순서 역전 대상 15곳 정정(H5) · D1/D1-b 완료 판정 재설계(M1·M2) · conv 전수 열거 + card-only 경계(M3·M4) · reconcile 회수 out-of-scope 명시(M5). 버전 유지, status `pending-review`(R43b 대기).
>
> **R43b 검증 → 2차 개정 (2026-07-21).** R43b 개정본(커밋 `5cd070b`)을 2레인 독립 검증했다 — claude 레인 **pending-revision**, codex 레인 **reject**(원장 [`docs/reviews/PANEDEATH-R43B.md`](reviews/PANEDEATH-R43B.md), 정본·수정 금지). R43 지적 12건 해소는 양 레인 수렴이나 신규 High 4·Med 3·Low 3가 났고, 아키텍트가 **pending-revision**을 채택해 국소 정밀화로 반영했다: §6.7.3 **dispatchHandoffId 스캔 폴백**(:850 strand — H-new-3≡H-new-1) · 락 5·§6.7.2 **issuer acquisition 보편 권위 각주**(H-new-2) · out-of-scope (C) **단계 순서 명시**(H-new-4) · 불변식 **주어 한정** + §9-ter 정직성 항목(H-new-1 부분) · §6.7.1 **quarantine 태그드 키 + 해제 (c) + 내구 세부**(M-new-1·M-new-2) · §6.7.2 **전이표 logicalKind**(M-new-3) · blocked 회귀 게이트 **:452→:529**(M-new-2) · 좌표 정정(:2408-2434 등) · §6.7.3 (i)(iv) **seq 리셋 유실 서술 철회**(L-new-3 검증 후). 버전 유지, status `pending-review`(R43c 대기 — High 4건이라 author-close 불가).
>
> **R43c 검증 → 3차 개정 (2026-07-21).** R43b 2차 개정본(커밋 `2b5a951`)을 R43c 2레인이 독립 검증했다 — claude 레인 **pending-revision**(신규 High 0·Med 1·Low 3), codex 레인 **reject**(High 1·Med 3·Low 3)(원장 [`docs/reviews/PANEDEATH-R43C.md`](reviews/PANEDEATH-R43C.md), 정본·수정 금지). 아키텍트 종합 = **pending-revision** — 신규 지적 전원이 문면·전파·표 분할이고 설계 재론 0, 수렴 High(시대 경계)도 매개변수화 명시로 닫힘. 3차 정밀화: §6.7.2 **시대 매개변수화**(전이 구조 시대 무관 · `initial` 성공형 status만 [v0.27.0]=`done`/[(C) 본체]=`needs_verification`) + **전이표 1행 분할**(성공형/failed형) · D5 CAS **`flightSideEffectOwner` 개명 + 접속 규칙**(Flight-backed=lifecycle CAS ∧ issuer acquire · Flight-less=issuer acquire 단독) · **태그드 판별 유니언 키·해제(c)** 파생 표면 전파(§6.7.1·§10·락 8·U2) + 자동-해소 fold + presence→send supersede · §6.7.3 **폴백 도달성**(성공=seq dedup 직행 · miss=`handoff_unmapped_or_stale` · 중복 매치=fail-visible drop) · §10 **seq-리셋 잔재 삭제** · 좌표 `:113` `2408-2434` · D1-b `:529` 편입. 버전 유지, status `pending-review`(**R43d 대기** — 경량 델타 검증, 잔여가 문면 전파였으므로 codex 단일 레인).
>
> **R43d 검증 → reject / 세션 종료 (2026-07-21).** R43c 3차 개정본(커밋 `cb18350`)을 R43d 단일 레인(codex-cli, reasoning=high, 경량 델타 — 사유: R43c 잔여가 전원 문면 전파)이 검증했다 — verdict **reject**, 단 9건 중 **7 resolved·2 partial**로 수렴 종반(원장 [`docs/reviews/PANEDEATH-R43D.md`](reviews/PANEDEATH-R43D.md), 정본·수정 금지). 잔여 3건은 전부 시대 태그 전파·정밀화(설계 재론 0 — 아키텍트 판정): **[High] 시대 태그 전파 미완**(PLAN D6/D8·§6.7-bis 결정 원문·§7.1 항목·Smoke 1·§10 일부가 무태그로 (C) 거동 강제 — §7.1-0과 동형 전파 필요) · **[Med] presence supersede↔자동 해제(c) 순서 모호**(`DESIGN:720-742` — 해소 순서 1문단 확정) · **[Low] PLAN `:107` 메타데이터**(무조건 `seq` → 판별 유니언 정정). 오너 지시(2026-07-21)로 이 세션은 여기서 종료하고 status를 `pending-revision`으로 둔다 — **다음 세션에서 잔여 3건 4차 반영(전부 문면·fable-advisor 자문 불요) → R43e 경량 델타(codex 단일 레인) → approve 시 종결·구현 웨이브 발주.**
>
> **4차 반영 (2026-07-21).** R43d 잔여 3건을 **문면 정정만으로 반영**(설계 재론 0 · 상태기계 무변경): **[High] 시대 태그 전파 완결** — §7.1-0과 동형의 [(C) 본체]/[v0.27.0] 표기를 PLAN D6·D8·§6.7-bis 결정 1·2·§7.1 항목 5/6a/12·Smoke 1·§10에 부착(v0.27.0=성공형 `done`·재귀 깊이 1·논리 result 1건 · (C) 본체=`needs_verification`·깊이 2·2건) · **[Med] presence supersede↔자동 해제(c) 순서 확정** — §6.7.1에 순차 규약 1문단(supersede 선행 → `"presence"` 레코드 즉시 fold, 이후 seq 레코드가 `relay_accepted`로 종결될 때 (c) 자동-해소 fold, 거짓 미해소 quarantine 항목 잔존 0) · **[Low] durable quarantine 메타데이터 판별 유니언 정정** — PLAN `:109`를 `send_unknown`=`seq` 필수·`presence_unknown`=`"presence"` 태그·`seq` 없음으로(§6.7.1·U2 정합). status `pending-revision` 유지 · **R43e 대기**(경량 델타·codex 단일 레인).
>
> **R43e approve → v0.27.0 `approved` (2026-07-21).** 4차 반영본(커밋 `65b3fb3`)을 R43e 단일 레인(codex-cli 0.144.6·gpt-5.6-sol·high·read-only)이 검증 — 1차 verdict **pending-revision**: H-new-1·M-new-1·L-new-1 **전부 resolved**이나 4차 반영이 파생한 **신규 Low 1건**(D8 라운드 북키핑 — ①–⑦이 R43e 범위로 오재지정·Review impact "R43d 필수" 잔존, `docs/PLAN.md:97,139`). Low 정정(커밋 `cabcb48` — ①–⑦=R43d까지 확인 완료 이력·R43e 범위=⑧ 한정·"R43e 필수" 동기, "R43d 필수" 전역 0건) 후 **마이크로 재확인 verdict `approve`**. **5라운드(R43→R43b→R43c→R43d→R43e) 수렴 종결 · 코드 무변경**. 원장 [`docs/reviews/PANEDEATH-R43E.md`](reviews/PANEDEATH-R43E.md), 정본·수정 금지. **다음 = v0.27.0 구현 웨이브**(구현=grok 레인·board claim §1.1 선행·불변식 유지). **Approved by:** codex 레인(R43e 마이크로 재확인) + 아키텍트 판정.

**Product one-liner:** 브릿지는 지금 카드 result를 **보내고 잊는다** — `bridge-runtime.ts:2160`의 `void sendFailedResult(...)`와 동형 결함 `:2207`·`:2222`의 `void finishCard(...)`는 전달 결과를 아무도 읽지 않고, `sendResult:2423`은 relay가 돌려준 ACK 봉투(`status` `recipientCount` `handoffId` `notified` `message`)를 **전량 폐기하고 `boolean`으로 뭉갠다**. 그 결과 발행 소유권 기제는 사실상 **`flights` 맵 삭제 하나뿐**이고(호출지점 기준 **18개**로 흩어짐 — `sendFailedResult` 7 · `finishCard` **11**; 구 "17개(7·10)"는 `finishCard` 1건 누락 — R43b 정정), 알림이 도착했는지 **호출부와 durable 회복 경로가 모른다**(프로세스-로컬 `result_delivery_unconfirmed` log/counter는 존재하나 **휘발** — 교훈 33 동형, 전칭 부정 제거 · R43b-2차 codex L-new-2). 이 MINOR는 **result 발행의 단일 소유권(dispatch-scoped result issuer) + ACK 경계 + tower currency 게이트**를 세워 (C) 하이브리드 전환이 닫지 못한 **liveness 축의 유실·고아를 관측 가능·회복 가능하게 만든다(자동 회수는 후속 PATCH — 결정 ⑧, 과장 제거)** — [`docs/spikes/PANE-DEATH-DESIGN.md`](spikes/PANE-DEATH-DESIGN.md) **§6.7**(요구사항)·**§6.7.1**(durable quarantine)·**§6.7.2**(issuer)·**§6.7.3**(tower currency 게이트)·**§6.7-bis**(함정 3건 + 아키텍트 결정 2건 확정본)·**§9-bis 락 5·8·9·13**이 설계 정본이다. **(C) 전환은 거짓 성공만 없앴을 뿐 유실을 없애지 않았다**(§9-ter — "거짓 성공 제거, 중복·유실·고아 존속"): `done` 대신 `needs_verification`을 보내더라도 **그것이 도착했는지 모르는 것은 똑같기 때문**이다.

**불변식(이 PATCH 전체를 지배):**

> 불확실한 관측은 **회복 가능한 행동만** 유발한다. 비가역 행동(result 발행 · `pane.close` · dispose)은 **결정적 증거 또는 멱등 경로에서만** 허용된다.

**Why:**
- **(C) 전환이 닫지 못한 축이 정확히 이것이다.** 설계 §9-ter가 정직하게 남긴 잔존 2건(중복 발행 경로 · F5/F6 `terminalPending` 소비와 ACK 유실 복구)은 모두 이 PATCH의 표적이다. 설계 §6.7이 못박은 대로 **"이 결함은 자동 `done`을 폐지해도 그대로 남는다."**
- **사람 확정 설계에서 알림 유실 = 카드 영구 정지.** (C) 전환 후 `done`은 사람이 워킹트리 검증 뒤에만 붙는다. 그 사람에게 도달하지 않는 알림은 카드를 `doing`에 **영구히 멈춘다**. 따라서 **durable alert + ACK 추적이 없으면 (C) 전환의 liveness는 무해해지지 않는다**(§6.7).
- **락 9는 §6.7 없이는 코드에서 성립하지 않는다.** 락 9의 "이름이 곧 불변식" 원칙을 세워도, **전달을 추적하지 않는 `void sendFailedResult(...)`와 반환값을 버리는 `await sendResult(...)`가 ACK 경계를 지운다**(§9-bis 락 9 (C) 전환 반영 각주 축자).
- **현행 `boolean`은 거짓 양성을 낸다 (코드 실독 확정).** `sendResult:2423`은 `client.handoff()`가 throw하지 않으면 무조건 `true`를 반환한다(`:2429` — R43b-2차 좌표 정정, 구 `:2437`은 스테일). 그런데 relay는 **수신자가 아예 없어도 정상 ACK를 돌려준다** — `room.ts:512-519`가 `status="peer_unknown"`·`recipientCount=0`인 `handoff.ack` 봉투를 만들고, `relay-client.ts:322-345`의 `handoff()`는 `error` 봉투일 때만 throw하므로 그 봉투는 **그대로 반환되어 `true`가 된다**. 즉 **아무도 받지 못한 result가 "전송 성공"으로 기록되고**, 현행 `finishCard:2349`는 그 `true`로 `pane.close`까지 게이팅한다.
- **관측이 비대칭으로 빠져 있다 (코드 실독 확정).** `sendFailedResult:2391`은 `sent === false`에서 `recordResultDeliveryUnconfirmed()`를 호출하지만, 대칭 위치인 `finishCard:2349`는 **호출하지 않는다** — **성공 경로의 전달 실패가 관측에서 통째로 빠져 있다**(§6.7 표).

**MINOR 근거(버전 판정):**
- **PATCH가 아닌 이유:** 이 변경은 "완료 기준 보강·보안 범위 명확화"(§Version control PATCH 정의)가 아니라 **result 발행 권위의 구조 변경**이다 — `sendResult`의 `Promise<boolean>` **시그니처 교체**(내부 계약 변경), 새 판정 상태(`result_relay_accepted` / 흡수 상태) 도입, **dispatch-scoped result issuer 신설 + seq 소유 이전(Flight→issuer; 전역 `cardSeq`는 attempt축으로 **유지** — R43b 개정, 구 "삭제"는 회귀였다)**, **tower currency 게이트 편입**(tower-local 적용 판정), 그리고 **payload 멱등키 각인**(tower inbox에 영속되므로 사후 수정 불가).
- **MAJOR가 아닌 이유:** 제품 방향·성공 정의는 불변이고, **relay wire protocol v1 · MCP 도구 · herdr RPC 표면은 전부 무변경**이다. `CardResultPayload`의 기존 필드만 쓰며 신규 필수 필드가 없다.
- **R{n} 필수 판정(`WORKFLOW.md` §5.1):** ① "새 **MINOR**" 행 → 필수. ② 완료 attestation·발행 권위는 R29/R32/R33 M-lock 인접 **신뢰 경계**다 → 필수. 설계 §8도 독립적으로 `rgate=needed`로 판정했다. **author-close 불가.**

**What (범위 — MINOR; 발행 소유권·ACK 판정·seq 규약 · **relay wire protocol v1 무변경 · conv wire 무변경 · MCP 도구 무변경 · herdr RPC 표면 무변경** — 변경은 전부 브릿지-로컬 판정과 payload 필드 값 채우기다):**

**단계 배치는 선결(D1·D2) → 본체(D3~D6) → 검증(D7·D8)이며, D1은 다른 모든 단계의 하드 선결이다** — 미실측 상수로 correctness를 닫지 않는다(lessons (30)).

| 단계 | 항목 | 내용 |
|---|------|------|
| **선결** | **D1 relay ACK 실측 — 다른 모든 단계의 하드 게이트** | strict 판정식을 켜기 전에 **실 경로가 돌려주는 값**을 먼저 고정한다. **함정 3(§6.7-bis 축자): `recipientCount=1` 조건을 실측 없이 켜지 마라 — `peer_unknown`도 정상 ACK로 반환됨이 실측 확인됐다(2026-07-21).** strict를 켜면 tower peer가 아직 붙지 않은 타이밍의 기존 통합 테스트가 `sent=false`로 뒤집혀 `pane.close`가 일어나지 않고 `pane-cleanup.test.ts:245`가 깨진다. **[코드 실독 확정]** `room.ts:452-459` `resolveTargets`는 `to !== "*"`일 때 `getPeer` → `findPeerByName` 순으로 **최대 1개**를 돌려주므로 카드 result 경로의 `recipientCount`는 **구조상 `{0,1}`**이고, `targets.length===0`이면 `peer_unknown`/`0`(`:512-519`), 그 외에는 online notify 성사 여부에 따라 `delivered`(`anyNotified`) 또는 `queued`이며 `recipientCount=targets.length`(`:553-557`)다. `handoffAckEnvelope:558-570`이 네 필드를 그대로 싣는다. **[미확인 — "없음"이 아니라 미확인]** ⓐ 실제 브릿지→타워 dispatch에서 **tower peer가 아직 join 전인 타이밍**의 값 분포 ⓑ durable/sticky relay **재시작 직후**의 값 ⓒ `to`가 peerId가 아니라 **name으로 풀릴 때**(`findPeerByName`)의 값 ⓓ `to: "*"` 경로가 result 발행에 실제로 도달하는지. **산출물:** (경로 × `status` × `recipientCount` × `notified` × `message`) 실측 표를 `docs/spikes/` 하위에 기록하고 각 행에 **실측/추정**을 표기. **완료 판정 (R43b 개정 — 결정 ⑤ · M1 해소):** 표의 ⓐ~ⓓ 4행이 실측으로 채워지고, 각 **관측 사례를 열거**한다 — 전칭 명제("정상 경로 전부에서 성립")는 **세우지 않는다**(4표본은 전칭을 세우지 못한다). 대신 **코드 불변식**으로 안전성을 무조건화한다: `room.ts:452-459` `resolveTargets`가 비-broadcast 타깃에 대해 구조상 recipient `{0,1}`을 돌려주고(정적 증명), **예상 밖 ACK는 전부 회복 가능한 unknown(`send_unknown`)으로 흡수**된다(반례가 안전을 깨지 않는다). 반례가 나오면 판정식(U1)을 재설계하고 D3 착수를 보류하되, **D1 실측의 지위는 correctness bound가 아니라 liveness/호환성 증거**다. *(결정 ①로 "진짜 미실측 상수 = D4의 10분" 문제도 소멸 — 이제 모든 상수가 가역 행동만 유발한다.)* **락:** 락 5(strict ACK 판정의 입력). |
| **선결** | **D1-b 순서 역전의 회귀 영향 실측 (아키텍트 추가 2026-07-21 — 미확인의 게이트 승격)** | 증거 팩이 정직하게 남긴 항목을 게이트로 올린다: *"순서 역전이 회귀 게이트 3개(`still-running.test.ts:452` · `pane-cleanup.test.ts:245`·`:816`)를 깨는지는 **미확인**이다. '안 깨진다'가 아니라 **확인 안 했다**."* **(R43c — claude Low-2 수용)** blocked-분기 게이트 **`still-running.test.ts:529`**(⑩)도 이 목록에 편입한다(D5·D7과 동기 — D1-b는 순서 역전 회귀 영향 실측이므로 blocked-분기 게이트도 대상; `:452`는 pane_closed 게이트로 목록에 잔존). 특히 **`pane-cleanup.test.ts:245`("exactly once")는 D3의 strict ACK로도 깨질 수 있다고 이미 경고된 바로 그 테스트**라 두 변경이 같은 테스트에 겹친다 — 어느 쪽이 깼는지 분리되지 않으면 진단이 엉킨다. **완료 판정 (R43b 재설계 — 결정 ⑥ · M2 해소):** "순서 역전만" 격리는 D5가 "셋은 쪼갤 수 없다"고 한 **의도적 무효 상태**를 만든다(Flight 노출 + CAS/latch 부재 = D5가 통제하려던 바로 그 레이스) — 따라서 그 격리에서 나온 실패는 진짜 회귀도 구 전제도 아닌 **중간 상태의 산물**일 수 있다. 재설계: **① ACK 분류를 결정적 `accepted` 스텁으로 고정**한 채 **baseline ↔ 완전한 D5 원자 집합(순서 역전+CAS+`terminalPending` latch)**을 비교하고, **② 그 다음 D3 strict ACK를 얹어 재비교**한다. 판별표 칸 = **(a) 진짜 회귀 · (b) 테스트의 구(舊) dispose 순서 전제 · (c) 중간(무효) 상태의 산물**. **이 판별 없이 D5 확정에 들어가지 않는다**(교훈 30). **락:** 없음(선결 실측) — **D5의 선결**. |
| **선결** | **D2 relay ACK 주입 표면 신설 (테스트 인프라 — 최대 과소평가 비용)** | **§6.7-bis 테스트 비용 경고 축자: `fake-herdr.ts`는 herdr만 흉내내고 relay/ACK는 흉내내지 않는다. 현행 테스트는 ACK 실패를 `relay.stop()`으로 만든다 — on/off뿐이라 `status`/`recipientCount` 조합을 만들 수 없다. strict ACK를 테스트하려면 relay 쪽 신규 주입 표면이 필요하고, 이것이 이 PATCH에서 가장 과소평가되기 쉬운 비용이다.** **산출물:** 테스트에서 `routeHandoff` 결과(또는 `handoff.ack` 봉투)를 임의 조합으로 강제할 수 있는 주입 훅 — **프로덕션 기본 경로 무변경**(주입은 테스트에서만 활성). **완료 판정:** `queued/1` · `delivered/1` · `peer_unknown/0` · `delivered/2`(다중 수신) · transport throw의 **5조합을 결정론적으로 생성 가능**함을 유닛으로 실증. **락:** 없음(테스트 인프라) — 단 **D3~D6 전부의 선결**이다. |
| **본체** | **D3 `sendResult` 시그니처 교체 + strict ACK 3분기 (이 PATCH의 진짜 blast radius)** | **함정 2(§6.7-bis 축자): strict ACK의 본체는 `sendResult` 시그니처 교체다. `Promise<boolean>`을 유지하면 strict 판정이 원리적으로 불가능하다(봉투가 이미 버려진 뒤다).** `sendResult:2408-2434`(R43b-2차 좌표 정정, 구 `:2415-2440`은 스테일)이 ACK 봉투를 보존해 반환하도록 바꾸고, 판정을 **락 5의 3분기**로 종결한다 — ① strict 성공(`status ∈ {queued, delivered}` ∧ `recipientCount=1`, **판정식 확정은 D1**) → `result_relay_accepted`(추가 발행 없음) · ② **명시적 거부** → 단일 소유자가 `failed`로 종결(로컬 발행 1회) · ③ **transport error / ACK 유실** → **흡수 상태**(두 번째 result 금지 · `pane.close` 금지 · dispose 금지). **함께 움직이는 것(§6.7-bis 축자):** `finishCard:2349`의 `sent &&` pane.close 게이팅 · `pane-cleanup.test.ts:245`("exactly once") · `:816`("relay down → no pane.close"). **락 9 정합:** ACK committed는 **relay-accept commit**이며 tower 관측·적용을 뜻하지 않는다 — phase명 `result_relay_accepted` / 시각 `relayAcceptedAt`으로 **이름에 경계를 싣는다**(락 9 축자: "경계는 산문이 아니라 이름에 실어야 한다"). **완료 판정:** `sendResult` 호출지점 전부가 봉투를 소비하고 `boolean` 반환 잔존 0건 · 3분기 각각이 D2 주입으로 결정론적 실증 · `:245`·`:816` green. **락:** 락 5(정본) · 락 9(보장 경계). |
| **본체** | **D4 failed send의 ACK — strict 1회 + 단일 흡수 상태 + durable alert (§6.7-bis 결정 1 확정본)** | **아키텍트 확정(§6.7-bis 축자, 재논증 금지):** **"failed send에도 strict ACK를 1회 적용한다. 비수락 결과는 유실·명시 거절을 구분하지 않고 단일 흡수 상태 **`send_unknown`**(§6.0.2)으로 보내고 durable alert(§6.7 요구 3)를 낸다."** **전제 교정 축자:** *"'A는 무한 후퇴(ACK의 ACK) 위험'이라는 반대 논거는 성립하지 않는다. `send_unknown`은 **흡수 상태**다 — failed send의 ACK가 유실되면 거기로 들어가고, 그 상태에서는 ACK를 요구하는 send가 더 이상 없다. 재귀 깊이는 구조적으로 2에서 끝난다(`needs_verification` → `failed` → 흡수). ACK 판정은 send 단위이고 send 횟수가 유한하다."* **[시대 표기 (R43c — codex H-new-1 ≡ claude Med-1)]:** 이 축자 예시의 `needs_verification → failed → 흡수`는 **[(C) 본체] 행**이다(§6.7.2 시대 매개변수화) — **[v0.27.0 / pre-C]**에서는 성공형 `initial`이 현행 `done`을 발행해 거절 대상이 없어 재귀 깊이 1이고, 흡수 상태(`send_unknown`)·진입 시점 durable quarantine 구조는 **시대 무관 동일**하다(D4가 다루는 failed send의 ACK·흡수·재알림은 pre-C에서도 그대로 성립). **이 선택을 결정지은 리스크(축자):** *"구현 B는 ACK 유실이라는 불확실한 관측에서 latch 해제 + dispose라는 비가역 행동을 실행한다 — 이 트랙의 불변식을 문면 그대로 위반한다. 게다가 그 경우 카드가 알림 없이 `doing`에 잔존한다. A의 잔존 리스크(카드 정지)도 실재하지만 durable alert로 운영자에게 보이므로 '조용히 멈추지 않는다'가 유지된다. 불변식이 이미 답을 정해둔 분기였다."* **durable alert가 필수인 이유(§6.7 축자):** 사람이 확정해야 done이 되는 설계에서 **사람에게 도달하지 않는 알림은 카드를 영구히 멈춘다**. **보존·해제 (R43b 개정 — 결정 ①② · U3 해소):** 흡수 상태를 **출구 없이 두지 않되**, 출구는 **dispose가 아니다.** unknown 상태 **진입 시점**에 **durable quarantine 레코드 1건**(§6.7.1 — durable alert와 동일 1건)을 기록하고, **10분 타이머는 재알림(re-escalate)만** 유발한다(가역 행동). in-memory 레코드 해제는 **(a) 운영자 ack 또는 (b) 프로세스 종료(로그 필수)**뿐이다 — durable 레코드가 진입 시점부터 디스크에 존재하므로 해제가 정보를 파괴하지 않는다. **락 8의 구 "만료 시 dispose"는 삭제됐다**(같은 락 본문·§6.6·락 13과 자기모순이었다 — R43 H2). *이로써 U3의 "타이머 비가역 dispose" 긴장 자체가 소멸한다 — 이제 흡수 상태가 유발하는 행동은 재알림뿐이다.* **완료 판정 (R43b 개정):** failed send 경로가 strict 판정을 **정확히 1회** 통과하고, 비수락 시 흡수 상태 진입과 **동시에 durable quarantine 레코드 1건**(§6.7.1)이 남으며, 그 상태에서 발생하는 추가 send가 **0건**이고, **10분 만료 시 재알림만 발화(dispose 0건)**이며, in-memory 해제가 **운영자 ack 또는 프로세스 종료에서만** 일어남을 실증. durable alert 매체는 브릿지-로컬 JSONL이고 대칭 위치 `recordResultDeliveryUnconfirmed`(휘발) 재사용 0건(결정 ⑨). **락:** 락 5 · 락 8 · 락 13. |
| **본체** | **D5 단일 소유권 = CAS/latch — `void`→`await` 단순 승격 금지 + 관측 누락 보강** | **함정 1(§6.7-bis 축자): `void` → `await` 단순 승격 금지.** *"`:2160`을 `await`로 바꾸면 `onCardHerdrEvent`가 async가 된다. 그런데 호출자 `onHerdrEvent:1936`은 동기 `void` 함수이고 herdr 이벤트 디스패치의 핫패스다. 현행 이중 발행 방어(`flights.get(paneId) !== flight` 가드 **5곳** — `:2003 :2019 :2067 :2080 :2096`; `:2019`가 `beginCardCompletion` 본류의 유일 방어 · R43b 정정)는 '이벤트 핸들러가 동기적으로 맵을 갱신한다'는 가정에 기대고 있다. 이벤트 처리를 직렬화하면 그 가정이 깨져 **방어가 오히려 약화된다.** 단일 소유권은 `await` 승격이 아니라 CAS/latch로 얻어야 한다."* **대상 좌표(§6.7 실측 정본 표 — 좌표 재실측 2026-07-21):** `:2160` `void sendFailedResult({...})`(`pane_closed` 분기, **미해소**) · **`:2207`·`:2222` `void finishCard(flight, "failed", …)`(미해소 · 신규 발견 — 동형 결함 2건, `:2160`만 고치면 반쪽이 된다)** · `:2349` `finishCard` 말미(**부분 해소** — `sent`가 `pane.close` 게이팅에만 쓰이고 `sent === false`에서 `recordResultDeliveryUnconfirmed()`를 **호출하지 않는다**; 대칭 위치 `sendFailedResult:2391`은 호출한다 → **성공 경로의 전달 실패 관측 누락 보강**). **⭐ D5의 원자 단위 = 3종 세트 (아키텍트 확정 2026-07-21 · 증거 팩 실측):** ① **`dispose` → `send` 순서 역전** ② **Flight 부작용 소유권 불리언(`flightSideEffectOwner` 상당 — R43c codex M-new-1: "발행 소유권"이 아니라 **Flight-backed 부작용 소유권**이다; 발행 권위 자체는 issuer acquisition이 갖는다 — §6.7.2·락 5)** ③ **`terminalPending` 레코드**. **셋은 쪼갤 수 없다.** 근거 = **순서 역전 없이 latch만 얹으면 그 latch는 영원히 발화하지 않는다** — 현행은 모든 완료 호출지점이 `disposeCardFlight` → 발행 순이라 `result_sending` 중의 terminal 이벤트가 `onHerdrEvent:1942`의 `flights.get(paneId)`에서 걸러져 **핸들러에 도달조차 못 한다**. **⭐ 순서 역전 대상 = `disposeCardFlight` 호출지점 15곳 전부 (아키텍트 직접 grep 재실측 확정 2026-07-21 — 16행 − 정의 1행 `:1983`):** ▸ **(a) 완료 판정 경로 6곳** `:2006 :2012 :2027 :2033`(`beginCardCompletion`) · `:2205`(`blocked`→failed) · **`:2219`(`done`/`idle`의 `shouldCorrectCompletionToBlocked`→failed)** · ▸ **(b) 실패 4곳** `:1244 :1268 :1908`(dispatch/pre-completion) · `:2158`(`pane_closed`) · ▸ **(c) still-running poll 5곳** `:2085 :2104 :2111 :2127 :2134`. **부분 역전 시 `terminalPending` latch가 일부 경로에서만 발화하는 죽은 코드**가 된다 — 15곳 전부를 역전해야 한다. **`disposeCardFlight` 분할 명시:** 타이머/리스너 정리(**조기** — leak 방지)와 `flights.delete`(**지연** — send 뒤)를 분리한다(§6.1 union 밖 별도 필드와 동형). 설계 §5 B′ 축자: *"핵심은 active `Flight`를 result send 전에 삭제하지 않는 것이다."* 락 5 근거 축자: *"권고 B는 `sendResult` 완료까지 flight를 살려두므로 `pane_closed` 핸들러가 **이미 커밋된 카드에도 발화한다.** 이것이 이 PATCH의 핵심 테스트다."* **구현자 경고: latch 필드만 추가하고 순서를 그대로 두면 유닛은 통과하는데 프로덕션에서 죽은 코드가 된다**(설계 §6.7-bis가 진단한 현행 증상을 그대로 재생산). **phase union(§6.1 12원소)은 D5에 넣지 않는다** — `terminalPending`은 설계가 **의도적으로 union 밖에 둔 별도 필드**이고(`§6.1` 축자 주석 *"phase가 아니다 — `result_sending`과 공존한다"*, §10 체크리스트도 재확인), D5가 실제로 필요로 하는 것은 **불리언 1 + latch 레코드**뿐이다(현행 `Flight`에 phase 계열 필드 **0개** — 실측). **마이그레이션 부담 없음:** §6.1 축자 *"별도 relay wire 필드는 필요 없다. `Flight` 내부 필드 또는 bridge-local registry다"* — 프로세스 수명 인메모리이므로 나중에 union을 도입해도 스키마 변경이 아니다. **비가역 각인 경보는 D5가 아니라 D6(seq 멱등키)에 걸린다.** **선례:** `stillRunningDeferral`(`:369-374` 주석 *"Set synchronously before any settle await so concurrent done/idle events are no-ops while this path owns the completion decision"*)이 이미 union 없이 불리언으로 소유권을 잡는 동형 패턴으로 검증돼 돌아가고 있다. **`terminalPending` latch:** `result_sending` 중 도착한 terminal은 **경쟁 failed를 보내지 않고 latch만** 하며, ACK 판정이 3분기로 종결한다(락 5). **3차 리뷰 배관 서술 정정 반영(§6.7-bis 축자):** `PANEDEATH-CODEX-REVIEW3.md:43`의 *"`finishCard`가 await하는 동안 terminal 핸들러가 독립적으로 failed를 쏠 수 있다"*는 **현행 코드에서 성립하지 않는다** — 모든 완료 호출지점이 `disposeCardFlight` → `finishCard` 순이라 카드는 이미 `flights`에서 사라져 있다. **실제 증상은 "done + failed 2건"이 아니라 `result_sending` 중 terminal 증거의 완전 소실**이다. **결론(락 5 필요)은 유지되고 배관 서술만 정정된다**(lessons (33) 적용례 — 검증자 지적은 결함 **존재**의 증거일 뿐 **서술**의 증거가 아니다). **완료 판정 (R43b 개정 — 결정 ④ · M1 해소):** fire-and-forget 3곳 잔존 0건 · **가드 5곳(`:2003 :2019 :2067 :2080 :2096`) 각각이 CAS 소유권 불리언을 함께 검사**함을 어서션으로 잠금(순서 역전 후에는 send 중에도 `flights.get(paneId) === flight`가 성립해 가드의 보호 효력이 정의상 소멸하므로 — CAS 없이 통과 가능한 판정은 판정이 아니다) · **접속 규칙 어서션 (R43c — codex M-new-1):** Flight-backed send = lifecycle CAS(`flightSideEffectOwner`) **그리고** issuer acquire 둘 다 통과 · Flight-less send(`:850 :1114 :1204`) = **issuer acquire 단독** — **lifecycle CAS를 발행 권위로 단독 사용하는 경로 0건**(§6.7.2 issuer acquisition = 보편 발행 권위 · 락 5). 이 접속 규칙을 D5·D7 어서션으로 잠근다 · **`blocked` 분기(`:2202-2208`)에 latch 검사 추가**(현재 `stillRunningDeferral` 가드 없음; done/idle latch는 `:2214`·`:2227`에만 있다) · `finishCard` 실패 경로 관측 1건 · **blocked-분기 회귀 게이트 `still-running.test.ts:529`("⑩ M-1: blocked during deferral → timer cleanup + failed once" — `agent_status:"blocked"` + `/agent_blocked/` single-result assert)**(순서 역전 시 `blocked` 분기에서 두 번째 result가 나가는지 — D1-b·D7에 명시 편입). **R43b-2차 정정 (claude M-new-2):** 구 인용 `:452`는 실제로 "⑦ pane_closed during deferral" 테스트라 blocked-분기 회귀의 게이트가 아니다 — blocked 게이트는 `:529`(⑩)이고 `:452`는 pane_closed 게이트로 별도 유지한다(둘 다 D7 회귀 목록에). **락:** 락 5. |
| **본체** | **D6 issuer 중앙화 + seq 소유 이전 + 멱등키 각인 + tower currency 게이트 (§6.7-bis 결정 2 · §6.7.2·§6.7.3 · R43b 개정)** | **아키텍트 확정(§6.7-bis 축자, 재논증 금지) — 제시된 두 안 모두 기각:** *"전제 교정: '`flight.seq`냐 `cardSeq`냐'로 물었으나 **둘 다 틀렸다.** `flight.seq`(스폰 고정)는 `dispatchHandoffId`와 **역할이 중복**이고, `cardSeq`(send마다 증가)는 **재전송에 새 키를 부여해 멱등키를 스스로 무효화한다.**"* **R43b 전제 교정 2건(재논증 금지 · R43 전제 P·Q):** **(P)** `sendFailedResult` 7경로 중 **Flight-less 3경로**(`:850`(payload_invalid — cardId가 `extractCardIdLoose(...) ?? "task_0"`로 **열화**, 단 `h.id`는 신뢰 가용) · `:1114`(agent_kind_not_allowed) · `:1204`(herdr_spawn_failed))가 있어 **Flight는 seq 소유 주체가 될 수 없다.** 시그니처가 Flight 미수령(`:2364-2371`). **(Q)** 전역 `cardSeq`(`:575`)는 result 시퀀스가 아니라 **per-card dispatch attempt 카운터**다(`:1124-1127` 주석 "Fix 2 (live-measured)"). 소비처 **4곳** = hook 소켓 경로(`:1141`) · herdr agent name 유일성(`:1192`) · `Flight.seq`(`:1220`) · `sendFailedResult` 채번(`:2372-2373`). **삭제하면 agent name·소켓 경로 충돌 = live-measured 수정의 회귀** → **"삭제" 요구를 제거하고 유지로 바꾼다.** **채택 규약(R43b 개정 — 소유 주체 Flight→issuer):** seq는 **dispatch-scoped result issuer가 소유**한다(§6.7.2). **result payload를 새로 구성할 때 1회 증가**하고 **재전송은 같은 seq를 재사용**한다(채번 규약 자체는 §6.7-bis 결정 2 그대로, **소유 주체만 개정** — 근거 = 전제 P). issuer는 handoff **수신 시점**에 `(cardId, dispatchHandoffId)` 키로 생성되어 **성공·실패·Flight-less 3경로 전부의 유일 통과점**이다. `sendFailedResult`의 `cardSeq` 채번(`:2372-2373`)만 issuer 채번으로 대체하고, **`cardSeq`는 attempt축 소비 3곳(`:1141 :1192 :1220`)만 남긴 채 유지**한다. **`(cardId, dispatchHandoffId)`만으로는 부족하다(축자):** *"결정 1의 구조상 한 dispatch 안에 서로 다른 **논리 result 2건**(거절된 `needs_verification`, 후속 `failed`)이 **합법적으로** 존재한다. 이 둘을 한 키로 뭉개면 relay의 **미실측 dedup 의미론에 correctness가 얹힌다** — 교훈 (30) 위반형이다."* **[시대 표기 (R43d — codex H-new-1)]:** 이 "논리 result 2건"(거절된 `needs_verification` + 후속 `failed`)은 **[(C) 본체]** 구조다(§6.7.2 시대 매개변수화) — **[v0.27.0 / pre-C]**에서는 성공형 `initial`이 `done`이라 거절·후속 failed가 없어 논리 result가 **1건**이다. 단 `(cardId, dispatchHandoffId, seq)` **각인 자체는 시대 무관 이번 PATCH 의무**다(payload는 tower inbox 영속이라 사후 정정 불가) — pre-C에서 논리 result가 1건이어도 seq는 payload 구성 시점에 각인된다. **범위 경계 (R43b 개정 — 결정 ③ · 락 9 범위 재획정):** payload에 `(cardId, dispatchHandoffId, seq)`를 **각인**하는 것은 이번 PATCH 의무다 — payload는 **tower inbox에 영속되므로 나중에 고칠 수 없다.** **더해, tower currency 게이트(§6.7.3)도 이번 PATCH에 편입**한다: `applyCardResult`에서 `payload.dispatchHandoffId` ↔ `task.handoffId`(영속본)를 대조해 **불일치=stale attempt 늦은 result로 drop(관측 로그+counter, 조용한 삼킴 금지) · 일치=per-dispatch seq dedup · 부재=현행 스칼라 폴백**. **태스크 해석 폴백(R43b-2차 — codex H-new-3 ≡ claude H-new-1):** 게이트보다 **먼저**, `findTask(payload.cardId)` miss(또는 `:850` 열화값 `"task_0"`) 시 **`task.handoffId === payload.dispatchHandoffId` 스캔 폴백**으로 태스크를 해석하고 해석 후 payload cardId를 **교차 검증**(불일치=drop+로그)한다 — 이 폴백이 없으면 `:850` result가 relay 수락되고도 `findTask` miss로 **조용히 strand**되어 §6.7.2 "유일 통과점"이 타워측에서 깨진다. 근거: (i) R43 H3 — 각인 대상(payload)에 **이미 현행 스칼라 판정자**(`card-ops.ts:190-197`, `payload.seq <= last`만 보고 `dispatchHandoffId`는 안 봄)가 붙어 있어 **각인과 판정자는 분리 불가**(각인은 tower inbox로 영속되므로 비가역) (ii) 순수 튜플 dedup은 현행 스칼라가 attempt-교차 단조성으로 우연히 막던 **stale-attempt 늦은 result 수용**을 새로 열어(fable-advisor 보정) currency 게이트가 **필요하다 — 이것이 load-bearing 근거다** (iii) tower dedup은 **relay 표면이 아니라 tower-local 판정**(`card-ops.ts` — host)이고 비교 값 2개(`payload.dispatchHandoffId`·`task.handoffId`)가 **이미 같은 함수 스코프**에 존재해 새 저장소·전송 불요. *(R43b-2차 정정 — claude L-new-3 검증 후 수용: 초판 근거의 "issuer-local seq는 재dispatch마다 재시작해 스칼라 게이트에 삼켜진다"와 부수 이득 "브릿지 재시작 seq 리셋 유실 해소"는 **둘 다 실측과 어긋나 철회**한다. board 재dispatch는 `dispatchCard`(`card-ops.ts:112-117`)가 `notes`를 `dispatched node=…`로 통째로 덮어써 `last_seq=` 토큰을 제거하므로 재dispatch 후 새 attempt seq는 스칼라 게이트에 **삼켜지지 않고**, 순수 "브릿지 재시작 후 재전송"은 active binding 복원이 §9-6 out-of-scope라 발생하지 않는다.)* **별도 PATCH로 남는 것은 relay/wire idempotency와 inbox reconcile뿐**이다. **완료 판정 (R43b 개정):** issuer가 seq를 소유하고 `cardSeq`의 **발행 채번 소비 0건**(attempt축 3곳 `:1141 :1192 :1220`만 잔존 — 삭제 0건) · payload 구성 1회당 seq 1 증가 · **재전송 시 seq 동일**을 실증 · `(cardId, seq, dispatchHandoffId)` 3필드가 모든 발행 payload에 각인됨 · **tower currency 게이트가 stale attempt 늦은 result를 drop(관측 로그+counter)하고 필드 부재 시 스칼라 폴백**함을 실증. **락:** 락 9(범위 판정·보장 경계) · 락 5(issuer 소유). |
| **검증** | **D7 테스트 계획 + 회귀 게이트 명기** | **신규 유닛/통합:** ① D2 주입으로 strict ACK **5조합** 각각의 종결 분기 · ② 3분기 종결(성공/명시적 거부/흡수 상태) · ③ `result_sending` 중 terminal → **`terminalPending` latch 1건 · 그 자리에서 failed 발행 0건** · ④ 흡수 상태에서 **두 번째 result 0건 · `pane.close` 0건 · 타이머 경로 dispose 0건**(결정 ② — 만료는 재알림만; 운영자 ack발 해제·프로세스 종료는 dispose 아님, 별개 경로) · ⑤ unknown 진입 시점 **durable quarantine 레코드 1건**(= durable alert, 동일 1건)이 남고 카드가 조용히 `doing`에 잔존하지 않음; `recordResultDeliveryUnconfirmed`(휘발) 재사용 0건(§7.1-13) · ⑥ 재전송 seq 동일성(issuer 소유) · ⑦ `cardSeq` **발행 채번 소비 0건**(attempt축 3곳 유지 — 삭제 아님) + **issuer가 Flight-less 3경로(`:850 :1114 :1204`) 포함 모든 발행의 유일 통과점** · ⑧ fire-and-forget 3곳(`:2160`·`:2207`·`:2222`) 잔존 0건 · ⑨ **tower currency 게이트** — stale `dispatchHandoffId`의 늦은 result drop(관측 로그+counter) · 일치 시 seq dedup · 필드 부재 시 스칼라 폴백 · **`findTask` miss(`:850` 열화 `"task_0"`) 시 `dispatchHandoffId` 스캔 폴백으로 태스크 해석 + cardId 교차 검증(불일치=drop) → 카드가 strand되지 않음**(R43b-2차 — H-new-3 ≡ H-new-1). *(구 "브릿지 재시작 seq 리셋 유실 미재현" 항목은 L-new-3 검증으로 철회 — board 재dispatch는 notes clobber로 last_seq를 제거하고 재시작 재전송은 §9-6 out-of-scope다.)* **회귀 게이트(반드시 green — 명시 지정):** **`still-running.test.ts:529`("⑩ M-1: blocked during deferral → timer cleanup + failed once")** — **blocked-분기 순서 역전 회귀의 진짜 게이트**(R43b-2차 M-new-2 정정; 구 인용 `:452`는 pane_closed 테스트였다) · **`still-running.test.ts:452`("⑦ pane_closed during deferral → timer cleanup + failed once")** — **현행 이중 발행 방어의 유일한 실증**이므로 D5의 CAS/latch 재설계가 이 방어를 약화시키지 않았음의 1급 증거다 · **`pane-cleanup.test.ts:245`("① no-indicator immediate done → sendResult then pane.close exactly once")** · **`pane-cleanup.test.ts:816`("⑦ sendResult fails (relay down) → no pane.close")**. **전역 게이트:** 전 패키지 typecheck 6/6 · **전체 스위트 차집합 0**(아키텍트 독립 실행 SSOT — lessons (18); "pass 수 동일"이 아니라 **직전 기준 대비 차집합 0**으로 판정). **테스트 비용 재경고:** D2 주입 표면이 없으면 ①②④는 **작성 자체가 불가능**하다 — 이 항목의 공수를 D3 이후로 미루면 검증이 통째로 미뤄진다. |
| **검증** | **D8 게이트 = v0.27.0 MINOR · R43e 필수(경량 델타 — R43d 잔여 3건 문면 반영 검증) (author-close 불가)** | Status `pending-review` → R43b 2레인 검증(pending-revision 채택) → 2차 개정 → **R43c 2레인 검증(pending-revision 채택 · 신규 High 0)** → 3차 개정 → **R43d(codex 단일 레인·경량 델타) reject — 9건 중 7 resolved·잔여 3건 전부 문면**(원장 [`docs/reviews/PANEDEATH-R43D.md`](reviews/PANEDEATH-R43D.md)) → **4차 반영 → R43e approve**(1차 pending-revision Low 1[D8 북키핑] → `cabcb48` 정정 → 마이크로 재확인 approve · 원장 [`docs/reviews/PANEDEATH-R43E.md`](reviews/PANEDEATH-R43E.md)). **경량 아님** — 발행 권위·ACK 경계는 R29/R32/R33 M-lock 인접 신뢰 경계이고 설계 §8이 독립적으로 `rgate=needed`로 판정했다. **락 후보(무변경 명시):** relay wire protocol v1 무변경 · conv wire 무변경 · MCP 도구 무변경 · herdr RPC 표면 무변경 — 바뀌는 것은 **브릿지-로컬 판정 + tower-local 적용 판정(currency 게이트) + payload 기존 필드의 값**뿐이다. **R43d까지 확인 완료 이력(①–⑦ — 재검증 불요):** ① D1 실측 표가 strict 판정식을 정당화하는가(전칭 명제 제거 + 코드 불변식으로 안전성 무조건화됐는가) ② 흡수 상태의 재귀 깊이 2(**[(C) 본체]** · [v0.27.0]는 깊이 1·성공형 `done` — §6.7.2) + **issuer 유한 전이 집합**이 실제 코드 경로(Flight-less 3경로 포함)에서 성립하는가 ③ seq 규약이 "한 dispatch에 논리 result 2건" 구조와 정합하고(단 "논리 result 2건"은 **[(C) 본체]** · [v0.27.0]는 1건 — §6.7.2) **tower currency 게이트**가 stale attempt를 조용히 삼키지 않는가 ④ D5의 CAS/latch가 **가드 5곳**(`:2019` 포함)과 **blocked 분기 latch**를 모두 검사하며 **순서 역전 대상 15곳 전부**를 덮는가 ⑤ payload 각인 + **durable quarantine JSONL(진입 시점)**이 **영속 표면**이라는 비가역성을 범위 경계가 올바로 반영했는가(만료 dispose 삭제·reconcile 회수 out-of-scope 명시 포함) ⑥ **(R43b-2차 신규)** §6.7.3 dispatchHandoffId 스캔 폴백이 `:850` strand를 실제로 닫는가(타워측 유일 통과점) · issuer acquisition 보편 권위가 Flight-less 3경로의 락 5 sole-owner를 성립시키는가 · 전이표 logicalKind가 "논리 result 2건"을 initial/rejection_escalation으로 정확히 가르는가 · quarantine 태그드 키·내구 세부(0600·fsync·replay·ack fold)와 presence_unknown 자동 해제 (c)가 §6.0.2와 정합하는가. ⑦ **(R43c 3차 신규)** §6.7.2 **시대 매개변수화**가 전이 구조를 시대 무관으로 두고 `initial` 성공형 status만 [v0.27.0]=`done`/[(C) 본체]=`needs_verification`로 가르는가 — §7.1-0 "done 0건"이 (C) 본체 게이트로 명시돼 pre-C v0.27.0(현행 `done` 발행)과 충돌하지 않는가 · **전이표 1행 분할**(성공형/failed형)이 pre-Flight 직행과 정합하는가 · D5 **`flightSideEffectOwner` 개명 + 접속 규칙**이 lifecycle CAS 단독 발행 경로를 제거했는가 · **태그드 판별 유니언 키·해제(c)**가 파생 표면 전부(§6.7.1·§10·락 8·U2)에 전파됐는가 · §6.7.3 **폴백 도달성**(성공=seq dedup 직행·miss=`handoff_unmapped_or_stale`)이 정확한가. **R43e가 최소 확인해야 할 것(경량 델타 — 범위 = ⑧뿐):** ⑧ **(R43d 잔여 3건 4차 반영)** 시대 태그 전파 6표면(PLAN D6/D8·§6.7-bis 결정 1·2·§7.1 항목 5/6a/12·Smoke 1·§10)·presence supersede 선행→`relay_accepted` fold 순차 규약(§6.7.1)·durable quarantine 판별 유니언(PLAN `:109`)이 원장 [`docs/reviews/PANEDEATH-R43D.md`](reviews/PANEDEATH-R43D.md)의 지적(H-new-1·M-new-1·L-new-1)을 정확히 닫는가. VERSION은 0.26.1 → **0.27.0** 동기 갱신 2곳(CLI `VERSION` `index.ts` + MCP serverInfo `mcp-server/src/stdio.ts`) — 0.23.7 미갱신 사고 선례. |

**Out of scope (이 버전 아님):**
- **relay/wire idempotency 판정 + inbox reconcile** — **락 9의 범위 분리(R43b 재획정 — 결정 ③).** 이번 PATCH는 payload 각인 + **tower currency 게이트(§6.7.3 — `card-ops.ts`의 tower-local 판정이라 편입)**까지 하고, **relay 표면 변경이 필요한 wire idempotency와 inbox reconcile만 별도 PATCH**로 남긴다. **wire exactly-once를 주장하지 않는다** — 최대 보장은 *"committed 판정이 거짓 양성을 내지 않는다 + tower가 stale attempt의 늦은 result를 조용히 삼키지 않는다"*까지이며, 문서·테스트가 말하는 **"로컬 발행 1회"(local single-issue)** 는 wire exactly-once와 **별개 어휘**다(락 9 축자).
- **§6.7 요구 4 = 자동 reconcile 회수** — **명시적 out-of-scope (R43b — 결정 ⑧ · R43 M5).** 이 PATCH는 유실·고아를 **관측 가능·회복 가능**하게 만들 뿐(durable quarantine + currency 게이트), **자동 회수는 하지 않는다.** one-liner의 "liveness 축을 닫는다"는 과장이었고 제거했다.
- **conv 경로의 동일 ACK 폐기 — card-only 경계 명시 (R43b 전수 열거 · 결정 ⑦ · R43 M3·M4).** 이번 범위는 **card 경로**이고 0.27.0은 **card-only 경계**를 명시적으로 주장한다. conv fire-and-forget/ACK-폐기 전수 목록(현행 트리 실측): **▸ 헬퍼 내부 ACK 봉투 폐기 3곳** — `:1300`(`sendConvAccept`) · `:1319`(`sendConvReject`) · `:1777`(`sendWorkerTurnFromPane`) · **▸ `void sendWorkerTurnFromPane(...)` 호출지점 4곳** — `:1534`(events_subscribe_failed) · `:2255`(pane_closed) · `:2274`(agent_blocked) · `:2279`(auto) · **▸ `await`이나 여전히 ACK-폐기 1곳** — `:1930`(inject_unconfirmed). **M4 비가역 상태 전진(무겁다):** `sendWorkerTurnFromPane`(`:1777-1797`)은 `client.handoff` 뒤 **비-throw ACK면(=`peer_unknown` 포함)** `lastOwnSeq`(`:1784`) · `emittedArtifacts`(`:1789-1791`) · `deltaAnchor`(`:1796`)를 **무조건 전진**시킨다 — 거짓 양성 위에서 **conv turn이 영구 유실**될 수 있다. **conv는 U4의 named follow-up으로 유지**하며 **미해소로 남는다는 사실을 명기한다**(무결점 주장 금지).
- **(C) 전환 본체** — 자동 `done` 폐지(락 11) · phase registry · lifecycle `generation`(락 8) · bounded reconcile · `send_unknown`/`presence_unknown` 분리 구현(락 13). 이 PATCH는 그 전환의 **liveness 축만** 담당하며 correctness 축은 별도 PATCH다. **단계 순서 명시 (R43b-2차 — codex H-new-4 해소):** v0.27.0은 **pre-C 현행 완료 경로**(자동 `done` · `sent && pane.close`) **위에** 구현된다. 락 11의 자동 close 금지와 §7.1-0의 `done` 0건은 **(C) 본체 PATCH의 수용 기준**이며, 그 PATCH가 `pane-cleanup.test.ts:245`("exactly once")의 기대를 교체한다. **v0.27.0의 `:245` green 요구는 pre-C 현행 계약의 회귀 게이트**다 — 즉 락 11 auto-close 금지와 `:245` green은 **서로 다른 PATCH의 수용 계약**이라 동시 충족을 한 구현에 요구하지 않는다(H-new-4가 지적한 "동시 배제·요구" 모순은 두 계약이 다른 PATCH 소속임을 명시함으로써 해소된다).
- **bridge restart 후 active binding 복원** — 설계 §9-6에서 **out-of-scope 확정**(브릿지가 재시작되면 tombstone이 방어하려던 이벤트 경로 자체가 사라진다 — fable-advisor 자문 2026-07-20). "나중에 풀 숙제"가 아니라 **설계 범위 밖**이다.
- **`sendResult` 판정을 hook·screen 센서와 결합** — strict ACK가 `A+`를 갖는 명제는 **P4 하나뿐**이고 P7(의미상 완료)에 대해서는 `N`이다(§6.0-bis 증거 명제표). 이 PATCH는 P4만 다룬다.

**Security / trust (R43b 판단 대상):**
- **신뢰 경계 자체는 무변경.** M-1 dispatch allowlist · L-2 `node`/assignee 대조 · pin 검사 전부 무변경이며, 이 PATCH는 **이미 인가된 카드의 result가 도달했는지를 추적**할 뿐이다. 권한 확대 없음. **tower currency 게이트**는 오히려 **stale attempt의 늦은 result가 최신 dispatch의 상태를 덮는 것을 막아** 무결성을 좁히는 방향이다(`dispatchHandoffId`는 relay 서버 생성이라 워커가 위조 불가).
- **경계가 **좁아지는** 방향이다.** 현행 `boolean`은 `peer_unknown`(수신자 0)을 "성공"으로 읽고 그 위에서 `pane.close`까지 수행한다 — strict 판정은 이 **거짓 양성을 제거**한다. 되돌리면 오늘의 오탐이 부활한다.
- **payload 각인은 영속 표면이다.** `(cardId, seq, dispatchHandoffId)`는 tower inbox에 **영속**되므로 사후 정정이 불가능하다. 각인 규약 오류는 **PATCH로 회수되지 않는다** — R43이 반드시 확인해야 할 비가역 지점.
- **durable quarantine의 내용 경계:** 레코드(= durable alert, §6.7.1)는 **메타데이터만** 싣고 워커 산출물 본문을 싣지 않는다(0.26.0 D6 계측 레코드 규약과 동형 — 본문성 §5.1 원칙 · 간접 프롬프트 주입 표면 차단). 공통 필드는 `cardId · dispatchHandoffId · 상태 · 시각 · 사유 · 카운터`이고 **키 필드는 상태로 판별하는 유니언**이다(§6.7.1 · U2): `send_unknown`은 `seq`를 **필수**로 싣고(payload 각인 튜플과 동일), `presence_unknown`은 result payload를 구성한 적이 없어 `seq`가 없으므로 `seq` 대신 **`"presence"` 태그**를 싣는다(무태그 `seq`로 두 상태를 뭉개지 않는다). 매체는 **브릿지-로컬 append-only JSONL**(프로필 스코프)이며, board(tower-local)나 tower 통지(의심 채널)에 의존하지 않는다. 대칭 위치 `recordResultDeliveryUnconfirmed`(휘발) 재사용 금지(결정 ⑨).

**추정 / 실측 구분 (lessons (30) — 미실측을 "없음"으로 적지 않는다):**

| 명제 | 지위 |
|---|---|
| 카드 result 경로의 `recipientCount`는 구조상 `{0,1}` | **코드 실독 확정**(`room.ts:452-459`) — 단 D1이 실경로로 재확인한다 |
| `peer_unknown`이 정상 ACK로 반환된다 | **실측 확정**(2026-07-21, §6.7-bis 함정 3) |
| `sendResult`가 `peer_unknown`에도 `true`를 반환한다 | **코드 실독 확정**(`bridge-runtime.ts:2408-2434`, handoff 호출은 `:2423-2428` + `relay-client.ts:336-344` · R43c codex L-new-2 좌표 정정, 구 `:2423-2440`은 스테일) |
| tower peer join 전 타이밍의 실제 ACK 값 분포 | **미확인** — D1 산출물 |
| durable/sticky relay 재시작 직후의 ACK 값 | **미확인** — D1 산출물 |
| `findPeerByName` 경로의 ACK 값 | **미확인** — D1 산출물 |
| relay의 dedup 의미론 | **미실측** — 그래서 correctness를 여기에 얹지 않는다(§6.7-bis 결정 2 축자) |
| D2 주입 표면의 실제 공수 | **미확인** — 설계가 "가장 과소평가되기 쉬운 비용"으로 경고 |
| 전역 `cardSeq`는 result 시퀀스가 아니라 per-card **dispatch attempt 카운터**다 | **코드 실독 확정**(`:575` 선언 · `:1124-1127` 주석 "Fix 2 (live-measured)" · 소비처 4곳 `:1141 :1192 :1220 :2372-2373`) — 삭제 시 회귀 |
| `sendFailedResult` 7경로 중 **Flight-less 3경로** 존재 | **코드 실독 확정**(`:850`(cardId 열화 → "task_0") · `:1114` · `:1204`; 시그니처 Flight 미수령 `:2364-2371`) |
| `disposeCardFlight` **호출지점 = 15곳**(순서 역전 대상) | **아키텍트 직접 재실측 확정(2026-07-21)** — `grep -c` 16행 − 정의 1행(`:1983`); `1244 1268 1908 2006 2012 2027 2033 2085 2104 2111 2127 2134 2158 2205 2219`(`:2219` 포함). 분류 (a)완료 6 · (b)실패 4 · (c)poll 5 |
| 이중 발행 방어 **가드 = 5곳** | **코드 실측**(`:2003 :2019 :2067 :2080 :2096`; `:2019`가 `beginCardCompletion` 본류) — 구 "4곳"은 `:2019` 누락 |
| tower가 `dispatchHandoffId`를 이미 안다(비교 값 2개 동일 스코프) | **코드 실독 확정**(`card-ops.ts:112-116` `task.handoffId` 영속 · `room.ts:442` 서버 생성 · `applyCardResult`가 `findTask`로 로드) |
| 발행 호출지점 **18개**(`sendFailedResult` 7 · `finishCard` 11) | **코드 실측** — 구 "17개(7·10)"는 `finishCard` 1건 누락 |
| board 재dispatch는 **`dispatchCard`가 notes를 clobber해 `last_seq`를 제거** → 재dispatch 후 새 attempt는 스칼라 게이트에 안 걸림 | **코드 실독 확정**(`card-ops.ts:112-117` `notes: 'dispatched node=…'` · `:190-197` 스칼라 `payload.seq <= last`) — R43b-2차 claude L-new-3. 순수 "브릿지 재시작 후 재전송" 삼킴은 active binding 복원이 §9-6 out-of-scope라 발생 안 함(구 표현 "재시작만으로 삼킴 재현"은 철회) |

**Unknowns (§3.5):**
- **U1** — D1 실측이 `recipientCount=1`의 반례를 내면 strict 판정식을 어떻게 재설계할 것인가(`≥1`로 완화 vs 경로별 분기). **D1 완료 전에는 답하지 않는다.**
- ~~**U2** — durable alert의 전달 매체와 운영자 도달 보장 수준~~ → **해소 (R43b · 결정 ①⑨).** 매체 = **브릿지-로컬 append-only JSONL**(프로필 스코프), dedup 키 **태그드 판별 유니언**(R43b-2차/R43c — `send_unknown`=`(cardId, dispatchHandoffId, seq)` · `presence_unknown`은 result 미구성이라 seq 없이 `(cardId, dispatchHandoffId, "presence")`), 진입 시점 flush, 보존=운영자 ack까지(+ `presence_unknown` 늦은 단일 발행 `relay_accepted` 자동 해제(c) — append-only 자동-해소 fold), 미해소 카운트 표면화(§6.7.1). board(tower-local)·tower 통지(의심 채널)를 배제한 crash-durable 매체 선정 근거 명시. 대칭 위치 `recordResultDeliveryUnconfirmed`(휘발) 재사용 금지.
- ~~**U3** — 흡수 상태의 만료 dispose와 불변식의 긴장~~ → **소멸 (R43b · 결정 ①②).** 긴장의 원인이던 **만료 dispose 자체를 삭제**했다(락 8 개정 — 같은 락 본문·§6.6·락 13과 자기모순이었다, R43 H2). 이제 10분 타이머는 **재알림만** 유발하고(가역), durable quarantine 레코드가 **진입 시점**부터 존재하며, in-memory 해제는 운영자 ack·프로세스 종료(+ `presence_unknown`의 늦은 단일 발행 자동 해제 — R43b-2차)에서만 일어난다. **이 PATCH가 다루는 흡수 상태·타이머 경로에서 불확실한 관측이 유발하는 비가역 행동이 0**이 되어 긴장이 남지 않는다. *(R43b-2차 — codex H-new-1 부분 수용: 주어를 이 PATCH의 흡수 상태·타이머 경로로 **한정**한다. 전역 명제 "설계 전체에서 불확실 관측→비가역 0"은 세우지 않는다 — 락 11의 "연속 absence = 결정적 부정 증거 → 자동 blocked"는 (C) 본체 소관이고 그 명명 vs 증거 등급 C±의 긴장은 설계 §9-ter 정직성 항목 3에 등재했다. 락 11 재론은 기결정으로 기각.)*
- **U4** — conv 경로의 fire-and-forget/ACK-폐기(전수 목록 = 헬퍼 3곳 `:1300 :1319 :1777` · `void` 호출 4곳 `:1534 :2255 :2274 :2279` · `await`-폐기 1곳 `:1930`)와 **M4 비가역 상태 전진**(`:1777-1797`)을 언제 회수할 것인가(별도 PATCH 시점). 0.27.0은 **card-only 경계**를 명시적으로 주장한다.

**Review impact:** result 발행 소유권(issuer 중앙화)·ACK 판정·seq 규약(소유 Flight→issuer)·**tower currency 게이트**·**durable quarantine** 변경. **relay wire·MCP·herdr RPC 표면은 무변경**이나 `sendResult` 시그니처 교체가 `finishCard` pane.close 게이팅과 `pane-cleanup.test.ts:245`·`:816`을 함께 움직이고, payload 멱등키 각인은 **tower inbox 영속이라 비가역**이며, tower currency 게이트는 **tower-local 적용 판정**(`card-ops.ts` — host)을 바꾼다. 설계 정본 [`docs/spikes/PANE-DEATH-DESIGN.md`](spikes/PANE-DEATH-DESIGN.md) §6.7·**§6.7.1·§6.7.2·§6.7.3**·§6.7-bis·§9-bis 락 5·8·9·13. R43 검증 원장 [`docs/reviews/PANEDEATH-R43.md`](reviews/PANEDEATH-R43.md)(정본·수정 금지) — 본 개정이 그 High 6·Medium 5를 해소한다. **R43b 검증 원장 [`docs/reviews/PANEDEATH-R43B.md`](reviews/PANEDEATH-R43B.md)(정본·수정 금지)** — 2레인 검증(pending-revision·reject) + 아키텍트 판정 표 + 2차 개정(신규 High 4·Med 3·Low 3 국소 정밀화). **R43c 검증 원장 [`docs/reviews/PANEDEATH-R43C.md`](reviews/PANEDEATH-R43C.md)(정본·수정 금지)** — R43c 2레인 검증(pending-revision·reject) + 3차 개정(시대 매개변수화·전이표 분할·CAS 개명·태그드 키 전파·폴백 도달성 등 문면 전파, 신규 High 0). 3차 검증 원장 [`docs/reviews/PANEDEATH-CODEX-REVIEW3.md`](reviews/PANEDEATH-CODEX-REVIEW3.md)(정본·수정 금지). **R43e 필수(경량 델타 — R43d 잔여 3건 문면 반영 검증) — author-close 불가**(WORKFLOW §5.1 신규 MINOR + 신뢰 경계, 설계 §8 `rgate=needed`; R43c는 2레인 검증 pending-revision, 신규 High 0·문면 전파 반영 후 재검증).
