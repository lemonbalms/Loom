...
#### 0.24.2 — 2026-07-19 (`pending-review` R39 — **Windows 실배포 결함 2건 수정 — persist 경로 가드 POSIX 구분자 오탐 + relay 메시지 핸들러 uncaught 크래시** (PATCH))
...
| **D2 메시지 핸들러 크래시 가드** | `server.ts:176` `self.handleMessage(state, data)` 호출을 try/catch로 감싸, 예외 시 `console.error`(`[loom relay] handler error:` + 메시지) 후 계속해 **프로세스를 생존**시킨다. 클라이언트 측은 해당 요청이 무응답(타임아웃)이 되어 **기존 fail-closed 의미가 유지**된다(자동 성공 응답 위조 없음). create의 롤백·rethrow(`room.ts:743-752`, fail-closed)는 무변경. **기존 개별 op catch와 무간섭**: `addPeer`(`server.ts:317`)·`routeHandoff`(`:392`)·`inbox.claim`(`:485`)·`leave`(`:510`)는 각자 handleMessage **안에서** 에러 envelope를 회신하고 rethrow하지 않으므로 새 외곽 catch에 도달하지 않는다. 외곽 catch가 실제로 잡는 것은 create의 rethrow처럼 개별 catch를 갖지 않은 경로다(중복·이중 처리 없음). |
...
| **테스트** | **① D1** — `sep` 기반 가드가 정상 파생 경로를 통과함을 확인(기존 persist 왕복 테스트가 POSIX에서 커버 — 동작 불변이므로 Windows 특이 회귀는 재배포 라이브로 검증) + **조작된 `room.id`로 이탈 경로가 여전히 거부되는 부정 케이스**(가드 오탐-교정이 실질 완화가 아님을 고정). **② D2** — `handleMessage`가 throw하는 op(예: persist 실패 주입 create) 투입 시 **서버 생존 + 후속 요청 정상 처리**(기존 `server.integration.test.ts` 스타일). |
...
- **(a) 가드 완화 아님** — D1은 구분자 교정으로 **정상 내부 경로만** 추가로 통과시킨다. `path`가 realState 파생(`roomStatePath`)이라 이탈 경로는 구조적으로 생성 불가하며, 이탈 경로가 여전히 거부됨을 **부정 테스트로 고정**한다. POSIX 판정은 문자 그대로 불변(`sep === "/"`).
...
- **U1(Known unknown)** — Windows에서 D2 가드 외 추가 uncaught 표면(다른 op의 persist 경유 실패). D2가 message 콜백 외곽의 전역 방어라 커버되나, 개별 op별 실패 UX(무응답 vs 명시 error envelope)는 op마다 상이할 수 있다.
