...
#### 0.27.0 — 2026-07-21 (`pending-revision` **R43d reject — 잔여 3건 (4차 반영 후 R43e)** — **result 발행 단일 소유권 + ACK 경계 — relay ACK 실측 선결 · strict ACK 판정 · issuer 중앙화 + seq 소유권 이전 · tower currency 게이트 · fire-and-forget 제거** (MINOR))
...
- **durable quarantine의 내용 경계:** 레코드(= durable alert, §6.7.1)는 **메타데이터(cardId · seq · dispatchHandoffId · 상태 · 시각 · 사유 · 카운터)만** 싣고 워커 산출물 본문을 싣지 않는다(0.26.0 D6 계측 레코드 규약과 동형 — 본문성 §5.1 원칙 · 간접 프롬프트 주입 표면 차단). 매체는 **브릿지-로컬 append-only JSONL**(프로필 스코프)이며, board(tower-local)나 tower 통지(의심 채널)에 의존하지 않는다. 대칭 위치 `recordResultDeliveryUnconfirmed`(휘발) 재사용 금지(결정 ⑨).
