...
| **Boot persist** | **양 노드 상시화 완료(2026-07-20)** — kb `@reboot` crontab 2줄 · node-wsl-1 systemd 2 unit(drop-in `HOME=/root`)+Windows LogonTrigger Task. 함정 = systemd HOME 미상속 → herdr 소켓 경로 불일치 crash-loop(lessons platform (17)). **node-wsl-1 = cold-boot 실증 완료**(`wsl --shutdown`→wake, systemd 자동 herdr PID159·bridge PID162 기동). **kb = crontab 설치·정합 확인 완료, 실제 `@reboot` 발효 실증만 유예**(sudo reboot에 비밀번호 필요 → 다음 자연 재부팅 때 `tail /tmp/loom-bridge-boot.log`로 확인, 현재 bridge 541727 무중단). 팩 `.loom-boot-persist-pack.md` |
