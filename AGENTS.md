...
| **Inject ops** | Default = **all nine HANDOFF axes + traps** in state inject. Concision = HANDOFF diet (tables/bullets), not permanent slim-delete. After HANDOFF/traps edits: `bun run session-context:lint` — **omitted must be (none)**; prefer raw ≤ `STATE_TARGET` 7500 (warn). Owner brief = `bun run status` only. Full rules: `docs/spikes/SESSION-INJECT-VIEW-DESIGN.md` |
| **HANDOFF ship check** | Before claiming handoff/docs wave done: `bun run handoff:check` (= `handoff:lint` + `session-context:lint`) |
