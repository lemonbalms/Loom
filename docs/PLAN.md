...
#### 0.26.0 — 2026-07-20 (`pending-review` R41 — **hooks 보조 센서 (claude 워커 상태 힌트) — 스파이크 최소 배선 5단계 구현** (MINOR))
...
| **D4 신뢰 경계 = 브릿지-로컬 0600 소켓 리스너 + `isPathUnderLoomDir` 가드 · 비정상 페이로드 조용히 무시(카운터만)** | 브릿지가 여는 hook 소켓 리스너는 `inject-control.ts`(로컬 0600 소켓 선례)와 **동형**으로 신뢰 경계를 세운다 — 소켓 경로는 `loomDir()/hook-<cardId>.sock`(`injectSocketPath` `inject-control.ts:24-26` 패턴), 생성 시 **0600 퍼미션** + 경로는 `isPathUnderLoomDir`(`inject-control.ts:32-35`, `relative` 기반 loomDir 하위 강제) 가드를 통과해야만 watch한다. **방향은 역전 — 브릿지가 서버**(inject-control은 클라이언트, 여기선 브릿지가 리스너로 수신). **비정상·파싱 실패 페이로드는 조용히 무시**(카운터만 증가 — 로그 스팸·주입 표면 억제, D6 계측이 빈도 관측). **hook 스크립트 규약(스파이크 안전성):** 관측 훅은 **항상 exit 0**(exit 반직관: 0=정상 · 1=논블로킹 · 2=블로킹 일부 이벤트만 — 관측 스크립트는 워커를 막지 않음) + **stdout 비움**(`UserPromptSubmit`·`SessionStart` stdout은 Claude 컨텍스트에 주입되는 함정 — 관측만 할 땐 파일/소켓 append만, stdout 컨텍스트 오염 회피). hook 페이로드는 **신뢰 입력이 아니라 "완료 힌트"로만** 소비하고 실제 결과는 스크레이프로 회수해 신뢰 노출을 최소화한다(스파이크 R{n} 판정). |
| **D5 폴백 무회귀 (fail-open) — hook 부재·no_listener·소켓 오류 → 현행 경로 그대로** | hookHint가 부재하거나(hook 미주입·미발화) 소켓이 `no_listener`(리스너 없음)·오류일 때는 **현행 herdr status + 스크레이프 + 5분 유예 경로를 그대로** 쓴다(`inject-control.ts:51,99` `no_listener` 폴백과 동형). **훅 없음 = 현행과 완전 동일**(§2.5.3 규칙 1 — fail-open, 회귀 없음). 즉 이 PATCH는 기존 판정 경로에 hookHint **우선 분기를 얹기만** 하고, hookHint가 없는 모든 경우 관측 동작이 0.25.0과 바이트 동일해야 한다(무회귀 어서션이 검증 계획의 핵심). |
| **D6 계측 = loomDir 하위 JSONL append (hookHint 발화·폴백·permission_prompt 감지·가짜 agent_blocked 교정 카운터)** | 스파이크가 지목한 계측 공백(정량 빈도 대조 불가)을 해소하기 위해 **최소 계측을 이 PATCH 스코프에 포함**한다 — `loomDir()` 하위 JSONL(append-only)에 (a) hookHint 발화 횟수·이벤트 종류 · (b) 폴백(hook 부재/no_listener) 횟수 · (c) `permission_prompt` 감지 횟수 · (d) 가짜 `agent_blocked` 교정 횟수를 append한다. **완료 기준 포함:** 이 계측으로 empirical 증거 팩(2026-07-20)의 계측 공백을 해소하고 사후 이득 A/B 측정이 가능해지는 것이 이 PATCH의 완료 기준에 든다(아키텍트 결정 2026-07-20). JSONL 회전/크기 상한은 Unknowns U6. |
...
- **U3 — 브릿지 소켓 리스너 생명주기·정리 원자성.** 브릿지가 서버(방향 역전)라 `cardId → flight` 매핑 소멸(카드 완료·pane 정리, 0.23.8 `pane.close`) 후 `hook-<cardId>.sock` 파일의 unlink 시점·경합. 스테일 소켓 잔류·재-cardId 충돌 정책 미정.
- **U4 — `Stop` 힌트와 still-running 유예의 상호작용.** `Stop`이 워커 백그라운드 커맨드 실행 중에도 발화하면(0.23.7 still-running과 동일 통증) `Stop` 힌트가 오히려 조기 완료를 유발할 수 있다 — D3의 "done 최종 권위는 기존 경로" 원칙이 이를 막는지(유예를 앞당기되 done 단독 확정 불가), Stop 힌트를 스크레이프 still-running 지표와 어떻게 AND 결합할지 실측 확정 필요.
