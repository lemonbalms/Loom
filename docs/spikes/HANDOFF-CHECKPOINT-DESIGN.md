...
| Field | Value |
|---|---|
| **Status** | **proposal / independent review requested** |
| **Date** | 2026-07-22 |
...
> 이 문서는 현재 `HANDOFF.md`를 즉시 교체하는 규칙이 아니라 **검증 대상 설계안**이다.
> 독립 리뷰가 끝나고 Owner가 채택하기 전까지 현재 세션 리추얼·PLAN v0.28.0·PATCH 순서·
> commit/ship 규칙은 모두 기존 문서를 따른다.

---

## 0. 독립 리뷰 요청

리뷰 모델은 본 문서의 문안 품질보다 다음을 우선 검증한다.
...
| Blocker | Clear condition | Owner | Verifiable without blocker |
|---|---|---|---|
...
### Phase C — HANDOFF 다이어트

- 완료 서사를 archive/WORKLOG 후보로 이동
- 링크·좌표·`Don't redo` 보존 검증
- 새 구조로 `HANDOFF.md` 전환
- `AGENTS.md` 부분 읽기 규칙과 session-context 추출 동기화

### Phase D — 자동화

- lint 구조 검사
- status 파서 fail-loud 전환
- session-context 대상 섹션 갱신
...
- `session-context --part state`가 One-line·Current loop·Current action·Blockers·Invariants를 포함한다.
- 무훅 리추얼의 상단 읽기로 같은 항목을 얻는다.
...
독립 리뷰와 Owner 결정이 필요한 항목:

1. 전체 8,192B 한도를 채택할지, 현행 top-80/16,384B 이중 한도를 유지할지
2. `Current loop`를 ROADMAP 채택 전 HANDOFF에 둘지, flywheel 도입 시점까지 유예할지
3. WORKLOG를 새 파일로 만들지, 기존 `docs/HANDOFF_ARCHIVE.md`를 역할 승격할지
4. SessionStart fail-open 실패를 status 경고로 표면화할지
5. strict metadata/front matter가 필요한지, anchored Markdown으로 충분한지
6. Windows 전용 진입 링크를 필수 섹션으로 둘지 Evidence에 둘지
7. handoff lint를 commit·CI 중 어디에 강제할지

이 결정이 닫히기 전에는 본 설계를 현행 HANDOFF에 부분 적용하지 않는다. 부분 적용은 자동/수동 경로와
문서 소유권을 다시 어긋나게 만들 수 있다.
...
독립 리뷰가 이 네 가지 중 하나라도 현재 프로젝트에서 성립하지 않는다고 판정하면, 정식 전환을 중단하고
기존 HANDOFF 구조를 유지한다.
