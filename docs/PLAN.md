...
- **UK-8** — 다중 노드(WSL/VPS/Windows)에서 **플러그인 user-global 전환**의 실제 영향. 0.7.3 시대 세션-로컬 상태에서 올라온 노드는 사용자당 1회 재설치/재링크가 필요하다(COMPAT §2.1) — 로컬 어댑터 증명 전에는 문서 노트로만 둔다.
...
- [ ] `HERDR_PROTOCOL_EXPECTED === 17` **그리고** 라이브 서버 0.7.5/17
- [ ] `bun test`가 host herdr/bridge 표면에서 green · **전체 스위트는 차집합 판정**(절대 수치 금지 — lessons (10) · 아키텍트 독립 실행 SSOT)
- [ ] **라이브 단일 카드 스모크 3종 — `claude` · `codex` · `grok` 각각**: 워커 안에 env 도달 · 프롬프트 제출 · status 이벤트 수신 · 스크레이프 · close
- [ ] `bun run dogfood:herdr`가 compatibility ok 출력 · `dogfood:up`이 호스트 변경까지 진행
- [ ] **M-2 / 주입 정책이 `implementation-notes.md`의 신규 섹션 `§0.28.1`(Deviations)에 기록**됨 — R46 L-3: 기존 protocol-16 M-2/bare-Enter deviation 항목을 덮어쓰지 않고 **0.28.1 전용 섹션을 신설**해 append한다(원자적 `agent.prompt` 채택 · dual-`agent.send` 소멸 · 중복 제출 방지 4절 · `BARE_ENTER` 처분 · `submitDelayMs` 제거를 각각 기재)
- [ ] `docs/spikes/HERDR-0.7.5-COMPAT.md` verdict가 **shipped + 커밋 SHA**로 전환 · HANDOFF dogfood 행 unblocked
- [ ] **v0.28.0 회귀 게이트 G-1 · G-2 · G-3이 여전히 green**(이벤트 additive 필드가 완료 권위로 새지 않았다는 증거)
