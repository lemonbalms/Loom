...
#### 0.27.0 — 2026-07-21 (`pending-review` **R43c 대기 (R43b 2차 개정)** — **result 발행 단일 소유권 + ACK 경계 — relay ACK 실측 선결 · strict ACK 판정 · issuer 중앙화 + seq 소유권 이전 · tower currency 게이트 · fire-and-forget 제거** (MINOR))
...
| **선결** | **D1-b 순서 역전의 회귀 영향 실측 (아키텍트 추가 2026-07-21 — 미확인의 게이트 승격)** | 증거 팩이 정직하게 남긴 항목을 게이트로 올린다: *"순서 역전이 회귀 게이트 3개(`still-running.test.ts:452` · `pane-cleanup.test.ts:245`·`:816`)를 깨는지는 **미확인**이다. '안 깨진다'가 아니라 **확인 안 했다**."* 특히 **`pane-cleanup.test.ts:245`("exactly once")는 D3의 strict ACK로도 깨질 수 있다고 이미 경고된 바로 그 테스트**라 두 변경이 같은 테스트에 겹친다 — 어느 쪽이 깼는지 분리되지 않으면 진단이 엉킨다. **완료 판정 (R43b 재설계 — 결정 ⑥ · M2 해소):** "순서 역전만" 격리는 D5가 "셋은 쪼갤 수 없다"고 한 **의도적 무효 상태**를 만든다(Flight 노출 + CAS/latch 부재 = D5가 통제하려던 바로 그 레이스) — 따라서 그 격리에서 나온 실패는 진짜 회귀도 구 전제도 아닌 **중간 상태의 산물**일 수 있다. 재설계: **① ACK 분류를 결정적 `accepted` 스텁으로 고정**한 채 **baseline ↔ 완전한 D5 원자 집합(순서 역전+CAS+`terminalPending` latch)**을 비교하고, **② 그 다음 D3 strict ACK를 얹어 재비교**한다. 판별표 칸 = **(a) 진짜 회귀 · (b) 테스트의 구(舊) dispose 순서 전제 · (c) 중간(무효) 상태의 산물**. **이 판별 없이 D5 확정에 들어가지 않는다**(교훈 30). **락:** 없음(선결 실측) — **D5의 선결**. |
...
| `sendResult`가 `peer_unknown`에도 `true`를 반환한다 | **코드 실독 확정**(`bridge-runtime.ts:2423-2440` + `relay-client.ts:336-344`) |
...
- ~~**U2** — durable alert의 전달 매체와 운영자 도달 보장 수준~~ → **해소 (R43b · 결정 ①⑨).** 매체 = **브릿지-로컬 append-only JSONL**(프로필 스코프), dedup 키 `(cardId, dispatchHandoffId, seq)`, 진입 시점 flush, 보존=운영자 ack까지, 미해소 카운트 표면화(§6.7.1). board(tower-local)·tower 통지(의심 채널)를 배제한 crash-durable 매체 선정 근거 명시. 대칭 위치 `recordResultDeliveryUnconfirmed`(휘발) 재사용 금지.
...
**Review impact:** result 발행 소유권(issuer 중앙화)·ACK 판정·seq 규약(소유 Flight→issuer)·**tower currency 게이트**·**durable quarantine** 변경. **relay wire·MCP·herdr RPC 표면은 무변경**이나 `sendResult` 시그니처 교체가 `finishCard` pane.close 게이팅과 `pane-cleanup.test.ts:245`·`:816`을 함께 움직이고, payload 멱등키 각인은 **tower inbox 영속이라 비가역**이며, tower currency 게이트는 **tower-local 적용 판정**(`card-ops.ts` — host)을 바꾼다. 설계 정본 [`docs/spikes/PANE-DEATH-DESIGN.md`](spikes/PANE-DEATH-DESIGN.md) §6.7·**§6.7.1·§6.7.2·§6.7.3**·§6.7-bis·§9-bis 락 5·8·9·13. R43 검증 원장 [`docs/reviews/PANEDEATH-R43.md`](reviews/PANEDEATH-R43.md)(정본·수정 금지) — 본 개정이 그 High 6·Medium 5를 해소한다. **R43b 검증 원장 [`docs/reviews/PANEDEATH-R43B.md`](reviews/PANEDEATH-R43B.md)(정본·수정 금지)** — 2레인 검증(pending-revision·reject) + 아키텍트 판정 표 + 2차 개정(신규 High 4·Med 3·Low 3 국소 정밀화). 3차 검증 원장 [`docs/reviews/PANEDEATH-CODEX-REVIEW3.md`](reviews/PANEDEATH-CODEX-REVIEW3.md)(정본·수정 금지). **R43c 필수 — author-close 불가**(WORKFLOW §5.1 신규 MINOR + 신뢰 경계, 설계 §8 `rgate=needed`; R43b는 2레인 검증 pending-revision, High 4건 반영 후 재검증).
