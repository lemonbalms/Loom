...
#### 0.27.0 — 2026-07-21 (`pending-review` **R43b** — **result 발행 단일 소유권 + ACK 경계 — relay ACK 실측 선결 · strict ACK 판정 · issuer 중앙화 + seq 소유권 이전 · tower currency 게이트 · fire-and-forget 제거** (MINOR))
...
- **(C) 전환 본체** — 자동 `done` 폐지(락 11) · phase registry · lifecycle `generation`(락 8) · bounded reconcile · `send_unknown`/`presence_unknown` 분리 구현(락 13). 이 PATCH는 그 전환의 **liveness 축만** 담당하며 correctness 축은 별도 PATCH다.
...
| 현행 설계도 **브릿지 재시작만으로 result 삼킴 재현**(in-memory seq 리셋 < 디스크 last_seq) | **코드 실독 확정**(`card-ops.ts:190-197` 스칼라 `payload.seq <= last`, `last_seq`는 board JSON 영속) |
...
- ~~**U3** — 흡수 상태의 만료 dispose와 불변식의 긴장~~ → **소멸 (R43b · 결정 ①②).** 긴장의 원인이던 **만료 dispose 자체를 삭제**했다(락 8 개정 — 같은 락 본문·§6.6·락 13과 자기모순이었다, R43 H2). 이제 10분 타이머는 **재알림만** 유발하고(가역), durable quarantine 레코드가 **진입 시점**부터 존재하며, in-memory 해제는 운영자 ack·프로세스 종료에서만 일어난다. **불확실한 관측이 유발하는 비가역 행동이 0**이 되어 긴장이 남지 않는다.
