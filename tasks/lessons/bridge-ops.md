...
## 2026-07-19 (17) — 멀티룸 `@node` dispatch는 active relay 바인딩이 라우팅 전제

**배경 (0-c ④ 라우팅 운용):** `@node` 어드레싱 자체는 기완비 — `@displayName` 파싱
(`envelope.ts:59`) → relay `room.ts:452` `resolveTargets`→`findPeerByName` 단일-타깃 enqueue,
브릿지단은 M-1 allowlist + `CARD_DISPATCH_LABEL` 이중 게이트, 회수는 `applyCardResult`가
L-2 assignee 대조. 신규 표면이 없어 R{n} 불요(ops). ROUTE 카드 3발로 라이브 실증(각 노드가
**자기 카드만** 스폰 — 스폰 사이클 정확 1개·타 카드 흔적 0, bridge stderr 실측).

**함정 (라우팅의 실무 전제):** `dispatchCard`는 `loadSession()`이 반환하는 **active 바인딩
룸**으로만 나간다. 노드가 여러 룸에 흩어져 있으면(이번 토폴로지: mac-node = loom-local
`room_32f3322b595456b7` / wsl·vps = loom-dev `room_ca184b781cfdabdc`, tower는 이중 신원
local `p_726870658689123e`·win `p_45115c32d2c462f9`) 한 룸 바인딩으로는 다른 룸 노드에
카드가 안 간다.

**Rule:** 멀티룸 dispatch는 **`loom relay use <name>`로 바인딩 전환 후 발사**한다. 전환은
새 bun dispatch 프로세스에서 즉시 반영되므로(sticky host "이미 다른 relay 구동 중" 경고는
새 프로세스라 무해) 룸별로 `use` → dispatch를 반복하면 된다. **회수(card.done)도 바인딩별
인박스**라 룸을 바꿔가며 claim한다.

**부수 실측:**
- `loom bridge status`는 **foreground bun 브릿지**(setsid nohup으로 띄운 노드)에서 pidfile을
  안 만들어 **false offline**로 표시된다 — 프로세스는 생존. 상태 확인은 `--profile` 지정 +
  프로세스(ps)/health 직접 확인으로.
- 원격 WSL 명령(`ssh win-loom 'wsl -e bash -c …'`)은 cmd.exe 이스케이프가 붕괴하므로
  **base64 래핑**(`echo <b64> | base64 -d | bash`)이 안전.
- VPS(kb) `stat` 타임스탬프는 **UTC**(mac/wsl은 KST) — 로그 시각을 노드 간 대조할 때 9시간
  오프셋 주의.
- 잔존 정리 후보: mac-node bridge config에 스테일 dispatcher `p_ed676195eecd9488`(무해).

## 2026-07-20 (21) — `card.done` 수신 ≠ 완료 · 종료코드로 실패 판정 금지

**인과 주의 (codex 교차검증이 아키텍트 진단을 정정):** pane이 죽어서 가짜 done이 나는 것이
**아니다**. 순서가 반대다 — **브릿지가 완료 후보를 너무 일찍 `done`으로 커밋하고, 그 결과로
`pane.close`를 호출한다** (`finishCard()` — `bridge-runtime.ts:2310-2323`).

**따라서:**

1. **산출물은 아키텍트가 워킹트리에서 독립 검증해야 확정된다.** `card.done` 수신은 착수
   신호일 뿐이다. 2026-07-20에 가짜 done 2건 실증(codex TUI 초기화면 스크레이프 ·
   `Working` 중 발행).
2. **종료 코드·시그널만으로 실패를 판정하지 마라.** 정상 cleanup에서도 exit 129/SIGKILL이
   나온다 — 실측: pane **215·217**은 `status=done` result 전송 뒤 각각 **106ms / 4ms** 만의
   정상 정리였다. 이것을 "조기 사망"으로 읽으면 멀쩡한 완주를 실패로 오판한다.
3. **`PaneDied for unknown pane`은 herdr 내부 경고**이지 브릿지가 통지를 버렸다는 증거가
   아니다. 아키텍트가 이 오독으로 **오너에게 틀린 진단을 보고**했다.

**설계 정본:** `docs/spikes/PANE-DEATH-DESIGN.md` (권고 B — 종료 펜스 + 결과-커밋 tombstone
+ bounded reconcile). cross-ref: orchestration (24).

## 2026-07-20 (22) — 임시 셸 감시 4연속 실패 → `scripts/watch-card.ts`

**실증 (2026-07-20):** 손으로 짠 감시가 **네 번 연속, 매번 다른 곳에서** 뚫렸다.

1. **마커만 보고 pane 사망을 놓침** — 성공 경로만 감시하니 워커가 죽어도 계속 기다렸다.
2. **관측을 가정으로 승격** — "codex는 안 죽는다"는 그날의 관측을 전제로 삼아 소멸 감시를
   제외했고, 사용자의 수동 종료를 놓쳤다.
3. **macOS bash 3.2 연관배열 미지원으로 즉사** — 그러면서 "35분 경과"라는 **거짓 성공**을
   보고했다. 스크립트가 죽었는데 감시가 성공한 것처럼 보였다.
4. **마커 검출 후 종료하지 않아 알림 미전달** — 검출은 했는데 통지가 안 나갔다.

**규칙:** 워커 감시는 **`scripts/watch-card.ts`를 쓴다 — 임시 셸 스크립트 금지.**
`watch-card`는 종료 사유를 exit code로 구분한다: `marker`=0 · `pane-gone`=1 · `limit`=2 ·
`timeout`=3.

**원칙:** **완주와 사망을 같은 코드로 뭉개지 마라** — 그것이 거짓 성공의 근원이다.
cross-ref: bridge-ops (21)·verification (23).
