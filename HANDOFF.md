...
| **PLAN** | **v0.25.0** `approved` (R40 정식 리뷰·author-close 아님, M-A~M-D 락 반영 + L-1..L-3) → **implemented** (`b343ada` + dist `3e77409`) · verify-0250 opus 독립 락 대조 12/12 PASS · **D10 라이브 스모크 미실시(다음 액션)**. 직전: **v0.24.2** `approved` (R39 author-close, no R39b) → **implemented** (`59bfeae` + dist `6aaf54f`) · **Windows 라이브 실증 완결**(재로그온 룸 유지 확정·loom-dev LOOM-GT4B 7 peers 재수립) |
...
> 순서: 멀티노드 트랙 진행 중 — ⓪ 단독 모드·① Windows relay 복귀·①-b relay 룸 영속화·② WSL 노드 브릿지 복제·③ Linux/VPS 노드 브릿지 복제·④ `@node` 라우팅 운용·ⓐ artifact fetch 자동 실행 도입(v0.25.0 `conv_fetch` R40 approved+구현 `b343ada`) 완주, **다음 = ⓐ-D10 라이브 스모크(실 노드 scp 왕복) + 노드 부팅 생존 상시화**.  
