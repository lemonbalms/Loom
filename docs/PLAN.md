...
#### 0.23.11 — 2026-07-19 (`pending-review` R35 요청 — **잔여 후보 일괄: claude 상태줄 chrome 필터 + summary 타이밍줄 선별 + 풀 탭 동시 스폰 직렬화 + still-running supersession (후보 ①③④⑤)** (PATCH))
...
| **⑤ still-running supersession** | `hasStillRunningIndicator` 현행 판정(tail-10 비공백 join·공백-정규화·보수 패턴) **유지** + **supersession 판정 추가**: joined 텍스트에서 **마지막 still-running 매치의 종료 위치 이후**에 완료-타이밍 패턴(`Worked for (?:\d+h)?(?:\d+m)?\d+(?:\.\d+)?s\.` — ③과 공유 상수의 비앵커드형, 복합형 커버) 매치가 존재하면 지표 소거로 판정(= false 반환). 위치 기반이라 지표줄 내부의 "Worked for 20s." 프리픽스는 supersession이 아님(매치 시작 < 지표 종료). **보수성**: supersession 증거가 없으면 현행 유지 — 조기 회신(0.23.7 수정 대상) 회귀 없음. **잔존 수용**: 워커가 실행 중 최종 답변에 "Worked for Ns." 문자열을 인용하는 코너 케이스는 조기 판정 가능 — 관찰된 바 없고 최악이 0.23.7 이전 동작(미완 화면 회신) 1회로 bounded. 유예 폴링 경로에도 동일 함수 적용(자동). |
| **VERSION** | CLI `VERSION` + MCP serverInfo 0.23.10 → **0.23.11** 동기화. |
| **테스트** | **①** claude 상태줄 실측형(`Fable 5 ⚡high 🧠 │ fable-advisor main`·effort 생략형 `Fable 5 🧠 │ …`) 제거 + 경계 보존(`│` 없는 `⚡` 콘텐츠 줄·` │ ` 있으나 이모지 없는 콘텐츠 줄·본문 `output` 무적용 회귀 R31 M-1) **③** 말미 "Worked for 3s." → 직전 실내용 줄 채택 · 복합형 "Worked for 1m49s." + 말미 `█` 잔재(라이브 실측형) 스킵 · 전부 타이밍줄 → 현행 폴백 · "Worked for 3s. 그리고…" 혼합 줄 비스킵 **④** fake-herdr로 동시 `spawnWorkerAgent` 2건 → `tab.create` 정확 1회 + 동일 tabId 스폰 2회 · 첫 스폰 예외(폴백)에도 두 번째 정상(비전파) · legacy 회귀 **⑤** 프로브 실측 시퀀스(스테일 지표줄+후속 "Worked for 25s.") → 무유예 즉시 완료(closePane) · 지표줄이 말미(진행 중) → 유예 진입(0.23.7 회귀) · "Worked for 48s. 1 command still running" 단일줄 → 유예 진입(내부 프리픽스 비-supersession) · 유예 폴링 중 supersession 등장 → 소거 완료(closePane) · 기존 카드·conv 왕복 회귀. |
...
- **⑤는 회신 타이밍 판정만** — 내용 무변경·상한(300s)·폴링 주기 무변경. supersession은 판정을 **완료 방향으로만** 이동(조기 회신 리스크는 위 잔존 수용 항목으로 bounded — 위조하려면 워커 자신이 자기 회수를 앞당길 뿐, 신뢰 경계 무영향).
