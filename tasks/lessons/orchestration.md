...
## 2026-07-26 (53) — 오너 지적: 하네스 웨이브라도 **제품 구동**을 확인하라

**실증 (D7 사전등록 웨이브):** 변경 표면이 문서(`RULE-ROUTER-PREREG.md`)와 하네스 스크립트
(`scripts/rules-registry.ts`)뿐이었으므로 아키텍트는 **제품(Loom) 구동 확인을 건너뛰고**
스위트 결과만 봤다. 오너가 물었다 — *"Loom 구동은 왜 확인을 안 하지?"*

지적이 옳았던 이유는 두 겹이다.

1. **스위트에 실패가 남아 있었다.** 그 상태에서 "제품 코드 미변경"은 실패를 환경성으로 넘길
   근거가 되지 못한다. 변경 표면이 좁다는 것은 **회귀 가능성이 낮다**는 뜻이지 **지금 제품이
   구동된다**는 뜻이 아니다 — 둘은 다른 명제다.
2. **확인 비용이 작고 산출이 실재했다.** `loom doctor`(0 fail) · `smoke:durable` OK ·
   `smoke:desktop` OK로 구동을 확정했고, `smoke:uc`에서 **상시 실패 2건**(UC-3 sticky host
   `already running` · handoff `peer_unknown`)을 발견했다. HEAD 베이스라인과 차집합 0이라
   이번 웨이브의 회귀는 아니었지만, **아무도 보지 않고 있던 실패**였다.

**규칙:** 게이트가 제품 표면을 건드리지 않아도, **웨이브를 닫기 전에 제품이 실제로 뜨는지
한 번 확인한다**(`doctor` + 소켓 스모크). 소켓·포트를 쓰는 스모크는 **전체 스위트와 동시
실행하지 않는다**(교훈 (4) 기아 위양성) — 스위트 종료 후에 붙인다. 그리고 스모크에서 나온
상시 실패는 **미진단 상태 그대로 HANDOFF Active checks에 올린다**; 조용히 넘기면 다음 세션이
같은 실패를 다시 "환경성"으로 추정한다.

cross-ref: verification (51)·(4)·(10).

## 2026-07-26 (54) — `single`에서 검증 피어를 띄우지 마라: 레인 분리는 **세션 승계**로 성립한다

**실증 (D7 사전등록 웨이브):** topology `single` 세션에서 아키텍트가 D7의 "리뷰어 승인"을 얻으려
**codex 검증 피어를 스폰**했다. 그 레인이 두 번 무응답으로 끝나자 이를 **블로커로 HANDOFF에 올리고**,
오너에게 "피어 세션 verdict vs 오너 직접" 중 승인 주체를 골라 달라고 했다. 오너가 물었다 —
*"피어 세션 verdict 이게 지금 왜 나오지?"*

**정본이 정반대를 말한다.** `DOGFOOD_LOOP.md` §0.5.1: *"single은 **교차 검증 피어를 안 띄운다**."*
§0.5.3: *"한 세션(한 벤더)이 구현 + 검증(명령 객관화) + ship. **별도 verify 피어 디스패치 없음**."*
즉 **애초에 라인에 없던 레인을 띄우고, 그것이 죽었다고 블로커로 보고**한 것이다. 없는 레인의
부재는 블로커가 아니다.

**그럼 `single`에서 리뷰어는 누구인가 — 같은 트랙에 선례가 있었다.** `RULE-ROUTER-REVIEW.md`
헤더: *"레인: 본세션(topology `single`) 리뷰어 · **저자 = 직전 세션**(레인 분리 준수)"*.
**레인 분리가 공간(피어 디스패치)이 아니라 시간(세션 승계)으로 성립**한다 — 다음 세션이
리뷰어 자격으로 직전 세션의 산출물을 판정하면 발견자≠수정자가 지켜진다.

**규칙:**
1. **`single`에서 검증 피어(codex-rev·claude-rev 등)를 스폰하지 마라.** 필요하면 그것은
   topology 승격 사유이지 조용히 띄울 일이 아니다(승격은 오너 결정 — §0.5).
2. **자문(advisor)과 검증 피어를 구분하라.** Claude line의 `fable-advisor`는 line 정의
   (`Claude → Grok → Claude Advisor`)에 있는 **자문**이라 `single`과 충돌하지 않는다.
   충돌하는 것은 **verification lane**을 새로 띄우는 행위다.
3. **디스패치하지 않은 게이트를 블로커로 적지 마라.** "리뷰어 승인 대기"는 다음 세션이
   집을 수 있는 **정상 인계**이지 막힘이 아니다 — 이 리포에서 Blockers는 `(none)`이 기본이고
   대기 사유는 Owner pending(안전 default 열)이 자리다.

cross-ref: orchestration (24) 레인 분리 · verification (51).
