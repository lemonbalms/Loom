...
| **Current verdict** | **[Addendum D](#addendum-d--rev-3-delta-재리뷰-2026-07-23)** — rev-3 delta **APPROVE-DELTA** (H1·M1–M4·L1–L4 **9/9 CLOSED** · 신규 Low 3, 정정/예약 완료). [Addendum C](#addendum-c--rev-2-전체-독립-재리뷰-2026-07-23)(rev-2 전체 PENDING-REVISION)는 **그 delta의 근거 원장**이며, 본문 F1–F8은 폐쇄 확인됨(§C.1) · **역사적 원장으로 읽을 것** |
...
# Addendum D — rev-3 delta 재리뷰 (2026-07-23)

| Field | Value |
|---|---|
| **Target** | rev-3 working tree (base `38202ff:docs/spikes/SESSION-START-UNIFIED-PROPOSE.md`) · delta 96+/40− |
| **Requested scope** | **delta only** — Addendum C가 §C.6에서 "전면 재리뷰 불요, 변경 절 + full-doc 무결성"으로 한정 |
| **Reviewer** | `fable-advisor` 서브에이전트 (model `fable`, read-only) · 아키텍트 코드 교차 실측 |
| **Verdict** | **APPROVE-DELTA** — H1 · M1–M4 · L1–L4 **9/9 CLOSED** · 신규 Critical/High/Medium **0** · Low 3 |
| **Effect** | rev-2 §11 approve eligibility가 **성립**한다 (Addendum C full review + 본 delta 재리뷰 합산 APPROVE). 설계 승인 자체는 **오너 pick** |
| **Frozen target (D7)** | `cc03474:docs/spikes/SESSION-START-UNIFIED-PROPOSE.md` · blob `6966d57c` — §11 `Target git blob/commit`에 기입할 immutable 좌표 |

## D.1 발견별 판정

| ID | 판정 | 근거 (rev-3) |
|---|---|---|
| **H1** | CLOSED | §5.2 builder가 "exactly one complete envelope" + `[LOOM-SESSION-CONTEXT-END v1 · <part>]`를 **wire 규범으로 승격**("prefer" 소멸) · "matching END 부재 → S ≠ full" 명문 · §9 S 판정 규칙 · smoke #2/#7 개정 |
| **M1** | CLOSED | §5.3 "N=LOADED 도달 경로" 열 신설(호스트별 훅 수·grammar·budget 조건) + package 소유권 봉합(payload=NORMS-RECEIPT · wiring/grammar/budget=DELIVERY · 양쪽 authorized 시에만 배선) |
| **M2** | CLOSED | §5.3 N primary 열이 호스트별 "봉투 없으면 UNVERIFIED"로 §7.1과 봉합 · 해제 경로를 같은 행에 명시 · `norms:raw`를 accelerator가 아닌 **정본 복구 경로**로 규정 |
| **M3** | CLOSED | §7.1 신규 행(channel omission·head/tail 표식 → UNVERIFIED, 모든 marker 가시여도 동일) · §7.2 END 유무 무관 규칙 · §7.3 gate 5 `norms:check` commit gate + drift 재측정 + fail-closed · smoke #10/#11 |
| **M4** | CLOSED | §5.7 enforcement registry — 3등급 정의 + 8개 규칙 표면 current/post class + §13 종속 선언. **동결 절(§6/§8/§11) 표를 건드리지 않고** 닫은 형태(C.6 처방과 배치가 다르나 목적 충족) |
| **L1** | CLOSED (코드 실증) | §5.2 "진단 줄은 BEGIN 앞 허용 · payload 첫 줄 계약 아님" + §9 "진단 줄 선행만으로 partial 아님". 대조: `session-context.ts:181` `${warn}\n\n${body}` · `:132` truncation head 선행 — **계약을 실동작에 맞춰 완화**한 것이 정답 |
| **L2** | CLOSED (배치 이동) | smoke #12 + 해석 규칙(순차 발화 = precedence 충돌 아님). C.6은 §6.1 수정을 적었으나 §6은 C.6 자신의 재개봉 금지 구역 — **저자가 §10.2 acceptance로 옮겨 동결을 지켰다** |
| **L3** | CLOSED | STALE 판정 주체 = `norms:raw`/`norms:check` 스크립트 고정 · digest 단위 = **각 pack body별** SHA-256 앞 8자 · §9와 정합 |
| **L4** | CLOSED | Codex CLI `0.144.6`/`0.145.0` 관측 병기 + upgrade 시 fixture 재검증(§5.3 · §14) |

## D.2 동결 구역 delta 점검

- **§0 · §6 · §8: 무변경** (diff 컨텍스트 줄뿐 — 실측 확인).
- **§11: delta 있음.** 내용은 (a) rev-2→rev-3 기계적 개번호, (b) approval eligibility를 "Addendum C full + rev-3 delta 합산 APPROVE"로 교정. (b)는 **C.6 "이후 절차"가 명시 요구한 교정**이고, 개번호 없이는 문서가 *"rev-3인데 full rev-2 재리뷰를 요구"* 하는 자기모순에 빠진다. 옵션 구조·subset 제약·prerequisite는 무변경 → **freeze rule 위반 아님**.

## D.3 신규 결함 (delta 도입분) 및 처리

| Sev | 좌표 | 내용 | 처리 |
|---|---|---|---|
| Low | §5.7 registry | "기존 S nine/traps/lessons completeness"를 `session-context:lint`로 귀속했으나, 해당 명령의 실패 조건은 **omit 존재·SOFT_CAP 초과뿐**이다. HANDOFF `## Blockers` 헤딩을 지워도 exit 0(결측은 `session-context.ts:324` 경고 part 주입만). 실제 게이트는 `handoff:lint`(`handoff-lint.ts:80-83` `REQUIRED_HANDOFF_HEADINGS`) | **정정 적용** — 셀을 `handoff:lint`(nine/traps) · `session-context:lint`(budget/omission) · 합 `handoff:check`로 분담 표기 |
| Low | §5.2 ↔ §9 | §5.2는 truncation 표식 시 "완전성을 **별도 판정**"(조건부), §9는 "channel/self-truncation은 **partial**"(무조건). 두 독법 모두 full로 승격시키지 않아 안전하나 문언 긴장 | **정정 적용** — §5.2를 "`full`이 아니며 판정은 §9를 따른다"로 정합화 |
| Low | §10.2 #12 ↔ §7.3 | 복합 발화 규칙이 §10.2에만 있어 `lexicon` pack 추출 범위(L0 `Triggers + precedence` 표)에 기계적으로 포함되지 않음. 봉투만 적재한 세션은 "상태 확인하고 이어서 해"를 §6 precedence 2>3으로 read-only 귀결(보수 방향) | **예약** — §12 Phase 0a에 "L0 `Triggers + precedence` 절에 §10.2 #12를 포함해 `lexicon`이 흡수" 명시 |

세 건 모두 **보수 방향**(과소 신뢰 쪽으로만 어긋남)이라 blocker가 아니었으나, freeze **전**이므로 delta 내에서 닫았다. 1·2번은 코드 실측으로 사실이 확정된 서술 오류이므로 새 설계 판단이 없다 — 재재리뷰 대상 아님.

## D.4 잔존 리스크 (승인 시 오너가 지고 가는 것)

**이번 폐쇄는 전부 문서 계약 수준이다.** S END 방출과 `norms:check`는 아직 코드에 없고(§5.7이 `prose`로 정직 선언), 기계 강제는 Phase 1/3 fixture가 도착해야 성립한다. 즉 `APPROVE-DELTA` = **설계 승인 자격 성립**이지 실행 표면이 닫혔다는 뜻이 아니다. Phase 1/3에서 fixture가 §5.2/§7.1 문언과 어긋나면 그 시점에 재개봉된다.

부수 관찰: 현행 hook은 END를 방출하지 않으므로 §9를 문자 그대로 적용하면 오늘 세션의 S는 `partial`이다. 이는 **ritual 복구를 부르는 보수 판정**이라 무해하며, §5.7이 해당 규칙을 `prose`로 분류해 현재 강제력을 과대 주장하지 않는다.

**Must not (이 addendum):** 설계 승인·구현 착수로 해석 · §0/§6/§8/§11 재개봉 · `docs/plan_review.md` R{n} 기입.

Advisor: fable-advisor consulted: **yes** (model `fable` · read-only).

---

# Addendum E — F6 routing finding retracted (Owner correction, 2026-07-23)

| Field | Value |
|---|---|
| **Target** | Initial review F6 → rev-3 P7/§6/§8/§10/§13 routing statements |
| **Verdict** | **RETRACTED** — F6 was a false-positive SSOT-precedence finding |
| **Authority** | Owner clarification · `docs/DOGFOOD_LOOP.md` §0.5 · `docs/spikes/SINGLE-TOPOLOGY-EXECUTION-DESIGN.md` |
| **Package effect** | none on NORMS §7 or DELIVERY wire contracts; execution routing only |

## E.1 Evidence

1. `dc83d2b`의 원 교훈은 approved `loom doctor` **product code**를 architect가 쓰려 한
   사건이며, DOGFOOD §1.2도 product-code 범위를 유지했다.
2. `5fafcd1`은 review 초안보다 앞서 topology SSOT를 등록했고 `single`을 “현재 세션 구현 +
   명령 검증 + ship”으로 정의했다.
3. F6는 초안과 AGENTS 요약만 대조했다. AGENTS 자신이 topology SSOT로 가리키던 DOGFOOD
   §0.5를 증거 목록과 conflict 판정에서 누락했다.
4. 따라서 초안의 `impl / single arch`는 상위 규범 약화가 아니라 당시 최신 topology 정책과
   정합했다. 범위를 잃은 것은 AGENTS 요약 쪽이었다.
5. `edfea98` HANDOFF가 `topology single`과 architect 손코딩 금지를 동시에 기록하면서 실제
   실행 모순이 확인됐다.

## E.2 Correction

- `single` = bounded current session implementation + objective-command verification.
- `full` = architect/implementer/verifier separation and impl-lane escalation.
- lockedness alone does not select topology.
- a **new unresolved** product/security/trust/fail-closed decision promotes to `full`.

Frozen `cc03474` remains immutable evidence. This addendum supersedes only its
F6-derived routing interpretation; N envelope, exact packs, digest, budget, and
enablement gates remain unchanged.
