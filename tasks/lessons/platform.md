...
## 2026-07-20 (n) — claude-mem 헤더 분 단위 ts 제거 (원인 B · B-4 홈 패치 · **비영속**)

**증상**: 프롬프트 캐시가 매 세션 깨진다. 원인 A(hook 커맨드 2분할 concat, (19)-②)를 고쳐도
"같은 분 안에 재시작"할 때만 히트한다. 미스 1회 = **33,915토큰 cache-write**(79k 환경 실측치 ·
실 리포 48k 환경은 34,635 — **환경별 수치 혼용 금지**), 세션당 1회 + 분 경계마다.

**원인**: claude-mem이 SessionStart로 주입하는 컨텍스트 헤더가
`# [proj] recent context, 2026-07-20 6:43pm GMT+9` — **분 단위 타임스탬프**가 박혀 있어
캐시 재사용 상한이 1분으로 고정된다. 홈(`~/.claude`) 설치본이라 **blast radius = 전 프로젝트**.
`~/.claude-mem/settings.json`에 헤더 포맷/ts 관련 키는 **없다**(`CLAUDE_MEM_CONTEXT_*` 전수 확인) —
설정으로는 못 고친다.

**⚠️ 함정 — `scripts/context-generator.cjs`는 데드 번들이다.** 설계 정본
`docs/spikes/HOOK-CACHE-FIX-DESIGN.md` §5와 초기 브리프가 이 파일의 `rt()`/`lt()`를 지목했으나,
13.11.0 전체에서 `grep -rl "context-generator"` **0건** — 어디서도 호출되지 않는다. 거기를 고치면
**런타임 동작이 전혀 바뀌지 않는다.** 실체는 `scripts/worker-service.cjs`(헤더 로직 인라인 복제).
`server-service.cjs`·`mcp-server.cjs`·`transcript-watcher.cjs`에는 없다(`recent context` 0건).

**조치 (실시행분)**:

- 패치 경로 — **13.11.0 한 곳만**:
  `~/.claude/plugins/cache/thedotmack/claude-mem/13.11.0/scripts/worker-service.cjs`
  (marketplaces 사본 `~/.claude/plugins/marketplaces/thedotmack/plugin`은 바이트 동일한 **도달 불가
  폴백**, `cache/…/13.10.2·13.10.3·13.10.4`는 잔존물 — 셋 다 미패치. 훅의 경로 해석은
  `$CLAUDE_PLUGIN_ROOT` → `ls -dt cache/…/[0-9]*/` → marketplaces 순인데 모두 13.11.0으로 수렴.)
- 백업: 같은 디렉터리 `worker-service.cjs.pre-hookcache`
  (원본 sha1 `bb81abf5293f82faa8edcf06d964b460a82ee00a`). **git 대상이 아니므로 되돌릴 방법은 백업뿐.**
- 두 함수(13.11.0 기준 `qJ()` 플레인 · `eQ()` ANSI)를 **날짜까지만** 반환하도록:
  `function qJ(){let t=new Date;return t.toLocaleDateString("en-CA")}`
  **한쪽만 고치면 출력 경로에 따라 분이 남는다** — 반드시 둘 다.

**재적용 절차** (업데이트로 원복됐을 때 그대로 따라 하면 된다):

1. 활성 설치본 경로 확인 — `~/.claude/plugins/installed_plugins.json`의
   `claude-mem@thedotmack` → `installPath`(**버전이 오르면 경로가 바뀐다. 하드코딩 금지**).
2. 대상 함수 재특정 — 미니파이 단일행 번들이라 **함수명·줄 좌표는 버전마다 변한다**
   (13.10.4에선 같은 함수가 `ts()`/`ps()`였다). 앵커는 이름이 아니라 **본문 패턴**:
   `toLocaleDateString("en-CA")` + `timeZoneName:"short"` 조합. 소비처
   `` `recent context, ${<fn>()}` `` 에서 함수명을 역추적하는 편이 확실하다.
3. 백업 → 해당 함수 **본문만** 치환(호출부·시그니처·이름 변경 금지, 재포맷·재빌드 금지 —
   미니파이 번들에서 호출부를 건드리면 추적 불가능한 파손이 난다). `node --check`로 구문 확인.
4. **워커 데몬 재기동이 필수다** — `worker-service.cjs --daemon`이 상주하며 구코드를 메모리에
   물고 있어, 파일만 고치면 헤더는 그대로 분이 나온다(이번에 실제로 1회 헛확인했다):
   `node <installPath>/scripts/bun-runner.js <installPath>/scripts/worker-service.cjs restart`
5. 실측 확인 — 같은 훅을 직접 실행해 헤더 문자열을 눈으로 본다(코드 읽기로 끝내지 말 것):
   `echo '{"hook_event_name":"SessionStart","source":"startup","cwd":"<repo>"}' | node …/bun-runner.js …/worker-service.cjs hook claude-code context`
   → `# [proj] recent context, 2026-07-20`(분 없음)이면 성공. **분 경계를 넘겨 2회** 비교할 것 —
   같은 분 안에서 두 번 보면 판별이 안 된다. 메모리 주입 본문이 살아 있는지도 함께 본다.

**탐지 방법**: 1차 방어선 = **`bun run check:mem-header`**(`scripts/check-mem-header.ts`).
`installed_plugins.json`의 `installPath`를 런타임 재해석해 헤더 함수 본문의 분 시그니처를 검사하고,
설치본이 없거나 경로 해석에 실패하면 **fail-open + stderr 고지**(다른 머신에서 게이트를 막지 않는다).
육안으로는 세션 헤더에 분이 다시 보이면 원복이다. **`known_marketplaces.json`에서 thedotmack은
`autoUpdate: true`이므로 원복은 예고 없이 일어나고**, 증상은 "어느 날부터 캐시가 다시 깨짐"이라
원인 추적이 지극히 어렵다 — 그래서 절차 기록만으로는 못 막고 감지장치가 카드의 절반이었다.
