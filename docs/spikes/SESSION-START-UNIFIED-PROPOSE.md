# Propose (rev-2) — 3-CLI 공통 세션 시작 규범

| Field | Value |
|---|---|
| **Status** | **revised after Addendum B** — still **not approved / not locked** |
| **Date** | 2026-07-23 (rev-2) |
...
| **Review verdict** | initial/Addendum A = historical snapshot · Addendum B §11 = PENDING-REVISION → rev-2 addresses D1–D7; full rev-2 re-review still required |
...
| Addendum B D1–D7 | rev-1 §11 | 현재 Decision log blocker · rev-2가 문서상 반영 · 재리뷰 대기 |

**Rules:**

- historical 좌표는 반드시 `commit:path:line`으로 읽는다.
- `addressed` ≠ `verified` ≠ `design-approved` ≠ `implementation-authorized`.
- 승인 전 이 rev-2를 **immutable blob/commit으로 freeze**한다. 현재 working-tree 문구 자체는
  아직 승인 대상 식별자가 아니다.
...
| Package | Status |
|---------|--------|
| SESSION-START-DELIVERY | draft rev-2 · pending re-review/owner |
| NORMS-RECEIPT | draft rev-2 · pending re-review/owner; implementation fixture gated |
| CONTEXT-MAP | draft rev-2 (M0) · pending re-review/owner |
...
```text
buildStateRaw()   → plain text starting with [LOOM-SESSION-CONTEXT v1 · state]
buildLessonsRaw() → plain text starting with [LOOM-SESSION-CONTEXT v1 · lessons]
...
| `codex-plain` | Codex SessionStart | **plain text only** → developer context | rev-2 wire SSOT · JSON alternative는 비범위 · **token budget** §5.4 |
...
### 5.3 Host delivery matrix (F1 · F4)

| Host | N (norms) primary | S (state+lessons) primary | Accelerator | Fallback (정본) |
|------|-------------------|---------------------------|-------------|-----------------|
| **Claude** | `CLAUDE.md` + **`@AGENTS.md`** + rules | SessionStart hooks `claude-json` ×2 (state, lessons) matcher **`startup\|resume\|clear\|compact`** | same hooks | ritual: `bun run status` + HANDOFF nine + traps + lessons index |
| **Codex** | `AGENTS.md` discovery (official) | **SessionStart hooks primary** (`codex-plain`; plain stdout; hooks default on) | hooks | ritual same; feature disabled / failure → fail-loud partial + ritual |
| **Grok** | project rules auto-load AGENTS/CLAUDE (**full file, no truncate** · vendor doc) | **Ritual only** (또는 공식 launcher/initial-rules가 생기면 그때 승격) | SessionStart hook = **env/setup only** · **stdout ignored** (0.2.111) | ritual always baseline |
...
|------|--------------------------------|---------|
| **Claude** | ~10_000 **chars** silent platform cut | our `HARD_CAP` 9500 chars · 2-hook split · **end marker on self-truncate** · prefer **begin+end envelope** on normal path too (NORMS/S packages) |
| **Codex** | model-visible hook output ~**2_500 tokens** (official “Large hook output”; overflow → temp file + head/tail) | **do not** assume 9500 chars safe; prefer slim state / two hooks / ritual for full nine if over |
...
채널 head/tail(Codex large output)도 END 없으면 fail-loud.
...
그대로 둔다. `packs` digest는 assembled pack body의 SHA-256 앞 8자이며 **stale 비교용 claimed
metadata**일 뿐 LOADED 근거가 아니다.
...
|-------|---------------|----------|
| N | LOADED / STALE / UNVERIFIED / ABSENT | BEGIN/END 봉투 · 아님: disk only |
| S | full / partial / absent | state(+lessons) 센티널 · omit · ritual 완전성 |
...
| 2 | hook command exit 0 but no envelope in context → **not** full (detect fail · ritual) |
...
| 7 | Codex: over token budget → partial/fail-loud not silent full |
...
2. hook 성공 · context 실패 감지? → 봉투/센티널 부재 → partial + ritual
3. 규범 본문 적재 증명? → BEGIN/END + body
...
**Approval eligibility:** full rev-2 독립 재리뷰가 `APPROVE`여야 `approve` 선택지가 유효하다.
그 전에는 `changes` 또는 `defer`만 기록할 수 있다.
...
|---|----------|---------|------------|
| 1 | SESSION-START-DELIVERY design | approve rev-2 / changes / defer | _TBD_ |
| 2 | NORMS-RECEIPT design | approve rev-2 pack contract / changes / defer | _TBD_ |
...
**Freeze/record rule:** Owner pick이 비어 있는 rev-2를 먼저 commit/blob으로 고정하고, 위 기록은
...
`Addressed in rev-2` 또는 changelog 행은 승인을 대신하지 않는다.
...
| Phase | Package | Work |
|-------|---------|------|
| 0 | — | full rev-2 independent re-review → immutable blob/commit 고정 → Owner design picks · pre-applied A3/F7 추인 |
| 0a | DELIVERY | implementation-authorized일 때 L0 extract + AGENTS trigger split (**Phase 1 prerequisite**) |
...
| 3 | NORMS | extractor fixture → host별 budget 측정 → 합격 host만 outer+per-pack envelope enable · Template I LOADED rules · optional `/context` note |
...
| Codex hooks | **current primary** for S accel · not future (F4) |
