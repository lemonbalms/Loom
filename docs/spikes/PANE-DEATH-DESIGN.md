...
(전송을 이미 시도했으므로 멱등성 없이 재발행 금지). durable 레코드가 진입 시점부터 존재하므로 in-memory
해제는 **정보 파괴가 아니다.** **타이머 유발 dispose는 금지** — 10분 타이머는 **재알림(re-escalate)만**
...
아래 전이표의 각 행은 `logicalKind`가 결정한다.

| logicalKind | 현 상태 | 이벤트 | 다음 |
|---|---|---|---|
| `initial` | 구성 | send → strict ACK 3분기(락 5) | `relay_accepted`(종결) · **명시적 거부(→ 다음 행)** · `send_unknown`(흡수 · 종결) |
| `initial` | 명시적 거부(`needs_verification` 거절) | `rejection_escalation` 재구성 **1회** | ↓ |
| `rejection_escalation` | failed send | strict ACK | `relay_accepted`(종결) · `send_unknown`(흡수 · 종결) |
| (양쪽) | transport/ACK 유실 | — | `send_unknown`(흡수 — 종결) |

**pre-Flight `initial failed` 직행 (codex M-new-3 수용):** `:850 :1114 :1204`의 Flight-less 3경로가 내는
result는 `logicalKind=initial`이고 **처음부터 `status=failed`**다. 이 경로는 **"명시적 거부 →
...
- 스캔 폴백도 miss면 기존대로 `task not found`로 종결하되 **관측 카운터에 남긴다**(조용한 폐기 금지).
...
**게이트(같은 PATCH):** 위 폴백으로 태스크가 해석된 뒤 —
