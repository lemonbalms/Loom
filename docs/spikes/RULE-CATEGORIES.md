# 카테고리 표 — **오너 선포 정본** (Phase 1 산출물)

작성 2026-07-26 · **오너 승인 2026-07-26 (권고안 그대로)** · 정본 근거: [`RULE-ROUTER-PROPOSE.md`](./RULE-ROUTER-PROPOSE.md) rev-6 §5.2.1 · §10 D9
상태: **approved — 이 표가 카테고리 정본이다.** 변경은 새 오너 선포를 요구한다.

> D9 확정(2026-07-26): **카테고리 정본은 오너 선포**이며, 초안은 기존 축에서 도출한다.
> **오너 선포(2026-07-26): §3의 권고안을 그대로 채택** — 카테고리 10개 확정 · 재량 pin **0건** ·
> 예외 매핑 **없음**. **표 자체는 여전히 감사 대상**이다(§5.2.1 — 오너 저작이라고 평가 면제 없음):
> replay가 “그 카테고리 턴에서 실제로 필요했던 룰이 매핑에 있었는가”를 감사한다.
>
> **승인의 범위**: 이 표는 Phase 2 replay의 **명세 ground truth**로 쓰인다. **주입 경로는 바뀌지
> 않는다**(§5). Phase 2 착수는 별도로 D7 3단 절차(M7 임계 봉인)를 선결로 요구한다.
...
## 3. 오너 선포 — 3건 **전건 확정 (2026-07-26 · 권고안 그대로)**

| 항목 | 확정 |
|---|---|
| 3.1 카테고리 목록 | **10개 그대로 선포** |
| 3.2 재량 pin | **0건 — 선포하지 않는다.** pinned는 자동 pin 13건뿐 |
| 3.3 예외 매핑 | **없음 — `surface` 조인만 사용** |

각 항목의 근거는 아래 그대로다(승인 전 권고문이 그대로 정본이 되었다).
...
[RULE-CATEGORIES] categories=10 units=32 pinned=13 owner-pin=0 exceptions=0 status=approved owner=2026-07-26
