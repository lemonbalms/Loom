...
**Review impact:** result 발행 소유권(issuer 중앙화)·ACK 판정·seq 규약(소유 Flight→issuer)·**tower currency 게이트**·**durable quarantine** 변경. **relay wire·MCP·herdr RPC 표면은 무변경**이나 `sendResult` 시그니처 교체가 `finishCard` pane.close 게이팅과 `pane-cleanup.test.ts:245`·`:816`을 함께 움직이고, payload 멱등키 각인은 **tower inbox 영속이라 비가역**이며, tower currency 게이트는 **tower-local 적용 판정**(`card-ops.ts` — host)을 바꾼다. 설계 정본 [`docs/spikes/PANE-DEATH-DESIGN.md`](spikes/PANE-DEATH-DESIGN.md) §6.7·**§6.7.1·§6.7.2·§6.7.3**·§6.7-bis·§9-bis 락 5·8·9·13. R43 검증 원장 [`docs/reviews/PANEDEATH-R43.md`](reviews/PANEDEATH-R43.md)(정본·수정 금지) — 본 개정이 그 High 6·Medium 5를 해소한다. **R43b 검증 원장 [`docs/reviews/PANEDEATH-R43B.md`](reviews/PANEDEATH-R43B.md)(정본·수정 금지)** — 2레인 검증(pending-revision·reject) + 아키텍트 판정 표 + 2차 개정(신규 High 4·Med 3·Low 3 국소 정밀화). **R43c 검증 원장 [`docs/reviews/PANEDEATH-R43C.md`](reviews/PANEDEATH-R43C.md)(정본·수정 금지)** — R43c 2레인 검증(pending-revision·reject) + 3차 개정(시대 매개변수화·전이표 분할·CAS 개명·태그드 키 전파·폴백 도달성 등 문면 전파, 신규 High 0). 3차 검증 원장 [`docs/reviews/PANEDEATH-CODEX-REVIEW3.md`](reviews/PANEDEATH-CODEX-REVIEW3.md)(정본·수정 금지). **R43d 필수(경량 델타 — 잔여가 문면 전파) — author-close 불가**(WORKFLOW §5.1 신규 MINOR + 신뢰 경계, 설계 §8 `rgate=needed`; R43c는 2레인 검증 pending-revision, 신규 High 0·문면 전파 반영 후 재검증).
