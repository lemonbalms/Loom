...
**dist·커밋·push**: 이번 ship 웨이브에서 소스 커밋 → `build:cli` → dist 커밋 → push 수행(분류기 차단 시 오너 `!`, lessons (15)⑤). **잔여(ship 후 선택)**: 라이브 스모크 — `hookSensor: true` 노드에서 승인 대기 카드로 `permission_prompt` hookHint 실발화·가짜 `agent_blocked` 1:1 교정·`Stop` 유예 입력 실발화를 empirically 확인(**U2 `--settings` 인라인 JSON 주입 실작동 방식 확정 포함** · benign 페이로드·읽기 전용).
