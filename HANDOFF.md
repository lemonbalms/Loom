...
> v0.28.1 · RULE-ROUTER **F1d 완료** — 준수 **4/12** · 대조군 **0/3** · **T1 성립**(규칙은 실린다) · next = **§5.3 문안 개정**.
...
| Harness | NORMS P3 · RULE-ROUTER **P2 R1 · F1 · F1b/c · F1d done** | runtime/tests · rev-11 |
...
**Goal:** F1d가 **준수 가능성을 성립**시켰다(T1 · 4/12 대 0/3). Phase 3의 **유일한 선결**인 §5.3 문안 개정을 연다 — rev-10 "호출 직전 = 직접 처방" 철회 자리를 무엇으로 채울지.
...
**Must not:** 단일 관측·n=3 부호에서 처방 뽑기(rev-10 철회 선례) · T2/T3 부호를 효과 크기로 인용 · F1b/F1c/F1d 봉인값 변경 · 리뷰 없이 Phase 3 착수 · holdout 개봉.
...
|---|---|---|---|
| **카테고리 표 개정** (S1-3) | `session-start`가 AGENTS standing-rules 유닛(verify·env·impl-deleg)을 미조인 — 표는 **오너 선포 정본** | 현행 유지(손실 0) | 결과 §4·§6 |
...
- F1 블록 = `<system-reminder> PreToolUse:<tool> hook additional context:`. F1b/F1c 봉인 `49cc5e9`·`171e063` · 증거 `~/.loom/f1-poc-2026-07-26`.
- F1b 채점 결함: `COMPLY`=문자열 포함 → **거부를 준수로 14/14 위양성**. 수정 코드(마지막 줄 일치)는 관측 전 선언 — 결론은 가설에 **불리**하게 이동(11/18→0/18).
- F1d: 하네스 `~/.loom/f1d-poc-2026-07-26` · 채점기 digest `14fb41179b317d88`(관측 전후 동일) · 자기검증 9/9. **한 축 대조**(임의 토큰→맥락값): F1b `norm` 0/9 → F1d `verify` 2/6.
- PREREG rev-4 sealed · receipt `rules/prereg-sample.json` · 아카이브 `~/.loom/prereg/rule-router-2026-07-26`(72MB·60세션).
- PREREG 계산값: pinned 2,586 · routable 5,042 · 전량 7,628 · 최악 합집합 5,300 → M7b=0. **자문 N1 해소**(F1 §3: 캡 10,000/커맨드 · 총량 캡 부재 = prefix·JIT 별도 슬롯).
- Phase 2 R1: 분류 입력 = 발화만(무누설) · receipt 2회 바이트 동일 · 미스 62/113 전량 **리추얼 턴 3유닛**(파일 라벨) · 턴당 26.1유닛/6,167자.
...
- Re-measure F1 delivery (6 probes); treat delivery as compliance; re-run an arbitrary-token canary; read 0/21 as "JIT cannot carry rules" (**F1d refuted it**); reopen the channel hypothesis (F1c rejected it).
- Read F1 YES or F1d T1 as Phase 3 authorization; keep §5.3's "호출 직전 = 직접 처방" or rev-10's prescription (both retracted); cite T2/T3 signs as effect sizes (n=3); change a sealed F1d variable; treat 4/12 as more than a lower bound.
