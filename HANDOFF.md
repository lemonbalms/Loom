...
> 순서: 멀티노드 트랙 진행 중 — ⓪ 단독 모드·① Windows relay 복귀·①-b relay 룸 영속화·② WSL 노드 브릿지 복제·③ Linux/VPS 노드 브릿지 복제·④ `@node` 라우팅 운용(노드 3식 mac-node·node-wsl-1·node-vps-1 ROUTE 카드 3발 라이브) 완주, **다음 = ⓐ artifact fetch 자동 실행 도입**(R{n} 재리뷰 필수).  
