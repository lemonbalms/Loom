...
> **최신:** **R35** PLAN **v0.23.11**(잔여 후보 일괄: claude 상태줄 chrome 필터 + summary 타이밍줄 선별 + 풀 탭 동시 스폰 직렬화 + still-running supersession — 후보 ①③④⑤, PATCH) **`pending-revision` → author-close `approved`**(2026-07-19, M-1 lock 반영, no R35b) — ①(실측 시그니처 ` │ `+`⚡`/`🧠`·R31 M-1 경계 유지 코드 확인)·③(앵커드 타이밍-전용 전체-매치 스킵·현행 폴백 회귀 안전)·④(비원자 실사→스폰 구간 `bridge-runtime.ts:738→:792` 코드 실재·promise chain 직렬화 bounded·R34 L-1 대체 계보 정당(탭 중복 생성 = 상위 클래스 + 오너 지시)) 총론 전부 타당, ⑤ 원인 규명(스테일 지표줄 영구 잔존 — 프로브 실측)·supersession 방향도 성립. 단 **M-1: ⑤ 오소거 클래스의 실제 최악이 스펙 서술("0.23.7-이전 미완 회신 1회")과 다름** — supersession 소거는 확신-완료 경로(`:1641-1651`/`:1697-1709`)를 타 `closePane:true`로 배송되므로 **진행-중 커맨드 pane kill 포함**(R33 M-1이 명시 배제한 결과·auto-close는 0.23.8 도입이라 "0.23.7-이전 동작" 서술 사실 아님)이고, 비앵커드 substring 매치(tail-10 joined 임의 위치)는 라이브 증거(독립 줄 "Worked for 25s.")보다 과도하게 느슨해 이 저장소 워커가 일상 출력하는 인용·픽스처 문자열로 발화 가능(0.23.11 구현 카드 자체가 첫 후보). lock: supersession 줄-앵커드 전환(③ 공유 앵커드 상수 단일화·마지막 지표-포함 줄 이후 줄 인덱스·위치 특정 실패 시 미발동 보수 폴백) + 잔존 수용/Security 최악 서술 정정(pane kill 명시) + 경계 테스트 2건(인용 비-supersession·랩 분단 비-supersession). **M-1 반영 후 재리뷰 없이 `approved` 전환 가능 (Fable 사전 승인, no R35b).**  
> **직전:** **R34** PLAN **v0.23.9**(conv done_proposal 규약 완결 + conv.open deny 클레임 정합 + 브릿지 pane 배치 정책 — 후보 ②③⑧, PATCH) **`pending-revision` → author-close `approved`**(2026-07-19, M-1 lock + L-1·L-2 반영, no R34b) — ③(무클레임 deny 호이스트 — 카드 관례 동형·처리 표면 축소)·⑧(풀 탭 정책 — 프로브 실측 전제·fail-open·root close 불변식·opt-out) 총론 타당, 보안 3항목(② 최악=보드 노트 표시뿐 · ③ 표면 축소 · ⑧ 신규 원격 표면 없음) 전부 코드로 성립. 단 **M-1: ② 규약 문구("최종 메시지 첫 줄")와 탐지기 의미론(delta **전체** 선두 `trimStart().startsWith`, `bridge-runtime.ts:1232`)이 불일치** — delta는 직전 턴 꼬리 앵커 이후 신규 콘텐츠 **전부**(타워 턴 에코 + 중간 출력 + 최종 메시지, `:1198`/`:223-248`)라서 규약-준수 워커도 통상 턴에서 구조적 미탐지(에코 존재는 inject 검증 `isInjectProbeHit` 자체가 의존하는 실측 사실 `:261-267`) → "탐지 로직 무변경" 전제가 ②의 명시 목표(라이브 도달 가능화)와 자기모순. lock: 탐지 = delta **말미 비공백 K줄 line-anchored** 판정으로 변경(스코프 명시) + 판정 입력 chrome-필터본 통일 + 규약 문안은 마커를 줄-선두에 두지 않기(`[ARTIFACT]` 선례) + 규약-준수 시나리오 테스트. L-1(⑧ 폴 중첩 동시 스폰 race — 탭 4 초과 cosmetic)·L-2(⑧ 재시작 후 풀 탭 재발견 불가 — 잔존 누적 cosmetic) author-close. **M-1 반영 + L author-close 후 재리뷰 없이 `approved` 전환 가능 (Fable 사전 승인, no R34b).**  
