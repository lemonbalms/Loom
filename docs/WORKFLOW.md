...
세션 리추얼(읽을 파일 순서·브리핑 형식)의 정본은 루트 **`AGENTS.md`** § "Session start ritual" 이다 (Claude는 `CLAUDE.md`도 참조). 아래는 도구별 진입 경로 보충.
...
브리핑 형식(상태 표 템플릿, 트레일링 질문 금지 등)의 정본은 루트 **`AGENTS.md`** § "Brief the user immediately" 이다.

사용자가 이미 매우 구체적인 명령만 준 경우(예: “이 파일만 고쳐”)는 브리핑을 **짧게** 하고 바로 실행해도 된다.  
「진행해」「이어서」「단계적으로」「자율적으로」의 의미는 **§3** 참조.
...
현재 게이트 상태의 SSOT는 **`HANDOFF.md`** 와 **`bun run status`** 다 — 여기에 하드코딩하지 않는다.
...
| 사용자 말 | 에이전트 행동 |
|-----------|----------------|
| **진행해 / 단계적으로 진행해** | §3 웨이브 규칙대로 다음 단계 실행 (승인 대기 없음) |
| **리뷰해 / plan_review 피드백 / Fable 리뷰 / R{n}** | Fable 5(또는 동등) 게이트: PLAN+코드 대조 → `plan_review.md` R{n}; **필수 상황 §5.1**. Claude Code dogfood: **`fable-advisor` 서브에이전트 필수** → [`DOGFOOD_LOOP.md`](./DOGFOOD_LOOP.md) |
