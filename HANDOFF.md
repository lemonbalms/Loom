...
> **⭐ MVP 종료·프로덕션 전환 (오너, 2026-07-19).** 멀티노드 단계 3(0-c) — ⓪ 단독 모드 기능화 **완주(v0.24.0 `b508a4c`, 2026-07-19 오후)** → ① Windows relay 복귀 **완결(D10, 2026-07-19 오후)**: v0.24.0 라이브 실증 5종(M-1 가드·구 SGLR 신원 흡수/D1 마이그레이션·relay use 왕복·병존·claude-impl `--as local`) + LOOM-SGLR 보드 북키핑(35/35 done, R31 미클레임 종결) + **오너 승인 후 유예분 완결**: Windows loom-dev 룸 재수립(초대 코드 **LOOM-QSFP**·바인딩명 `win`) + 로스터 7종 win 바인딩 흡수(local+win 2바인딩 병존·활성=local) + sticky host 전 복원. **⚠️ 룸 소실 원인 확정**: relay 재시작(로그온 트리거)으로 인메모리 룸 소실 → 재로그온 시마다 룸 재수립 필요 → **relay 룸 영속화 신규 후속 후보**. **최전선 = ①-b relay 룸 영속화 웨이브**(오너 지시 2026-07-19 "룸 영속화 먼저 진행" — ② WSL 노드보다 **선행** 확정; 대상 `packages/relay/src/server.ts` 디스크 영속화, 배포 시 Windows relay 재배포+룸 재수립 1회 수반, R38 리뷰 가능성) → ② WSL 노드 브릿지 복제 → @node 라우팅·SSH/git 전송 자동화 점진 도입. npm publish는 오너 계정 대기(0-a).
>
...
| Item | Value |
|------|--------|
| **CLI / code** | **0.24.0** — 단독 모드 기능화(relay별 신원 병존 `relays`·`relay use <name>` 명시 전환·`--as` 멱등 명명·이종-relay 파괴 가드·`relay list`·`relay local start\|stop\|status`, `b508a4c`+dist `2c513c9`) + claude 상태줄 chrome 필터·summary 실내용 선별·스폰 직렬화·still-running supersession(0.23.11) + pane-inject.sh + 워커 풀 탭 수평 배치(0.23.10) + done_proposal 규약 완결·conv.open deny 정합·풀 탭 배치(0.23.9) + pane 정리 정책·conv-hosts CLI(0.23.8) + still-running 유예(0.23.7) + 스크레이프 delta화·chrome 필터(0.23.6) + 주입 verify 3분기(0.23.5) + conv 멀티턴 + artifact 트리거 + agentKind 3종 |
| **PLAN** | **v0.24.0** `approved` (R37 author-close, no R37b) → **implemented** (`b508a4c` + dist `2c513c9`) · D10 라이브 실증 **완료**(0-c ① Windows relay 복귀 완결 — loom-dev 룸 재수립 LOOM-QSFP·로스터 7종 win 바인딩 흡수) |
| **Open blocking** | none — R24–R37 모두 closed · GitHub Issues 전부 closed |
| **Tests** | `relay-bindings.test.ts` **29 pass / 0 fail** · 6 pkg typecheck green (전체 스위트는 HEAD 베이스라인 차집합 판정 — 환경성 실패 클래스, lessons (10)) |
