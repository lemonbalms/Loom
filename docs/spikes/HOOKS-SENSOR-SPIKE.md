...
| 이벤트 합류부 | `onCardHerdrEvent`(`bridge-runtime.ts:1967-2010`) — `:1974` status 계산 → `:1978` working / `:1992` blocked → `finishCard("failed","agent_blocked")` / `:2000` done\|\|idle&&sawWorking → `beginCardCompletion` | hookHint 결합 지점 (명제별 — 전역 우선순위 아님, 락 12) |
...
4. **hookHint 적재 + 명제별 결합** — 수신 이벤트를 `flight.hookHint`에 적재 →
   `onCardHerdrEvent`(`:1974`) · still-running poll에서 참조. `Stop`= **completion 재평가
   트리거 / poll 조기 실행**(P2에 `C+`) · `Notification` permission_prompt=승인 대기(codex 가짜
   `agent_blocked` 교정, lessons bridge-ops 관찰 ⓔ).

   > **⚠️ 정정 (2026-07-21 · (C) 전환).** 구 문안은 **`hookHint 우선`(전역 우선순위)** 과
   > **`Stop`=완료 확정(스크레이프 상한 우회)** 이었다. **둘 다 틀렸다** — 코드가 옳았다
   > (`hook-sensor.ts:169-171` `never Stop alone`). `Stop`은 취소·에러 턴에도 발화하고
   > SIGKILL이면 발화 자체가 없으며 발신자가 워커 자신이다. 전역 우선순위는 폐기됐고
   > 결합 정본은 `PANE-DEATH-DESIGN.md` **§6.0-bis 증거 명제표**(락 12)다.
...
`onCardHerdrEvent`(`:1974`)와 still-running poll에서 **증거 명제표**(`PANE-DEATH-DESIGN.md`
§6.0-bis · 락 12)에 따라 **명제별로** 결합. ~~"hookHint 우선, 없으면 herdr status + 스크레이프"~~
— 전역 우선순위는 (C) 전환으로 **폐기**됐다(2026-07-21).
...
| 조기 `card.done` | still-running 휴리스틱(최대 5분 유예) | `Stop`으로 **재평가 조기 실행**(유예 단축) — 완료 확정 아님(락 12) |
