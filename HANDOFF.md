...
> v0.28.1 · RULE-ROUTER **F1b·F1c 완료** — 준수 **0/21** · 채널 가설 기각 · **측정 도구 무효** · next = **F1d 규범형 카나리아**.
...
| Harness | NORMS P3 · RULE-ROUTER **P2 R1 · F1 · F1b/c done** | runtime/tests · rev-11 |
| Reuse | not proven | evidence |

## Current action

### RULE-ROUTER — F1d: **규범형** 준수 카나리아 (도구 재설계)

**Goal:** **준수 가능성 자체가 미측정**이다. 21/21 거부는 라우터가 아니라 **카나리아 결함** — 준수를 관측 가능하게 만든 **임의 토큰**이 곧 인젝션 방어를 발화시켰다.
...
**Done when:** 사전등록 봉인 → 준수율 판정 기록. 그럴듯한 규범도 거부되면 **주입으로는 규칙을 못 싣는다**가 확정된다(그것도 결과다).

**Must not:** F1b/F1c 봉인값 변경 · 임의 토큰 카나리아 재사용 · 준수를 문자열 포함으로 정의(위양성 14/14) · 리뷰 없이 Phase 3 착수 · holdout 개봉.
...
| **F1 계열** (27런) | 비차단 주입 **YES**(**C1** `tool_result` 동봉 · **C2** 서브에이전트 미도달 · **C3** 10,000자/커맨드) · 전달 21/21인데 **준수 0/21** · 채널 가설 **기각** | **도구 무효**(임의 토큰=인젝션) · Phase 3 = 문안 개정 선결 | `F1-RESULT` · `F1B-RESULT` |
...
|---|---|---|---|
| **카테고리 표 개정** (S1-3) | `session-start`가 AGENTS.md standing-rules 유닛(verify·env·impl-delegation)을 미조인 — 표는 **오너 선포 정본** | 현행 유지(안전 손실 0) | 결과 §4·§6 |
...
- F1 블록 = `<system-reminder> PreToolUse:<tool> hook additional context:`. F1b/F1c 봉인 `49cc5e9`·`171e063` · 증거 `~/.loom/f1-poc-2026-07-26`(12MB).
...
- Phase 2 R1: 분류 입력 = 발화만(무누설) · receipt 2회 바이트 동일 · 미스 62/113 전량 **리추얼 턴 3유닛**(파일 단위 라벨) · 턴당 26.1유닛/6,167 chars.
...
- Re-run an arbitrary-token canary (invalidated, 0/21); read 0/21 as "JIT cannot carry rules"; reopen the channel hypothesis (F1c rejected it); keep rev-10's norm-vs-imperative prescription (retracted).
