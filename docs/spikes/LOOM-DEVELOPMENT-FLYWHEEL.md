...
| **Ordering decision** | SESSION-CONTINUITY bounded Phase B→C → PANE-DEATH PATCH 1 → 실제 전환 2회 → Phase D 검토 |
...
| 축 | 현재 위치 |
|---|---|
| **Product** | `docs/PLAN.md` v0.28.0 approved · PATCH 1 착수 전 clean boundary에서 bounded supporting wave 동안 일시 정지 |
...
**Owner ordering decision (2026-07-22):** 이 supporting loop는 PANE-DEATH를 대체하지 않는다.
Phase B fixture와 Phase C 원자 전환·새 세션 복원 스모크까지만 먼저 수행하고, 통과 즉시 primary의
PATCH 1로 복귀한다. Phase D 자동화는 실제 PATCH 전환 2회가 검증 증거를 만든 뒤에만 재개한다.
...
Mission·ROADMAP·portable 정책에 대한 나머지 결정 전까지 본 문서는 proposal이다. 단, supporting
loop의 bounded 선행 순서는 Owner가 2026-07-22 확정했으며 현재 HANDOFF가 실행 순서의 정본이다.
