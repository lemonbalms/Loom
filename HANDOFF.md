...
> **🎯 D10 라이브 실증 (2026-07-19 오후 — 0-c ① Windows relay 복귀 부분 수행) — v0.24.0 단독 모드의 첫 라이브 실증 5종 + 보드 북키핑 완료, ① 완결은 오너 직접 실행 유예.** v0.24.0(단독 모드 기능화) 산출물을 라이브에서 실증: **① R37 M-1 가드 발화** — claude-impl에서 relay-name 미지정 이종-relay join(로컬→Windows) 시도 → **fail-closed 거부·기존 바인딩 무손상** 실증. **② 구 LOOM-SGLR 신원 흡수** — 백업(`profiles.bak-sglr-20260719`의 `claude-impl.json`)을 스크래치 프로필 `sglr-bookkeep`로 복원 → `relay use --as sglr` 명명 성공: **pre-0.24 세션이 v0.24.0 CLI를 무변형 통과(D1 additive 마이그레이션 라이브 실증)**, 구 peerId `p_ed676195eecd9488` 유지. **③ 보드 북키핑** — LOOM-SGLR 보드(`boards/b24105986c5b3d96.json`)의 유일 비종결 카드 `task_87fca9d2f36af224`(R31 리뷰, doing 고착)를 **원 생성 peer 신원으로 done 전이** → 보드 35/35 done·비종결 0(R31 card.done 미클레임 종결). **④ relay use 왕복 전환 실측** — sglr-bookkeep에 로컬 relay 스로어웨이 룸 `d10-roundtrip`(초대 LOOM-CQDB)을 `--relay-name localrt`로 생성(이종-relay 가드 경유) → **2 바인딩 병존**(sglr=Windows 구 peerId / localrt=local 신규 `p_df6014a853bf6f41`) → `use sglr`→`use localrt` 왕복: **양방향 전환·무손실 복원·활성 표시 정확 전이 + sticky host 구동-중 경고 경로 라이브 발화**. **⑤ claude-impl 실프로필 v0.24.0 첫 적용** — `relay use --as local` 명명 완료. **⚠️ 중대 발견 — Windows relay 룸 레지스트리 소실**: LOOM-SGLR(loom-dev)·LOOM-4HXU(demo) 둘 다 **"No room for code"** — 접속·토큰 인증(101)은 정상. 룸/초대 레지스트리는 **relay 인메모리로 추정**, relay 재시작(Scheduled Task `LoomRelayTeam`은 **로그온 트리거**) 시 소실. `loom up --status`의 "mac @ demo online"은 **프로세스 생존 표시일 뿐 룸 존재 보증 아님**. 구 룸 부활 불가 → Windows relay 위 팀 룸은 **신규 생성으로 재수립** 필요(원인 확인은 Windows `%USERPROFILE%\.loom\relay.log`·`relay.err.log`). **버전 스큐 무관 확정**(PROTOCOL_VERSION=1 동일, `envelope.ts:4` + 오늘 인증 왕복 실측). **유예분(세션 권한 제약)**: 원격 relay(100.65.103.113) 대상 room create/join CLI가 이 세션의 권한 분류기에 **반복 차단**(로컬 relay 대상은 통과) → claude-impl의 Windows 바인딩(`win`) 흡수 + 신규 loom-dev 룸 수립은 **오너 직접 실행** 필요. **절차**: claude-impl 프로필로 Windows relay(ws 100.65.103.113 포트 7842)에 room create → 룸명 `loom-dev`·바인딩명 `win`(`--relay-name win`)·토큰은 셸 env `LOOM_RELAY_TOKEN` → 나머지 피어 프로필도 새 초대 코드로 `--relay-name win` join → 0-c ① 완결 → 다음 = ② WSL 노드 복제. **정리 상태**: 스크래치 프로필 sglr-bookkeep 삭제(신원은 백업 디렉터리 보존)·그 sticky host 중지. claude-impl sticky host 재기동(pid 68567, loom-local). mac-node 브릿지 무접촉(pid 45974). 스로어웨이 룸 d10-roundtrip 로컬 relay 잔존(무해).
>
...
> 0-c. **⭐ 다음 제품 트랙 = 멀티노드 단계 3 (오너 확정 2026-07-19: "다음 트랙에서 진행" + MVP 종료·프로덕션 전환)** — Loom×Herdr 권고 문서(`~/Downloads/loom-herdr-architecture.html`)의 수직 슬라이스 로드맵 중 0~2는 완료(STEP0 PoC·0.22.0 슬라이스·MCP 카드 도구), **잔여 = 단계 3 노드 복제·멀티 OS 확장**: ~~⓪ **단독 모드 기능화** — `loom relay use <name>` 명시 전환·프로필 relay별 신원 병존·`loom relay local` 라이프사이클~~ → **완주(v0.24.0 `b508a4c`+dist `2c513c9`, 2026-07-19 오후 — R37 approved → grok 3차 구현 → 독립 검증 29/0·typecheck 6/6. 위 Current action 참조).** → ① Windows relay 복귀를 **이 기능(v0.24.0)의 첫 라이브 실증으로** 수행 — **부분 완주(2026-07-19 오후, D10 — 위 Current action 참조)**: v0.24.0 산출물 라이브 실증 5종(M-1 가드·구 SGLR 신원 `--as sglr` 흡수/D1 마이그레이션·relay use 왕복·병존·claude-impl `--as local`) + 보드 북키핑 완료(LOOM-SGLR 35/35 done, R31 card.done 미클레임 종결). **⚠️ 중대 발견**: Windows relay 룸 레지스트리 소실(LOOM-SGLR·LOOM-4HXU 둘 다 "No room for code" — 접속·인증 101 정상, 룸은 relay 인메모리 추정·재시작 시 소실) → **구 룸 부활 불가, 신규 생성 재수립 필요**. **잔여(유예 — 오너 직접 실행)**: 원격 relay 대상 room create/join CLI가 세션 권한 분류기에 반복 차단 → claude-impl Windows 바인딩(`win`) 흡수 + 신규 `loom-dev` 룸 수립은 오너가 직접(절차: claude-impl 프로필 → Windows relay(ws 100.65.103.113:7842) room create → 룸명 loom-dev·`--relay-name win`·토큰 env `LOOM_RELAY_TOKEN` → 나머지 피어 새 초대 코드로 `--relay-name win` join). 참고: Windows relay 프로세스·토큰 인증 101 가동 실측(2026-07-19). → ② WSL 노드 브릿지 복제 → ③ Linux/VPS 노드 → ④ `@node` 라우팅 운용. 트랙 내 **SSH/git 전송 자동화 유예분 점진 도입**: ⓐ artifact fetch 자동 실행(현행 "제시까지" — 도입 시 R26 M-1/M-2 High 승격이라 **R{n} 재리뷰 필수**, plan_review R26:435) ⓑ 브릿지 자동 git push(R26:431 유예). 참고: SSH를 herdr **제어 전송**으로 쓰는 안(배치 (b))은 기각 확정(장기 이벤트 스트림 취약) — 유예분은 **artifact 전송 자동화**에 한정. **프로덕션 단계 기준**: 신규 표면은 §5.1 게이트 보수 관례 유지 + 팀 실사용(6인) 안정성이 완료 기준.
...
> **⭐ MVP 종료·프로덕션 전환 (오너, 2026-07-19).** 멀티노드 단계 3(0-c) — ⓪ 단독 모드 기능화 **완주(v0.24.0 `b508a4c`, 2026-07-19 오후)** → ① Windows relay 복귀 **부분 완주(D10, 2026-07-19 오후)**: v0.24.0 라이브 실증 5종(M-1 가드·구 SGLR 신원 흡수/D1 마이그레이션·relay use 왕복·병존·claude-impl `--as local`) + LOOM-SGLR 보드 북키핑(35/35 done, R31 미클레임 종결) 완료. **⚠️ Windows relay 룸 레지스트리 소실 발견**(인메모리 추정·재시작 시 소실 — 구 룸 부활 불가, 신규 재수립 필요). **최전선 = ① 잔여**(오너 직접 실행 유예 — 세션 권한 차단: claude-impl Windows 바인딩 `win` 흡수 + 신규 loom-dev 룸 수립) → ② WSL 노드 복제·@node 라우팅·SSH/git 전송 자동화 점진 도입. npm publish는 오너 계정 대기(0-a).
>
...
| **PLAN** | **v0.24.0** `approved` (R37 author-close, no R37b) → **implemented** (`b508a4c` + dist `2c513c9`) · D10 라이브 실증 **부분 완료**(오너 유예분: Windows 룸 재수립) |
