...
- [x] 1줄 설치(`bunx` / `bun install -g github:lemonbalms/Loom`) · self-contained invite blob · `loom doctor` — 완비
- [x] relay 재시작 내구(0.14.x) · 배관 검증(Docker/LAN 하네스)
- [x] **Windows 온보딩 경로 = WSL 안내** (Step 2 참조). bun/git 인증 전제 → Windows 팀원은 WSL(Ubuntu) 안에서 동일 2줄 실행.
...
전제: [Bun](https://bun.sh) 설치됨 + private repo `lemonbalms/Loom`에 git/gh 인증됨.

**macOS/Linux 팀원 — 딱 2줄** (택 1로 설치, 그다음 join):

```bash
# 설치 (택 1)
bun install -g github:lemonbalms/Loom          # PATH에 loom 등록
# 또는 매번: bunx --bun github:lemonbalms/Loom loom …
...
2. 재부팅 후 **Ubuntu 터미널**에서 Bun 설치 + macOS/Linux와 동일한 2줄:
   ```bash
   curl -fsSL https://bun.sh/install | bash && exec $SHELL
   bun install -g github:lemonbalms/Loom
   loom room join "loom://join/<받은-blob>"
   ```
...
제품 코드를 수정·기여할 때만. 팀 온보딩 기본 경로가 아님.

```bash
git clone git@github.com:lemonbalms/Loom.git
cd Loom && bun install
bun run loom room join "loom://join/<받은-blob>"
