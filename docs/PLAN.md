...
#### 0.28.0 — 2026-07-21 (`pending-review` **R45 필요 — author-close 불가** — **완료 권한 컷 — 브릿지 자동 `done` 폐지 · tower ingress `blocked` 격리 · card 자동 `pane.close` 0곳 · 비수락 ACK 단일 흡수** (MINOR))
...
| **G-10** | flight map miss inert 처리의 **구조 로그**: 발행 후 map에서 사라진 pane의 herdr 이벤트가 inert 처리될 때 structured log/counter 1건(조용한 return 0건) | 설계 §0.2 D10 · 구 §6.2 규칙 7 — **본 세션 신설**(R44 L4 반영) |
...
| ④ | R44 / 설계 §8-5 | quarantine `process_exit` fold의 원 구현 의도 | **존속** → UK-3 · R45 재확인 |
...
| **⑨** | R44 보고자 추가 | **U8 ↔ R43b §4 "락 11 재론 기각" 충돌 여부.** grok은 충돌 없음으로 봤으나 **근거가 얇다** — 정합 표 한 줄뿐이고 R43b §4 **축자 대조 미실시** | **존속 · 최우선.** **codex 레인 복구 후(리셋 2026-07-25 14:27) 가장 먼저 재검증**한다 — 락 U8 철회의 정당성이 걸린 **유일한** 미검증 항목이다 |
