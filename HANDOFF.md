...
> **🎯 0-c ③ Linux/VPS 노드 브릿지 복제 완주 (오너 위임, 2026-07-19 저녁).** 순수 ops 웨이브(제품 코드 무변경·R{n} 불요, ② 선례 승계) — Tailscale `kb`(100.116.39.101, Ubuntu 24.04.4 LTS, x86_64, **non-root zerocode·sudo 불가**) 세 번째 노드 브릿지 online·스모크 1차 클린 완주 `[SMOKE-VPS1-OK] head=d1c0dd9 loom=0.24.2`. 셋업(bun 1.3.14·herdr 0.7.4·loom v0.24.2 `git bundle` scp→`git clone`+`bun link`, **GitHub 접근 불요**·claude 2.1.215+Keychain 이전·busybox unzip shim) → **sudo 불가 3함정**: PATH는 `/usr/local/bin` 불가라 `~/.bashrc` 가드 앞 PATH 블록 삽입 · **bridge 기동 cwd `~/.loom-src` 필수**(bridge-spawn.ts:29 2단 해석, 홈 기동은 "Cannot find bridge-main.ts" 즉사) · **Option B′ 파일-복제 이식**(경로-독립 settings.json + 신뢰 2키 python 병합, 스크립트화). room join LOOM-GT4B node-vps-1 `p_aadcd1e3dc9c5b5a`·bridge config 직접 작성 no `--allow`·데몬 setsid 분리(pid 541727). 분류기 차단 8건 오너 `!` 직접 해소(WSL 6회보다 폭 확대, Loom 승인 ≠ Claude 분류기 재실증). **최전선 = 0-c ④ `@node` 라우팅 운용**(노드 3식). 후속 후보: kb 부팅 생존 상시화(non-root `systemd --user` 후보)·summary 하단 상태줄 스크레이프(표시 아티팩트, VPS 동형). npm publish는 오너 계정 대기(0-a).
...
> 순서: 멀티노드 트랙 진행 중 — ⓪ 단독 모드·① Windows relay 복귀·①-b relay 룸 영속화·② WSL 노드 브릿지 복제·③ Linux/VPS 노드 브릿지 복제 완주, **다음 = ④ @node 라우팅 운용**(노드 3식: mac-node·node-wsl-1·node-vps-1).  
