...
| **Fable 5 when** | **R38 요청 대기(2026-07-19)**: 0.24.1 relay 포그라운드 durable 배선 — 보안·신뢰 경계(peerSecret 평문 스냅샷 적용 범위 확대) R38 필수(PATCH여도). 직전: **R37 완료(2026-07-19)**: `pending-revision` → **author-close `approved`**(M-1(이종-relay 파괴 가드 명명 프로필 확장)·M-2(`--as` 진짜 멱등·기존-상이 fail-closed+`--force`) lock + L-1..L-5 author-close, no R37b — Fable 사전 승인). 직전: **R36 완료(2026-07-19)**: **`approved` 직행 — M 부재(R27 이후 최초), L-1·L-2 author-close(재리뷰 불요 명시)**. 직전: **R35 완료(2026-07-19)**: `pending-revision` → **author-close `approved`**(M-1(⑤ supersession 줄-앵커드 + 오소거 최악 서술 정정 + 경계 테스트 2건) lock 반영, no R35b — Fable 사전 승인). 직전: **R34 완료(2026-07-19)**: `pending-revision` → **author-close `approved`**(M-1(② 탐지 도달성 — 말미 K줄 line-anchored) lock + L-1·L-2 author-close, no R34b — Fable 사전 승인). 직전: R33 완료(2026-07-19) `pending-revision` → author-close `approved`(M-1(close 적격 명시 파라미터)·M-2(charset `:` 제거) lock + L-1·L-2, no R33b — Fable 사전 승인) → implemented `93c6283`. |
...
#### 0.24.1 — 2026-07-19 (`pending-review` R38 — **relay 룸 영속화 배선 갭 보완 — `loom relay` 포그라운드 durable 배선** (PATCH))
...
| **D3 실패 힌트(escape hatch)** | registry 생성 실패(M-23 락 충돌 포함) 시 기존 에러 메시지를 유지한 채 stderr에 1줄 힌트를 추가한다: `LOOM_RELAY_EPHEMERAL=1`(종전 인메모리 동작 복귀) 또는 `LOOM_RELAY_STATE_DIR=<다른 경로>`. **fail-closed 유지 — 자동 ephemeral 폴백 금지**(조용한 룸 소실 재도입 차단). |
| **D6 구현 배치** | **공유 헬퍼 추출 우선**: relay 패키지에 env→registry 해석 순수 함수(예: `resolveRegistryFromEnv()`)를 두고 `relay/cli.ts`와 `index.ts` 양쪽이 사용한다. **단 `relay/cli.ts`(데몬 경유 경로)의 관측 가능 동작은 무변경**이 추출의 필수 조건. 추출이 과대해지면 소규모 복제(2개소) 허용 — 구현 재량이며 R38에서 판단한다. |
| **D9 VERSION** | CLI `VERSION`(`index.ts`) + MCP serverInfo를 0.24.0 → **0.24.1**로 동기 갱신(0.23.7 미갱신 사고 선례 — CLI·MCP 동시). |
