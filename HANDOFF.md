...
| **Boot persist** | **트랙 종료(2026-07-20, 오너 옵션 B)** — kb `@reboot` crontab 2줄 · node-wsl-1 systemd 2 unit(drop-in `HOME=/root`)+Windows LogonTrigger Task. 함정 = systemd HOME 미상속 → herdr 소켓 경로 불일치 crash-loop(lessons platform (17)). **node-wsl-1 = cold-boot 실증 완료**(`wsl --shutdown`→wake, systemd 자동 herdr PID159·bridge PID162 기동). **kb = crontab 설치·정합 확인 완료 — 실제 발효 실증은 오너가 건너뜀 결정**(sudo reboot 비밀번호 필요 + `!`가 대화형 TTY 미제공 + backup job 겸용 노드라 실익<blast radius). 미결 TODO 아님 — 다음 자연 재부팅 때 `tail /tmp/loom-bridge-boot.log`로 사후확인만(현재 bridge 541727 무중단). 팩 `.loom-boot-persist-pack.md` |
