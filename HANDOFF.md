...
> 0-b. ~~**⭐ 다음 세션 = PLAN 0.23.12 웨이브 (오너 지시 2026-07-19: "2·3번 묶어서 0.23.12로 다음 세션에서 진행")** — ⓐ summary 말미 TUI 타임스탬프 소거 · ⓑ 풀 pane 균등 폭 `pane.resize` 후처리~~ → **전부 완주(2026-07-19 정오, v0.23.12 — 위 Current action 참조)**: R36 게이트(M 부재) → grok 구현(`d3afb55`, 474/0) → SMOKE-02312. ⓒ(타이밍줄 공백 복합형)도 동일 웨이브 흡수. **신규 후속 후보 2건**: ① **위상-인지 균등화** — ⓑ의 L-2(i) 유일성 가드가 라이브 중첩-좌측 위상에서 발화(균등화 미적용·스폰 무영향). herdr 스폰 split 타깃 비제어(`pane_id` 무시, 0.23.9 실측)라 위상 가변이 구조적 — 트리 구조 판별 일반화(직속 split = 최소-폭 포함 split + 서브트리 leaf-count 비율) 필요, 균등화 로직 변경이라 §5.1 게이트 판단 ② **sonnet 스모크 claude-mem 피드백 루프** — 거부 관찰이 저장→주입되어 후속 benign 스모크도 재거부(lessons (9)). 스모크 설계 대응 필요(운영 절차 후보). 잔존 Low(등재만): claude TUI 말미 비콘텐츠 줄 추가 가족 표면화 시 개별 보정 대신 여기 등재(원칙 유지).
> 0-c. **⭐ 다음 제품 트랙 = 멀티노드 단계 3 (오너 확정 2026-07-19: "다음 트랙에서 진행" + MVP 종료·프로덕션 전환)** — Loom×Herdr 권고 문서(`~/Downloads/loom-herdr-architecture.html`)의 수직 슬라이스 로드맵 중 0~2는 완료(STEP0 PoC·0.22.0 슬라이스·MCP 카드 도구), **잔여 = 단계 3 노드 복제·멀티 OS 확장**: ~~⓪ **단독 모드 기능화** — `loom relay use <name>` 명시 전환·프로필 relay별 신원 병존·`loom relay local` 라이프사이클~~ → **완주(v0.24.0 `b508a4c`+dist `2c513c9`, 2026-07-19 오후 — R37 approved → grok 3차 구현 → 독립 검증 29/0·typecheck 6/6. 위 Current action 참조).** → ① Windows relay 복귀를 **이 기능(v0.24.0)의 첫 라이브 실증으로** 수행 — **완결(2026-07-19 오후, D10 — 위 Current action 참조)**: v0.24.0 산출물 라이브 실증 5종(M-1 가드·구 SGLR 신원 `--as sglr` 흡수/D1 마이그레이션·relay use 왕복·병존·claude-impl `--as local`) + 보드 북키핑(LOOM-SGLR 35/35 done, R31 card.done 미클레임 종결) + **오너 승인 후 유예분 즉시 완결**: Windows loom-dev 룸 재수립(초대 코드 **LOOM-QSFP**·바인딩명 `win`) + 로스터 7종 전 프로필 win 바인딩 흡수(local+win 2바인딩 병존·활성=local) + sticky host 전 복원. **⚠️ 룸 소실 원인 로그 확정**: Windows relay 재시작(Scheduled Task `LoomRelayTeam` 로그온 트리거)으로 인메모리 룸 소실(LOOM-SGLR·LOOM-4HXU "No room for code"도 동일 원인) → **운영 함의: Windows 재로그온 시마다 룸 재수립 필요(LOOM-QSFP도 무효화)** → relay 룸 영속화가 신규 후속 후보. → ~~**①-b relay 룸 영속화 웨이브 (오너 지시 2026-07-19: "룸 영속화 먼저 진행" — ②보다 선행 확정)**~~ → **완주(v0.24.1 `71ace35`+dist `b544429`, 2026-07-19 오후 — 위 Current action 참조)**: **재프레임** — 영속화 체인(persist.ts `RoomSnapshotV1`·기동 복원·M-23 락)은 0.14.x부터 기구현, 근인은 `loom relay` 포그라운드 분기(`cli/index.ts:3204`)만 durable `RoomRegistry` 미배선(스코프 힌트의 "server.ts 신설·MINOR"와 달리 **배선 갭 PATCH**로 확정, lessons (11)) → R38 approved(M-1 D6 헬퍼 경계 lock) → grok 구현 → verify-0241 독립 검증 전 항목 PASS(relay 50/0·typecheck 6/6·전체 510/1 플레이크 판정) → SMOKE-0241 라이브 A~D PASS(재기동 후 동일 초대코드 join 성공 = 레지스트리 복원 핵심 실증, 초대 LOOM-D5XB). **⚠️ Windows 재배포 = 오너 유예분**(git pull/번들 갱신 + Scheduled Task `LoomRelayTeam` 재시작, 재시작 시 LOOM-QSFP 소실 → 배포 직후 룸 재수립 1회 = D10 로스터 join 절차 재사용, 배포 후부터 재로그온에도 룸 유지). **후속 후보**: 룸 스냅샷 GC(D7). → **다음 = ② WSL 노드 브릿지 복제(최전선)** → ③ Linux/VPS 노드 → ④ `@node` 라우팅 운용. 트랙 내 **SSH/git 전송 자동화 유예분 점진 도입**: ⓐ artifact fetch 자동 실행(현행 "제시까지" — 도입 시 R26 M-1/M-2 High 승격이라 **R{n} 재리뷰 필수**, plan_review R26:435) ⓑ 브릿지 자동 git push(R26:431 유예). 참고: SSH를 herdr **제어 전송**으로 쓰는 안(배치 (b))은 기각 확정(장기 이벤트 스트림 취약) — 유예분은 **artifact 전송 자동화**에 한정. **프로덕션 단계 기준**: 신규 표면은 §5.1 게이트 보수 관례 유지 + 팀 실사용(6인) 안정성이 완료 기준.
...
> **⭐ MVP 종료·프로덕션 전환 (오너, 2026-07-19).** 멀티노드 단계 3(0-c) — ⓪ 단독 모드 기능화 **완주(v0.24.0 `b508a4c`)** → ① Windows relay 복귀 **완결(D10)** → ①-b relay 룸 영속화 **완주(v0.24.1 `71ace35`+dist `b544429`, 2026-07-19 오후)**: 스코프 힌트("server.ts 신설·MINOR")를 claude-mem 교차로 재프레임 — 영속화 체인은 0.14.x부터 기구현, 근인은 `loom relay` 포그라운드 분기(`cli/index.ts:3204`)만 durable `RoomRegistry` 미배선 → **배선 갭 PATCH**(R38 approved, M-1 D6 헬퍼 경계 lock) → grok 구현 → verify-0241 독립 검증(relay 50/0·typecheck 6/6·전체 510/1 플레이크 판정) → **SMOKE-0241 A~D PASS**(재기동 후 동일 초대코드 LOOM-D5XB join 성공 = 레지스트리 복원 핵심 실증). **⚠️ Windows 상시 relay 재배포 = 오너 유예분**(git pull+Scheduled Task 재시작 → LOOM-QSFP 소실 → 룸 재수립 1회 = D10 join 절차 재사용, 배포 후 재로그온에도 룸 유지). **최전선 = ② WSL 노드 브릿지 복제**(①-b 완결로 relay 룸 영속화 선행분 종료) → ③ Linux/VPS 노드 → ④ @node 라우팅·SSH/git 전송 자동화 점진 도입. 후속 후보: 룸 스냅샷 GC(D7). npm publish는 오너 계정 대기(0-a).
>
...
| Item | Value |
|------|--------|
| **CLI / code** | **0.24.1** — `loom relay` 포그라운드 durable `RoomRegistry` 배선(룸/초대코드/멤버십 재시작 보존·D6 헬퍼 `resolveRegistryOptionsFromEnv`·D2 SIGTERM 락 해제·D11 `State: durable\|ephemeral` 로그, `71ace35`+dist `b544429`) + 단독 모드 기능화(relay별 신원 병존 `relays`·`relay use <name>`·`--as` 멱등·이종-relay 파괴 가드·`relay list`·`relay local start\|stop\|status`, 0.24.0 `b508a4c`) + claude 상태줄 chrome 필터·summary 실내용 선별·스폰 직렬화·still-running supersession(0.23.11) + pane-inject.sh + 워커 풀 탭 수평 배치(0.23.10) + done_proposal 규약 완결·conv.open deny 정합·풀 탭 배치(0.23.9) + pane 정리 정책·conv-hosts CLI(0.23.8) + still-running 유예(0.23.7) + 스크레이프 delta화·chrome 필터(0.23.6) + 주입 verify 3분기(0.23.5) + conv 멀티턴 + artifact 트리거 + agentKind 3종 |
| **PLAN** | **v0.24.1** `approved` (R38 author-close, no R38b) → **implemented** (`71ace35` + dist `b544429`) · SMOKE-0241 라이브 A~D **PASS**(재기동 후 동일 초대코드 join 성공 = 레지스트리 복원 실증). 직전: **v0.24.0** `approved`(R37)→implemented `b508a4c` · D10 라이브 실증 완료(Windows relay 복귀 — LOOM-QSFP·로스터 7종 win 바인딩 흡수). **⚠️ Windows 상시 relay 재배포 = 오너 유예분** |
| **Open blocking** | none — R24–R38 모두 closed · GitHub Issues 전부 closed |
| **Tests** | relay 스위트 **50 pass / 0 fail** · `registry-options.test.ts` 신규 182줄 · 6 pkg typecheck green · 전체 510/1(실패 1건 herdr subscribe-ACK 플레이크 — 비재현·변경면 무관, 차집합 신규 회귀 없음, lessons (10)) |
| **Herdr design** | `docs/HERDR_DESIGN.md` · **Conv spec: `docs/CONV_SPEC.md`** |
| **Remote** | `origin/main` **`b544429`** (v0.24.1 구현+dist 푸시 완료) · 시연 `docs/spikes/DISPATCH-DEMO.md` |
