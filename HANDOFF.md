...
> 0-c. **⭐ 다음 제품 트랙 = 멀티노드 단계 3 (오너 확정 2026-07-19: "다음 트랙에서 진행" + MVP 종료·프로덕션 전환)** — Loom×Herdr 권고 문서(`~/Downloads/loom-herdr-architecture.html`)의 수직 슬라이스 로드맵 중 0~2는 완료(STEP0 PoC·0.22.0 슬라이스·MCP 카드 도구), **잔여 = 단계 3 노드 복제·멀티 OS 확장**: ⓪ **단독 모드 기능화(오너 결정 2026-07-19: "기능부터 만들고 기능으로 복귀하자")** — relay 모드 전환을 제품 기능으로: `loom relay use <name>` 명시 전환(자동 failover 금지 — 룸별 보드 분기 은닉 위험)·프로필 스키마에 **relay(룸)별 peer ID·토큰 병존**(현행 "프로필 디렉터리 통째 백업/스왑" 절차 대체, 전환 시 재발급·allowlist 재등록 구조적 해소)·`loom relay local start|stop|status` 로컬 relay 라이프사이클 관리 흡수·룸 간 보드 동기화는 명시적 out of scope. 프로필 스키마 변경+CLI 표면 신설이라 **§5.1 기준 R{n} 게이트 예상**. → ① Windows relay 복귀를 **이 기능의 첫 라이브 실증으로** 수행(구 LOOM-SGLR 신원 흡수 + 보드 북키핑 정리 — R31 card.done 미클레임. 참고: Windows relay 가동 확인 완료 2026-07-19 — TCP OK·토큰 인증 101 실측) → ② WSL 노드 브릿지 복제 → ③ Linux/VPS 노드 → ④ `@node` 라우팅 운용. 트랙 내 **SSH/git 전송 자동화 유예분 점진 도입**: ⓐ artifact fetch 자동 실행(현행 "제시까지" — 도입 시 R26 M-1/M-2 High 승격이라 **R{n} 재리뷰 필수**, plan_review R26:435) ⓑ 브릿지 자동 git push(R26:431 유예). 참고: SSH를 herdr **제어 전송**으로 쓰는 안(배치 (b))은 기각 확정(장기 이벤트 스트림 취약) — 유예분은 **artifact 전송 자동화**에 한정. **프로덕션 단계 기준**: 신규 표면은 §5.1 게이트 보수 관례 유지 + 팀 실사용(6인) 안정성이 완료 기준.
...
> **⭐ MVP 종료·프로덕션 전환 선언 (오너, 2026-07-19).** ~~다음 세션 = PLAN 0.23.12 웨이브(0-b)~~ → **완주(2026-07-19 정오)**. 이제 최전선 = **다음 제품 트랙 = 멀티노드 단계 3**(0-c — **⓪ 단독 모드 기능화(오너 결정 2026-07-19) → 기능으로 relay 복귀**·노드 복제·@node 라우팅·SSH/git 전송 자동화 점진 도입. Windows relay 가동 실측 확인 완료). npm publish는 오너 계정 대기(0-a).
>
