...
| **Fable 5 when** | **R37 완료(2026-07-19)**: `pending-revision` → **author-close `approved`**(M-1(이종-relay 파괴 가드 명명 프로필 확장)·M-2(`--as` 진짜 멱등·기존-상이 fail-closed+`--force`) lock + L-1..L-5 author-close, no R37b — Fable 사전 승인). 직전: **R36 완료(2026-07-19)**: **`approved` 직행 — M 부재(R27 이후 최초), L-1·L-2 author-close(재리뷰 불요 명시)**. 직전: **R35 완료(2026-07-19)**: `pending-revision` → **author-close `approved`**(M-1(⑤ supersession 줄-앵커드 + 오소거 최악 서술 정정 + 경계 테스트 2건) lock 반영, no R35b — Fable 사전 승인). 직전: **R34 완료(2026-07-19)**: `pending-revision` → **author-close `approved`**(M-1(② 탐지 도달성 — 말미 K줄 line-anchored) lock + L-1·L-2 author-close, no R34b — Fable 사전 승인). 직전: R33 완료(2026-07-19) `pending-revision` → author-close `approved`(M-1(close 적격 명시 파라미터)·M-2(charset `:` 제거) lock + L-1·L-2, no R33b — Fable 사전 승인) → implemented `93c6283`. |
