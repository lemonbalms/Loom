...
→ **발행 경로는 함수 기준 2개(`sendFailedResult`·`finishCard`)이나 호출지점 기준 17개**
(`sendFailedResult` 7 · `finishCard` 10)로 흩어져 있다. 단일 소유권 기제는 현재
**`flights` 맵 삭제 하나뿐**이다.
...
핫패스다. 현행 이중 발행 방어(`flights.get(paneId) !== flight` 가드 4곳 — `:2003 :2067 :2080 :2096`)는
**"이벤트 핸들러가 동기적으로 맵을 갱신한다"**는 가정에 기대고 있다. 이벤트 처리를 직렬화하면
...
> `commit_unknown`은 **흡수 상태**다 — failed send의 ACK가 유실되면 거기로 들어가고, 그 상태에서는
...
않고** 단일 흡수 상태 `commit_unknown`(락 5의 용어)으로 보내고 **durable alert**(§6.7 요구 3)를 낸다.
...
**채택 규약:** seq는 **Flight에 산다.** **result payload를 새로 구성할 때 1회 증가**하고,
**재전송은 같은 seq를 재사용**한다. 전역 `cardSeq` 맵은 **삭제**한다(dispose 후에도 잔존하는
누수이기도 하다).
...
**이번 PATCH의 범위 경계 (중요):** relay idempotency **판정**은 락 9대로 **별도 PATCH**다.
이번 PATCH의 의무는 **payload에 키를 위 규약대로 각인하는 것까지**다 — payload는 **tower inbox에
영속되므로 나중에 고칠 수 없다.** 판정을 미루는 것은 되지만 각인을 미루는 것은 안 된다.
...
- [ ] **발행 단일 소유권 + 전달 추적(§6.7) — `bridge-runtime.ts:2160`·`:2207`·`:2222`
      fire-and-forget 제거 · `sendResult:2423`의 ACK 봉투 소비(`boolean` 시그니처 교체) ·
      `finishCard:2349` 실패 경로 관측 누락 보강 · 유실 시 durable alert · reconcile**
