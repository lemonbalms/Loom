/**
 * IMPL-0241 card.done 결과를 타워 보드에 반영 + 인박스 정리.
 * card.done handoff: ho_d58c707f20d00bc8 (from mac-node) — card task_7a8894b96d4cebb0.
 * 실행: env -u LOOM_RELAY_URL -u LOOM_RELAY_TOKEN LOOM_PROFILE=claude-impl bun run .loom-claim-impl-0241.ts
 * 선례: .loom-claim-r38.ts (parse→apply→accept).
 */
...
const doneId = "ho_d58c707f20d00bc8"; // [DONE] card.done from mac-node (IMPL-0241)
...
console.log(`\nIMPL-0241 board transition: ${applied.task.id} -> ${applied.status}`);
