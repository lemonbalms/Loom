...
- [verification] 2026-07-21 (40) **skip 블록 안 어서션 = 검증된 적 없는 어서션** — 소스-락·불변식은 환경 게이트 밖 무조건 실행 블록에. 활성화 시 "통과할 것" 가정 금지.

- [verification] 2026-07-21 (41) **env는 생성 시점에 latch된다** — `start()`/`init()`이 아니라 **생성자**를 확인하라. describe 본문 생성물은 모든 `beforeAll`보다 먼저다(훅 삭제는 원리적으로 늦음). `opts.x || envX()`는 down-override 불가.

- [verification] 2026-07-21 (42) **앰비언트 env 오염은 테스트를 사라지게 한다(거짓 개선)** — fail이 2→1로 *줄고* 총수 24→19·27s→0.07s. 판정은 pass/fail이 아니라 **총수+실행시간**. 밀폐는 문서 규칙이 아니라 테스트 코드가.

- [verification] 2026-07-21 (43) **세션 메모리의 이전 서술도 검증 대상** — 한 세션 오진 3회 전부 "메모리를 무검증 승계"가 원인. "확정/definitive" 라벨일수록 재실측. 인과 주장엔 대조군 선행. (33)의 메모리판.
