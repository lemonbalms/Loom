...
**MINOR 근거(버전 판정):**
- **PATCH가 아닌 이유:** 이 변경은 "완료 기준 보강·보안 범위 명확화"(§Version control PATCH 정의)가 아니라 **result 발행 권위의 구조 변경**이다 — `sendResult`의 `Promise<boolean>` **시그니처 교체**(내부 계약 변경), 새 판정 상태(`result_relay_accepted` / 흡수 상태) 도입, seq 소유권 이전(전역 `cardSeq` 맵 **삭제**), 그리고 **payload 멱등키 각인**(tower inbox에 영속되므로 사후 수정 불가).
...
| **본체** | **D4 failed send의 ACK — strict 1회 + 단일 흡수 상태 + durable alert (§6.7-bis 결정 1 확정본)** | **아키텍트 확정(§6.7-bis 축자, 재논증 금지):** **"failed send에도 strict ACK를 1회 적용한다. 비수락 결과는 유실·명시 거절을 구분하지 않고 단일 흡수 상태 **`send_unknown`**(§6.0.2)으로 보내고 durable alert(§6.7 요구 3)를 낸다."** **전제 교정 축자:** *"'A는 무한 후퇴(ACK의 ACK) 위험'이라는 반대 논거는 성립하지 않는다. `send_unknown`은 **흡수 상태**다 — failed send의 ACK가 유실되면 거기로 들어가고, 그 상태에서는 ACK를 요구하는 send가 더 이상 없다. 재귀 깊이는 구조적으로 2에서 끝난다(`needs_verification` → `failed` → 흡수). ACK 판정은 send 단위이고 send 횟수가 유한하다."* **이 선택을 결정지은 리스크(축자):** *"구현 B는 ACK 유실이라는 불확실한 관측에서 latch 해제 + dispose라는 비가역 행동을 실행한다 — 이 트랙의 불변식을 문면 그대로 위반한다. 게다가 그 경우 카드가 알림 없이 `doing`에 잔존한다. A의 잔존 리스크(카드 정지)도 실재하지만 durable alert로 운영자에게 보이므로 '조용히 멈추지 않는다'가 유지된다. 불변식이 이미 답을 정해둔 분기였다."* **durable alert가 필수인 이유(§6.7 축자):** 사람이 확정해야 done이 되는 설계에서 **사람에게 도달하지 않는 알림은 카드를 영구히 멈춘다**. **보존 상한 (아키텍트 판정 2026-07-21 — U3 범위 편입):** 흡수 상태를 **출구 없이 두지 않는다.** 락 8대로 **10분 보존 → 만료 시 structured log + counter 1건 → dispose**. 이걸 빼면 liveness를 닫겠다는 PATCH가 새 정지 상태를 만든다. **단 만료 dispose는 타이머가 유발하는 비가역 행동이라 불변식과 긴장 관계이며, R43이 명시적으로 짚어야 할 지점이다**(U3 잔여). **완료 판정:** failed send 경로가 strict 판정을 **정확히 1회** 통과하고, 비수락 시 흡수 상태 진입 + **durable alert 1건**이 남으며, 그 상태에서 발생하는 추가 send가 **0건**이고, **10분 만료 시 log+counter 1건 후 dispose**됨을 실증. **락:** 락 5 · 락 13. |
| **본체** | **D5 단일 소유권 = CAS/latch — `void`→`await` 단순 승격 금지 + 관측 누락 보강** | **함정 1(§6.7-bis 축자): `void` → `await` 단순 승격 금지.** *"`:2160`을 `await`로 바꾸면 `onCardHerdrEvent`가 async가 된다. 그런데 호출자 `onHerdrEvent:1936`은 동기 `void` 함수이고 herdr 이벤트 디스패치의 핫패스다. 현행 이중 발행 방어(`flights.get(paneId) !== flight` 가드 4곳 — `:2003 :2067 :2080 :2096`)는 '이벤트 핸들러가 동기적으로 맵을 갱신한다'는 가정에 기대고 있다. 이벤트 처리를 직렬화하면 그 가정이 깨져 **방어가 오히려 약화된다.** 단일 소유권은 `await` 승격이 아니라 CAS/latch로 얻어야 한다."* **대상 좌표(§6.7 실측 정본 표 — 좌표 재실측 2026-07-21):** `:2160` `void sendFailedResult({...})`(`pane_closed` 분기, **미해소**) · **`:2207`·`:2222` `void finishCard(flight, "failed", …)`(미해소 · 신규 발견 — 동형 결함 2건, `:2160`만 고치면 반쪽이 된다)** · `:2349` `finishCard` 말미(**부분 해소** — `sent`가 `pane.close` 게이팅에만 쓰이고 `sent === false`에서 `recordResultDeliveryUnconfirmed()`를 **호출하지 않는다**; 대칭 위치 `sendFailedResult:2391`은 호출한다 → **성공 경로의 전달 실패 관측 누락 보강**). **⭐ D5의 원자 단위 = 3종 세트 (아키텍트 확정 2026-07-21 · 증거 팩 실측):** ① **`dispose` → `send` 순서 역전** ② **발행 소유권 불리언(CAS)** ③ **`terminalPending` 레코드**. **셋은 쪼갤 수 없다.** 근거 = **순서 역전 없이 latch만 얹으면 그 latch는 영원히 발화하지 않는다** — 현행은 모든 완료 호출지점이 `disposeCardFlight` → 발행 순이라(`:2158 :2205-2207 :2219-2222 :2006 :2012 :2027`) `result_sending` 중의 terminal 이벤트가 `onHerdrEvent:1942`의 `flights.get(paneId)`에서 걸러져 **핸들러에 도달조차 못 한다**. 설계 §5 B′ 축자: *"핵심은 active `Flight`를 result send 전에 삭제하지 않는 것이다."* 락 5 근거 축자: *"권고 B는 `sendResult` 완료까지 flight를 살려두므로 `pane_closed` 핸들러가 **이미 커밋된 카드에도 발화한다.** 이것이 이 PATCH의 핵심 테스트다."* **구현자 경고: latch 필드만 추가하고 순서를 그대로 두면 유닛은 통과하는데 프로덕션에서 죽은 코드가 된다**(설계 §6.7-bis가 진단한 현행 증상을 그대로 재생산). **phase union(§6.1 12원소)은 D5에 넣지 않는다** — `terminalPending`은 설계가 **의도적으로 union 밖에 둔 별도 필드**이고(`§6.1` 축자 주석 *"phase가 아니다 — `result_sending`과 공존한다"*, §10 체크리스트도 재확인), D5가 실제로 필요로 하는 것은 **불리언 1 + latch 레코드**뿐이다(현행 `Flight`에 phase 계열 필드 **0개** — 실측). **마이그레이션 부담 없음:** §6.1 축자 *"별도 relay wire 필드는 필요 없다. `Flight` 내부 필드 또는 bridge-local registry다"* — 프로세스 수명 인메모리이므로 나중에 union을 도입해도 스키마 변경이 아니다. **비가역 각인 경보는 D5가 아니라 D6(seq 멱등키)에 걸린다.** **선례:** `stillRunningDeferral`(`:369-374` 주석 *"Set synchronously before any settle await so concurrent done/idle events are no-ops while this path owns the completion decision"*)이 이미 union 없이 불리언으로 소유권을 잡는 동형 패턴으로 검증돼 돌아가고 있다. **`terminalPending` latch:** `result_sending` 중 도착한 terminal은 **경쟁 failed를 보내지 않고 latch만** 하며, ACK 판정이 3분기로 종결한다(락 5). **3차 리뷰 배관 서술 정정 반영(§6.7-bis 축자):** `PANEDEATH-CODEX-REVIEW3.md:43`의 *"`finishCard`가 await하는 동안 terminal 핸들러가 독립적으로 failed를 쏠 수 있다"*는 **현행 코드에서 성립하지 않는다** — 모든 완료 호출지점이 `disposeCardFlight` → `finishCard` 순이라 카드는 이미 `flights`에서 사라져 있다. **실제 증상은 "done + failed 2건"이 아니라 `result_sending` 중 terminal 증거의 완전 소실**이다. **결론(락 5 필요)은 유지되고 배관 서술만 정정된다**(lessons (33) 적용례 — 검증자 지적은 결함 **존재**의 증거일 뿐 **서술**의 증거가 아니다). **완료 판정:** fire-and-forget 3곳 잔존 0건 · 가드 4곳의 동기 갱신 가정이 **깨지지 않았음**을 어서션으로 잠금 · `finishCard` 실패 경로 관측 1건. **락:** 락 5. |
| **본체** | **D6 seq 소유권 이전 + 멱등키 각인 (§6.7-bis 결정 2 확정본 · 범위 경계 포함)** | **아키텍트 확정(§6.7-bis 축자, 재논증 금지) — 제시된 두 안 모두 기각:** *"전제 교정: '`flight.seq`냐 `cardSeq`냐'로 물었으나 **둘 다 틀렸다.** `flight.seq`(스폰 고정)는 `dispatchHandoffId`와 **역할이 중복**이고, `cardSeq`(send마다 증가)는 **재전송에 새 키를 부여해 멱등키를 스스로 무효화한다.**"* **채택 규약(축자):** *"seq는 **Flight에 산다.** **result payload를 새로 구성할 때 1회 증가**하고, **재전송은 같은 seq를 재사용**한다. 전역 `cardSeq` 맵은 **삭제**한다(dispose 후에도 잔존하는 누수이기도 하다)."* **[코드 실독 확정]** 현행 전역 맵은 `bridge-runtime.ts:575` 선언 · `:1128-1129`(dispatch) · `:2372-2373`(`sendFailedResult`) 3지점에서 쓰이며 **삭제 시점이 없다.** **`(cardId, dispatchHandoffId)`만으로는 부족하다(축자):** *"결정 1의 구조상 한 dispatch 안에 서로 다른 **논리 result 2건**(거절된 `needs_verification`, 후속 `failed`)이 **합법적으로** 존재한다. 이 둘을 한 키로 뭉개면 relay의 **미실측 dedup 의미론에 correctness가 얹힌다** — 교훈 (30) 위반형이다."* **범위 경계 (중요 — 축자):** *"relay idempotency **판정**은 락 9대로 **별도 PATCH**다. 이번 PATCH의 의무는 **payload에 키를 위 규약대로 각인하는 것까지**다 — payload는 **tower inbox에 영속되므로 나중에 고칠 수 없다.** 판정을 미루는 것은 되지만 각인을 미루는 것은 안 된다."* **완료 판정:** 전역 `cardSeq` 잔존 0건 · payload 구성 1회당 seq 1 증가 · **재전송 시 seq 동일**을 실증 · `(cardId, seq, dispatchHandoffId)` 3필드가 모든 발행 payload에 각인됨. **락:** 락 9(범위 판정·보장 경계). |
| **검증** | **D7 테스트 계획 + 회귀 게이트 명기** | **신규 유닛/통합:** ① D2 주입으로 strict ACK **5조합** 각각의 종결 분기 · ② 3분기 종결(성공/명시적 거부/흡수 상태) · ③ `result_sending` 중 terminal → **`terminalPending` latch 1건 · 그 자리에서 failed 발행 0건** · ④ 흡수 상태에서 **두 번째 result 0건 · `pane.close` 0건 · dispose 0건** · ⑤ **durable alert 1건**이 남고 카드가 조용히 `doing`에 잔존하지 않음(§7.1-13) · ⑥ 재전송 seq 동일성 · ⑦ 전역 `cardSeq` 잔존 0건 · ⑧ fire-and-forget 3곳(`:2160`·`:2207`·`:2222`) 잔존 0건. **회귀 게이트(반드시 green — 명시 지정):** **`still-running.test.ts:452`("⑦ pane_closed during deferral → timer cleanup + failed once")** — **현행 이중 발행 방어의 유일한 실증**이므로 D5의 CAS/latch 재설계가 이 방어를 약화시키지 않았음의 1급 증거다 · **`pane-cleanup.test.ts:245`("① no-indicator immediate done → sendResult then pane.close exactly once")** · **`pane-cleanup.test.ts:816`("⑦ sendResult fails (relay down) → no pane.close")**. **전역 게이트:** 전 패키지 typecheck 6/6 · **전체 스위트 차집합 0**(아키텍트 독립 실행 SSOT — lessons (18); "pass 수 동일"이 아니라 **직전 기준 대비 차집합 0**으로 판정). **테스트 비용 재경고:** D2 주입 표면이 없으면 ①②④는 **작성 자체가 불가능**하다 — 이 항목의 공수를 D3 이후로 미루면 검증이 통째로 미뤄진다. |
| **검증** | **D8 게이트 = v0.27.0 MINOR · R43 필수 (author-close 불가)** | Status `pending-review` → **R43**. **경량 아님** — 발행 권위·ACK 경계는 R29/R32/R33 M-lock 인접 신뢰 경계이고 설계 §8이 독립적으로 `rgate=needed`로 판정했다. **락 후보(무변경 명시):** relay wire protocol v1 무변경 · conv wire 무변경 · MCP 도구 무변경 · herdr RPC 표면 무변경 — 바뀌는 것은 **브릿지-로컬 판정과 payload 기존 필드의 값**뿐이다. **R43이 최소 확인해야 할 것:** ① D1 실측 표가 strict 판정식을 정당화하는가(미실측 상수로 correctness를 닫지 않았는가) ② 흡수 상태의 재귀 깊이 2 논증이 실제 코드 경로에서 성립하는가 ③ seq 규약이 "한 dispatch에 논리 result 2건" 구조와 정합하는가 ④ D5의 CAS/latch가 가드 4곳의 동기 갱신 가정을 보존하는가 ⑤ payload 각인이 **영속 표면**이라는 비가역성을 범위 경계가 올바로 반영했는가. VERSION은 0.26.1 → **0.27.0** 동기 갱신 2곳(CLI `VERSION` `index.ts` + MCP serverInfo `mcp-server/src/stdio.ts`) — 0.23.7 미갱신 사고 선례. |

**Out of scope (이 버전 아님):**
- **relay/tower idempotency 판정 + inbox reconcile** — **락 9의 명시적 범위 분리**(relay 표면 변경 필요). 이번 PATCH는 **키 각인까지**만 하고 **wire exactly-once를 주장하지 않는다** — 최대 보장은 *"committed 판정이 거짓 양성을 내지 않는다"*까지이며, 문서·테스트가 말하는 **"로컬 발행 1회"(local single-issue)** 는 wire exactly-once와 **별개 어휘**다(락 9 축자).
- **conv 경로의 동일 ACK 폐기**(`bridge-runtime.ts:1300`·`:1319`·`:1777`) — §6.7이 존재를 확인했으나 이번 범위는 **card 경로**다. card 실증 후 별도 PATCH. **미해소로 남는다는 사실을 명기한다**(무결점 주장 금지).
...
**Security / trust (R43 판단 대상):**
- **신뢰 경계 자체는 무변경.** M-1 dispatch allowlist · L-2 `node`/assignee 대조 · pin 검사 전부 무변경이며, 이 PATCH는 **이미 인가된 카드의 result가 도달했는지를 추적**할 뿐이다. 권한 확대 없음.
...
- **durable alert의 내용 경계:** alert는 **메타데이터(카드 id · seq · 상태 · 시각 · 카운터)만** 싣고 워커 산출물 본문을 싣지 않는다(0.26.0 D6 계측 레코드 규약과 동형 — 본문성 §5.1 원칙 · 간접 프롬프트 주입 표면 차단).
...
- **U2** — durable alert의 전달 매체(로컬 JSONL · 보드 표면 · 타워 알림)와 운영자 도달 보장 수준. 설계 §6.7은 "durable"을 요구하나 매체를 지정하지 않는다.
- ~~**U3** — 흡수 상태의 보존 상한이 이번 PATCH 범위인가~~ → **아키텍트 판정 2026-07-21: 범위 안(D4 포함).** 근거 = 락 8이 이미 *"`send_unknown` 보존 10분 만료 시 structured log + counter 1건 발행 후 dispose"*를 규정한다(§9-bis). 이걸 빼면 이번 PATCH가 **출구 없는 흡수 상태**를 만들어, **liveness를 닫겠다는 PATCH가 새 정지 상태를 만드는 자기모순**이 된다. → **잔여 U3 (축소 재정의)**: 만료 시 `dispose`는 **타이머가 유발하는 비가역 행동**이라 이 트랙의 불변식("불확실한 관측은 회복 가능한 행동만")과 **긴장 관계**다. 락 8이 이미 그렇게 락돼 있으므로 이번 PATCH는 **따르되**, 이 긴장은 **R43 검증자가 명시적으로 짚어야 할 지점**이다 — 아키텍트가 지금 락을 뒤집을 사안이 아니다. (dispose 대상이 result 발행이 아니라 **로컬 flight 정리**이고 durable alert가 **선행**한다는 점이 완화 근거이나, 이것이 충분한지는 **미확인**.)
- **U4** — conv 경로 3지점의 동일 결함을 언제 회수할 것인가(별도 PATCH 시점).

**Review impact:** result 발행 소유권·ACK 판정·seq 규약 변경. **wire·MCP·herdr RPC 표면은 무변경**이나 `sendResult` 시그니처 교체가 `finishCard` pane.close 게이팅과 `pane-cleanup.test.ts:245`·`:816`을 함께 움직이고, payload 멱등키 각인은 **tower inbox 영속이라 비가역**이다. 설계 정본 [`docs/spikes/PANE-DEATH-DESIGN.md`](spikes/PANE-DEATH-DESIGN.md) §6.7·§6.7-bis·§9-bis 락 5·9·13. 3차 검증 원장 [`docs/reviews/PANEDEATH-CODEX-REVIEW3.md`](reviews/PANEDEATH-CODEX-REVIEW3.md)(정본·수정 금지). **R43 필수 — author-close 불가**(WORKFLOW §5.1 신규 MINOR + 신뢰 경계, 설계 §8 `rgate=needed`).
