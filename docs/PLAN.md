...
| **Fable 5 when** | **R35 완료(2026-07-19)**: `pending-revision` → **author-close `approved`**(M-1(⑤ supersession 줄-앵커드 + 오소거 최악 서술 정정 + 경계 테스트 2건) lock 반영, no R35b — Fable 사전 승인). 직전: **R34 완료(2026-07-19)**: `pending-revision` → **author-close `approved`**(M-1(② 탐지 도달성 — 말미 K줄 line-anchored) lock + L-1·L-2 author-close, no R34b — Fable 사전 승인). 직전: R33 완료(2026-07-19) `pending-revision` → author-close `approved`(M-1(close 적격 명시 파라미터)·M-2(charset `:` 제거) lock + L-1·L-2, no R33b — Fable 사전 승인) → implemented `93c6283`. |
...
#### 0.23.12 — 2026-07-19 (`pending-review` R36 요청 — **summary 말미 TUI 타임스탬프 소거 + 풀 pane 균등 폭 `pane.resize` 후처리 (0-b ⓐⓑ)** (PATCH))
...
**Review impact:** ⓐ는 R31 M-1(필터 경계) 인접 + **신규 필터 클래스(첫 줄-내부 편집)** — 경계 재판단 필요. ⓑ는 R34 ⑧(기존 op 신규 호출) 동형이나 신규 op 2종 추가. 락 인접 + 신규 클래스라 **R36 요청**(§5.1 보수 관례). 프로브 실측이 ⓑ 전제를 해소해 [가정] 없음.
