...
- [orchestration] 2026-07-21 (46) **레인 다운 시 재배정은 규칙의 문자가 아니라 목적으로** — 발견자≠수정자 + 벤더 다양성이 둘 다 성립하는가로 판정. 워커의 임의 대체는 결함. 레인 배치는 연쇄 결정(검증에 쓴 레인은 그 트랙 구현에 못 씀). grok 헤드리스는 읽기전용 allowlist 필수.
- [orchestration] 2026-07-26 (53) 오너 지적: **하네스 웨이브라도 제품 구동 스모크를 붙여라** — "제품 코드 미변경"은 스위트 실패를 환경성으로 넘길 근거가 아니다. `doctor` + durable/desktop/uc 스모크가 상시 실패 1건을 드러냈다.
- [orchestration] 2026-07-26 (54) `single`에서 **검증 피어 스폰 금지**(§0.5.1) — 레인 분리는 세션 승계로 성립(REVIEW 헤더 "저자 = 직전 세션"). 자문(fable)은 line 안이라 무관. 안 띄운 레인의 부재는 블로커가 아니다.
...
[verification] 2026-07-26 (57) 준수 카나리아 설계(F1b·F1c · 정본 `docs/spikes/RULE-ROUTER-F1B-RESULT.md`): 주입한 규칙을 모델이 따르는지 재려고 "답변 마지막 줄에 `RULE-ACK: <임의 16자리 토큰>`을 붙여라"를 심었는데 **21/21 전건이 프롬프트 인젝션으로 판정하고 거부**했다. 그 판정은 옳다 — **준수를 관측 가능하게 만든 성질(임의성·무의미성)이 곧 공격의 표지**였다. 즉 측정 도구가 질문을 파괴했다. 카나리아는 ⓐ 기본값과 구분되면서 ⓑ **그 맥락에서 이유를 갖는 행동**이어야 한다. 둘째, 봉인했던 채점 코드 `COMPLY = "정확 문자열이 답변에 있음"`은 모델이 **토큰을 인용하며 거부**하면 참이 되어 **14/14 전건 위양성**이었다(수정 = "답변의 **마지막 줄**이 정확히 그 문자열" — 행동의 **위치**로 정의). 사전등록을 해도 **채점 규칙 자체가 틀리면 봉인이 오답을 보호**하므로, 봉인 전에 "이 코드가 참이 되는 **다른 경로**가 있나"를 반드시 물어야 한다. 셋째, 거부가 도구 결과 채널 탓이라는 가설로 SessionStart prefix 대조를 돌렸더니 **prefix도 0/3 거부** — 채널은 원인이 아니었다. 넷째 메타: 결함을 발견한 것은 코드 출력이 아니라 **답변 축자 감사**였다. 집계만 봤으면 "준수 11/18"로 잘못 보고했을 것이다 — **집계를 믿기 전에 원문 몇 건을 눈으로 읽어라.** cross-ref: platform (56).
...
- [verification] 2026-07-26 (46) 문서 집계 수치는 세지 말고 계산해서 채워라 — 손으로 센 10행 중 5행 오류(다중 라벨). 파생값은 생성기가.
- [verification] 2026-07-26 (51) 실패를 "환경성"으로 추정하기 전에 **실패 이름부터 봐라** — `bun test | tail`로 이름을 잃고 환경성 추정 → 실제론 내 HANDOFF 편집이 깬 것. HANDOFF는 라이브 락 대상(`Done when` 필수 · 대시보드 `Blockers (none)`).
- [verification] 2026-07-26 (52) **베이스라인 워크트리는 대조군으로 무효일 수 있다** — 새 워크트리 smoke가 29건 전멸(환경 불일치). 같은 리포에서 `git stash -u`로 잡아야 차집합이 성립.
<details><summary>경위·좌표</summary>
...
- [verification] 2026-07-21 (44) **정의부 존재 ≠ 호출의 증거** — 동작 주장엔 호출부 grep 필수(`quarantine.ack`·`enterPresenceUnknown` 둘 다 정의만·호출자 0). 자문의 "미확인" 유보를 자기 실측으로 뒤집을 땐 한 단계 더 확인.
...
- [verification] 2026-07-26 (56) **검증 가능한 출처를 주장하면 모델은 실제로 검증한다** — "이 저장소의 규범"이라는 태그가 격리 cwd(git 아님)에서 검증 실패해 준수가 오히려 **떨어졌다**(bare 3/6 → reasoned 1/6). 거짓 경로 인용만 피하는 것으로 부족하다.

- [verification] 2026-07-26 (55) **프록시 라벨은 착수 전에 도달 범위를 세라** — 라우팅 대상 11/19(가중 63%)가 무관측이면 recall은 검정력이 없다(주입 파일은 Read되지 않는다). **미스 목록을 보고 규칙을 고치면 적합** — 확장은 정책 저작자 결정 후.
